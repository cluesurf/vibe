...
- **E-NVG-0001** - the cells carry unique O(log n) tree addresses with no cousin edges and exact neighbour reconstruction.
- **E-NVG-0002** - the addressing invariants survive growth, clean at two build sizes.
- **E-NVG-0007** - ShortLex normal forms give exact finite group orders and exact cell facet counts, integer-only.
...
- **E-NVG-0005** - on-demand neighbor computation reproduces the exact dodecagrid graph with no stored adjacency.
- **E-NVG-0004** - address arithmetic routes every signal exactly and efficiently on the heptagrid.
...
- **E-NVG-0003** - greedy hyperbolic-address routing delivers at low stretch on the dodecagrid.
- **E-NVG-0010** - local greedy descent routes with 100 percent success and unit stretch, the exact walkway, while a scrambled map stalls.
- **E-NVG-0006** - the addressing-versus-Lorentz fork, whether one substrate can hold reach, isotropy, and navigability at once.
...
- **E-NVG-0008** - the bulk depth a route reaches equals the logarithm of the boundary separation, the holographic depth-is-RG-scale law, against a linear flat-chain control.
- **E-NVG-0009** - the bulk diameter grows logarithmically (global access) while the cusp grows as N^(1/3) (locality), one substrate splitting into mind-fast and physics-local regimes.
- **E-NVG-0011** - recalling a memory is a coarse-to-fine descent of the bulk hierarchy in log(N) steps, and scrambling the coarse patterns collapses recall to chance.
- **E-NVG-0012** - a boundary loop encloses a definite bulk region, disjoint loops enclose disjoint regions, and a wider loop reaches deeper, an exact composable anchor for an object.
