...
The **renormalization** arena asks the central bridge question. The base is a discrete rule on a mesh. Real physics is smooth continuum field theory. **Coarse-graining** is the operation that connects them, averaging fine cells into coarse blocks, and the **renormalization flow** is how the effective theory changes as you climb scales.
...
- **[`E-SCL-0016`](../../../test/experiment/renormalization/screen-renderer.ts)** - (L3) the committed rule's population screen: it hides 1.58 of the log2 3 = 1.585 bits per slot at the whole-mesh scale, and two microstates with the same screen disagree later by a block distance falling as b^-2.1 (L = 15) and b^-2.0 (L = 9), the law of large numbers, while a hidden variable the rule reads breaks it until the rule erases it. On a gradient inside a block the populations alone have no law (a reversed first moment adds a disagreement that grows with b, in proportion to the slope), and populations plus the first moment along the gradient restore b^-2. See [`../scale/engine-and-screen.md`](../scale/engine-and-screen.md).
