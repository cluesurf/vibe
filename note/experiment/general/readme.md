...
The renormalization **mechanism** is concrete and correct, not asserted. Block-spin decimation is verified by direct summation, the flow runs to its fixed point, and the beta function comes out of the calculation. This nails the method that the physics arenas rely on and marks the exact boundary of what remains, running the real 4D collision through the same blocking to reach the **absolute Standard Model numbers**.
