...
72 experiments, grouped below into nine sub-themes. Every code appears once.
...
- **[`E-FND-0075`](../../../test/experiment/foundations/adjacency-invariance.ts)** - varying the ADJACENCY itself and not merely the partition, the balance is exactly zero on a square coin of degree four, a cubic coin of degree six and the D4 coin of degree twenty-four, for two rules and three geometry-free partitions, with charge conserved and the rule reversible on each, so the law is not an artefact of one complex. Momentum, which needs the coin vectors rather than only adjacency, is kept by the rotating rule and lost by the pair rule on all three, which is the anti-tautology leg showing the battery discriminates. What IS complex-relative is the richness of the available structure, the directional slot count and whether spinors exist at all.
- **[`E-FND-0076`](../../../test/experiment/foundations/recoverability-horizon.ts)** - HONEST NEGATIVE. There is no intermediate recoverability horizon. A difference built to be invisible to a coarse observer, by exchanging the whole contents of two cells sharing a block, surfaces in that observer's coarse charge TRAJECTORY within a single beat at every resolution short of the degenerate whole-system one, because streaming carries it across the block boundary and conservation forbids it vanishing. The only permanent loss is the degenerate single-block case, whose observable is the constant global charge and so distinguishes nothing. The lesson is that a coarse SNAPSHOT loses information (E-GRV-0040) while a coarse TRAJECTORY does not, so the two must not be quoted interchangeably.
...
- **[`E-FND-0062`](../../../test/experiment/foundations/complex-structure-from-distinction.ts)** - the complex unit i is built from the signed tone distinction and the beat alone (Kauffman iterant, the alternation [+1,-1] gives an element squaring to minus the identity), with a tone-alphabet control.
- **[`E-FND-0063`](../../../test/experiment/foundations/quantum-speed-limit-saturation.ts)** - the coined Dirac walk model saturates the Margolus-Levitin and Mandelstam-Tamm speed limits, so the walk model computes at the quantum speed limit set by its energy.
- **[`E-FND-0065`](../../../test/experiment/foundations/emergent-no-cloning.ts)** - the unitary walk model forbids cloning of non-orthogonal states while allowing distinguishable ones (emergent superinformation), the no-cloning theorem from the reversible rule preserving inner products.
...
- **[`E-FND-0022`](../../../test/experiment/foundations/generator-growth.ts)** - the dock is the optimal 4D kissing shell and admits no 25th direction, now checked against half-integer candidates as well as the integer shells (growth route). Read as MAXIMALITY of the configuration, not as forcing of a process: the earlier wording inferred that greedy densest growth is therefore forced to the 24 directions, which `E-FND-0077` refutes. The open tail remains, that a generic energy minimization does not self-assemble it.
- **[`E-FND-0077`](../../../test/experiment/foundations/generator-reachability.ts)** - greedy densest growth actually RUN, rather than inferred from maximality. It jams strictly below the dock in three of seven deterministic orderings, terminally at the 8-direction cross-polytope under nearest-first over integer directions, and reaches the dock exactly when the candidate set is closed under the 16 half-integer directions. So the growth route gives conditional reachability, not forcing.
- **[`E-FND-0078`](../../../test/experiment/foundations/generator-reachability.ts)** - the control that makes the self-assembly negative meaningful. The same relaxer on the same schedule reaches four of five universally optimal configurations exactly (tetrahedron, octahedron, icosahedron, cross-polytope) and misses only the dock, including a hit on the same sphere in the same dimension, and the trap holds at 55.607 degrees from 6,000 to 96,000 steps. So the miss is a landscape fact, not a weak optimizer or an unconverged run.
- **[`E-FND-0079`](../../../test/experiment/foundations/generator-reachability.ts)** - the dock basin is riddled rather than an interval: isolated pockets of success sit inside failure at low mixing fractions with a solid basin only above about 0.52, and all 46 failing starts converge to the identical rival at 55.607 degrees, so the obstruction has a name rather than being diffuse roughness.
...
- **[`E-FND-0086`](../../../test/experiment/foundations/growth-shifts-the-clock.ts)** - (L3, paper) the clock shifter is growth: domains born late keep offset clocks, a domain wall exists exactly when the offset is nonzero mod 3, and dynamically hosted defects interfere with the exact cross-term law including destructive beats
- **[`E-FND-0087`](../../../test/experiment/foundations/interactions-preserve-the-clock.ts)** - (L2, paper) the one contact interaction (the same-cell breather) is bound and clock-locked, every separated pair an exact union, and charge conjugation acts on the coarse amplitude as complex conjugation
- **[`E-FND-0088`](../../../test/experiment/foundations/all-conserving-pair-rules.ts)** - (L1, paper) all 24 reversible conserving pair rules enumerated: a cancelling vacuum clock exactly when the charge-zero class is a 3-cycle (8 of 24), and the one-phase birth restriction universal in the class
- **[`E-FND-0089`](../../../test/experiment/foundations/two-point-koopman-closure.ts)** - (L1, paper) the amplitude is not in the two-point sector: 48 percent leakage per beat under the charge rule, zero period-three eigenmodes for every rule tried on the exact 6561-state ring
- **[`E-FND-0090`](../../../test/experiment/foundations/madelung-gap.ts)** - (L2, paper) the walk obeys the quantum current-phase law J = rho grad theta / tan mass while the charge gas has phases without currents and the momentum gas currents without phases, the measured specification of what a sixth thing must couple
- **[`E-FND-0091`](../../../test/experiment/foundations/wall-measures-the-clock.ts)** - (L3, paper) the domain wall is a projective measurement of the clock class with counting weights: it amplifies exactly one offset class over an exhaustive ensemble at two sizes, leaves the others at exactly sqrt 3, and responds to separated defects as the exact sum of singles, so collapse and phase selection are substrate phenomena and the quadratic Born weight is not
- **[`E-FND-0092`](../../../test/experiment/foundations/clock-coupled-swap-addition.ts)** - (L2, paper) the first sixth-thing built and its exact negative: the clock-coupled swap is a legal reversible conserving rule that moves the density where the bare rule is static and amplifies what it moves, tightening the specification to magnitude-preserving transport
...
- **[`E-FND-0084`](../../../test/experiment/foundations/vacuum-clock-amplitude.ts)** (L2, paper) - the one amplitude the rule does supply: summing the cube root of unity to the power of each tone over the mesh, the charge rule's flashing vacuum cancels exactly over its three-beat period (1.8e-12), a defect keeps magnitude sqrt 3 with a phase locked to the vacuum clock (30, 30, 150 degrees), and amplitudes add exactly, while the exact negative is that every defect carries the same phase, so the cross term is maximal and there is no interference. The momentum rule's vacuum does not cancel (magnitude 15000, one phase), the control
- **[`E-FND-0085`](../../../test/experiment/foundations/birth-beat-interference.ts)** (L2, paper) - the clock read at different beats: a hole made in the (+1,-1) beat of the vacuum carries the tone's phase cycle negated and one beat offset, so against a tone made in the empty beat its relative phase is 60 or 300 degrees and the cross term is half of maximal, a flipped slot reaches 120 degrees with a negative cross term, amplitudes add to 2e-16 and the cross term is 2|A||B| cos of the relative phase in every case, while the momentum rule's defects keep one constant relative phase. Made by hand, which is the open construction
