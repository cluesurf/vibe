...
**165 experiments.** Codes **[`E-FRC-0001`](../../../test/experiment/gauge/actual-rule-soliton.ts)** through **[`E-FRC-0165`](../../../test/experiment/gauge/photon-light.ts)**.
...
- **[`E-FRC-0124`](../../../test/experiment/gauge/color-is-local.ts)** (L1) - color is an exact local law in the three-trit model if and only if the line table has no hop. Of 16 sum-keeping tables that make pairs from calm, 4 admit it, none with a hop, each with a calm slot's role counted as color signed by its side. The color weave on the hop-free table leaks 0 of 3,888 cell-beats, against 3,641 for the committed table.
- **[`E-FRC-0125`](../../../test/experiment/gauge/color-weave-acceptance.ts)** (L2) - fails. The hop-free table matches the committed one on reversal, charge, CPT, the vacuum, the line graph, superposition and walls, and travels farther (15 of 24 directions against 12). But a lone disturbance spreads 1.7 to 2.3 times wider in every period, which fails the comparative dressing gate.
- **[`E-FRC-0126`](../../../test/experiment/gauge/deterministic-confinement.ts)** (L2) - the Sigma(648) automaton confines with no random number: Creutz ratios 1.41 and 1.15 matching the heatbath, and a Polyakov loop near 0 in the melted phase against 0.34 on the ordered branch.
- **[`E-FRC-0127`](../../../test/experiment/gauge/fear-beat.ts)** (L1) - the cube-root swap phase as an exact, reversible, charge-conserving integer beat on loves and fears, exact for as many steps as the whole has grain, and the gate set checked universal (a Lie algebra of all 80 dimensions of su(9)).
- **[`E-FRC-0128`](../../../test/experiment/gauge/roles-on-moving-links.ts)** (L2) - roles on moving links. Links reflect through a triangle's staple, with exact reversal, energy to the unit, and a frame change in every cell commuting, and 1,814 of 1,944 links move. The index-order step breaks the frame change as the control.
- **[`E-FRC-0129`](../../../test/experiment/gauge/string-binds.ts)** (L2) - a part is bound and a whole is free, on a line, with the triality flux as an energy a hop must pay. A meson keeps a mean gap of 1.9 cells while travelling 486, against 64 with no tension, and a baryon holds together and moves freely against its antibaryon.
- **[`E-FRC-0130`](../../../test/experiment/gauge/sign-class-drift.ts)** (L1) - a lone color cannot return under the committed orientation, whose sign class is an open half-space. It is one of the 192 of 4,096 line orientations that are (a positive system of D4). The other 3,904 hold a zero-sum triangle.
- **[`E-FRC-0131`](../../../test/experiment/gauge/d4-string-binds.ts)** (L2) - moving binding on the D4 lattice with matter that waits: at the coldest demon energy a meson keeps a mean gap of 1.72 while travelling 34, against 122 with no tension, and the string melts as the demons warm.
- **[`E-FRC-0132`](../../../test/experiment/gauge/waiting-slots.ts)** (L2) - waiting slots, built as argued: exact in reversal, charge, energy, Gauss's law, local color and frame change, but a charge that cannot pay cannot wait. In a dock where no direction can be paid, 12 of the 13 places of a lone charge cross unpaid for any number of waiting slots.
- **[`E-FRC-0133`](../../../test/experiment/gauge/slot-binding.ts)** (L2) - waiting slots do not bind (1,012 of 1,024 string crossings unpaid). A crossing that bounces when unpaid binds a meson (mean gap 0.17 against 441) and a baryon when cold and melts when warm, but the pair is stuck: 0.7 docks in 600 beats against 34 in `E-FRC-0131`.
- **[`E-FRC-0134`](../../../test/experiment/gauge/links-feel-matter.ts)** (L2) - links with the hopping term in their energy and every move paid to a demon: exact reversal, energy to the unit and a frame change in every dock. A link priced without the matter term leaks energy, and matter coupled without the link breaks the frame change.
- **[`E-FRC-0135`](../../../test/experiment/gauge/matter-shapes-the-field.ts)** (L2) - static source lines make their links carry their color 0.81 of the time melted (0.48 cold, 1/9 by chance), but no string forms between separated lines, the meson is bound by contact only, and the cold-field gate fails.
- **[`E-FRC-0136`](../../../test/experiment/gauge/color-turn-weave-acceptance.ts)** (L2) - the color turn weave, a hop-free table with another turn of the couples and swap order, passes every acceptance gate of the committed knit, dressing included: love 31, 115, 265, 507 slots at side 9 against 33, 160, 565, 1,508, bounded at sides 7 and 11, 0 color leaks.
- **[`E-FRC-0137`](../../../test/experiment/gauge/color-local-dressing-search.ts)** (L1) - a lone vibe's dressing is pair creation from calm, 1 + 2 x the pairs left. The committed knit keeps it low only through its line reflection, which breaks color. One freedom at a time finds no winner in 98,468 members, and the turn and swap order together find several.
- **[`E-FRC-0138`](../../../test/experiment/gauge/finite-color-quarks.ts)** (L2) - dynamical color triplets on the deterministic Sigma(648) automaton break the center: <Re P> = 0.0853 against 0.0005 for pure gauge and 0.0009 for center-blind matter.
- **[`E-FRC-0139`](../../../test/experiment/gauge/finite-color-screening.ts)** (L2) - the same matter screens a static pair: the Polyakov correlator levels off at |<P>|^2, its connected part vanishing at R = 2. The R = 3 gate fails.
- **[`E-FRC-0140`](../../../test/experiment/gauge/generation-copies.ts)** (L2) - the three triplet copies as generations: exactly degenerate under the triality weave, split by the committed turning weave (reach 6.74, 2.74 and 1.38 by copy).
- **[`E-FRC-0141`](../../../test/experiment/gauge/generation-splitting.ts)** (L2) - fails its gate. What splits them: the committed turning weave splits all three on all 16 A2 planes (1 + 1 + 1), but a condensate on one copy under the triality weave gives only 1 + 2, leaving the other two exactly degenerate, so the gate asking for a split between them fails.
- **[`E-FRC-0142`](../../../test/experiment/gauge/parity-of-the-rule.ts)** (L2) - the committed knit tells left from right: of 1152 x 6 x 24 candidates it keeps only the identity and C with reversal, and its lone-love response is 4.1 and 4.7 times more self-dual than anti-self-dual at sides 9 and 13.
- **[`E-FRC-0143`](../../../test/experiment/gauge/vacuum-condensate-and-su2.ts)** (L2) - a Higgs-like vacuum without a Higgs mechanism: 2 of 9! two-vibe maps respect SU(2), the vacuum is a period-24 condensate leaving the same 2 lines empty at all 24 birth phases, and the only free lone vibes are the 2 leading ends of those lines.
- **[`E-FRC-0144`](../../../test/experiment/gauge/center-flux-links.ts)** (L2) - the full three-trit link, grid move and center flux, every move paid to a demon: exact reversal, energy, Gauss's law at every dock and a frame change in every dock.
- **[`E-FRC-0145`](../../../test/experiment/gauge/quark-string-forms.ts)** (L2) - a quark string forms with the full link on the melted field (excess under 0.8 per slice at R = 1 to 4, meson gap 1.48 against 65.6 while it travels). On the cold field the string gates fail.
- **[`E-FRC-0146`](../../../test/experiment/gauge/steered-slots.ts)** (L2) - steering a charge onto or off its string, with a store of energy per charge, keeps every local law of the reflecting slots exact (0 color leaks with the sign carried, about 3,750 with it fixed).
- **[`E-FRC-0147`](../../../test/experiment/gauge/steered-binding.ts)** (L2) - a bound pair travels in the slot architecture once a charge can steer: a cold meson keeps a mean gap of 1.87 while traveling 13, against 409 with no tension, and 510 of 576 headings give a pair that travels, on a round robin of all 11 line matchings, a new schedule.
- **[`E-FRC-0150`](../../../test/experiment/gauge/sigma-flux-links.ts)** (L2) - one Sigma(648) element per link beside the flux: exact reversal, energy and Gauss's law, and a frame change by all 648 elements. Setting the center phase to E mod 3 instead breaks Gauss's law at 187 of 256 docks.
- **[`E-FRC-0151`](../../../test/experiment/gauge/sigma-quark-string.ts)** (L2) - fails two gates. The string stays taut on both branches and the melted meson binds while it moves (gap 1.43 against 65.6), but no Creutz ratio can be read and the cold meson is frozen.
- **[`E-FRC-0152`](../../../test/experiment/gauge/steering-schedule.ts)** (L1) - steering needs every line to meet every other. The color turn weave's couples reach 12 of 66 pairs and no power of any of the 1,152 box symmetries more than 36, while a round robin folded into the 24-beat palindrome reaches all 66 and keeps CPT.
- **[`E-FRC-0153`](../../../test/experiment/gauge/knit-binding.ts)** (L2) - binding on the folded round robin: a cold meson keeps a mean gap of 1.27 while traveling 20, against 252 with no tension, and a baryon holds (spread 8.1 against 383). Still short of `E-FRC-0131` (travel 34, spread 2.7).
- **[`E-FRC-0148`](../../../test/experiment/gauge/color-knit-choice.ts)** (L2) - the knit chosen by a rule written before the numbers: both color-local candidates pass every gate of `E-FRC-0136`, dressing ratios and vacuum components tie, and travelers 16 against 15 choose the color turn weave.
- **[`E-FRC-0149`](../../../test/experiment/gauge/color-knit-characterization.ts)** (L2) - what changes with the knit: CPT, the ledger, the vacuum period, the walls and the generation split carry over, while the unit phase kick is lost for every protected species and the loop of turns lifts to (-1, -1) instead of (-1, +1).
- **[`E-FRC-0154`](../../../test/experiment/gauge/sigma-conjugate-pair.ts)** (L2) - the center phase and the flux as a conjugate pair: every flux change twists the link by the matching center element, exact in reversal, energy, Gauss's law and a frame change by all 648 elements. The lock is built in, not derived.
- **[`E-FRC-0155`](../../../test/experiment/gauge/sigma-string-tension.ts)** (L2) - fails three gates. The coupled field holds a flux string within a few links of taut (excess 0.9 to 2.6, against 4,207 uncoupled), but the stationary point is deconfined (Polyakov 0.58) and a cold meson's pattern is carried only 4.2 docks in 600 beats.
- **[`E-FRC-0156`](../../../test/experiment/gauge/folded-acceptance.ts)** (L2) - fails as gated. The folded round robin keeps reversal, charge, CPT, superposition, walls and local color, but a lone vibe's disturbance grows about tenfold a period (side 11: 119 to 75,050, against 33 to 2,241).
- **[`E-FRC-0157`](../../../test/experiment/gauge/baryon-steering.ts)** (L2) - fails as gated. No steering closes the baryon gap while the meson's pattern is carried: line steering holds the baryon at 2.6 but the meson's pattern stops (travel 1.8).
- **[`E-FRC-0158`](../../../test/experiment/gauge/combined-knit.ts)** (L2) - one combined knit: the head-on turn base, the four-line scatter block, the folded round robin with lone steering and the fear beat in color mode, each piece alone reproducing its own experiment bit for bit, the whole reversing exactly.
- **[`E-FRC-0159`](../../../test/experiment/gauge/combined-knit-battery.ts)** (L2) - the combined knit through every battery: no gate fails only with the fear beat on, and every quantum gate passes (CHSH 2.55 unfolded, 2.37 folded, against 2.00). The base's own gates fail on a lone fear's dressing (33 against 27), the fold and lone steering. The fear beat is adopted into the knit on this result.
- **[`E-FRC-0160`](../../../test/experiment/gauge/sigma-confined-point.ts)** (L2) - fails. The coupled Sigma(648) rule cannot hold the confined point: its drain stalls 4.4 to 9.7 units per link above the lowest level, and released it goes to beta0 7 to 9 (deconfined) or 3.8 (loops at the noise).
- **[`E-FRC-0161`](../../../test/experiment/gauge/sigma-field-string.ts)** (L2) - fails. At beta0 7.4 the field alone charges a static pair 19.3, 16.6, 30.8 per slice at R = 1 to 3, not monotonic, and the meson's pattern is barely carried (travel 2.45).
- **[`E-FRC-0162`](../../../test/experiment/gauge/sigma-cooling-moves.ts)** (L2) - covariant link moves U -> z W U^-1 W that cool the field: from 0.74 per triangle with one staple to -0.53 with 40 words, and -0.80 heat-assisted, below an annealing reference at -0.71.
- **[`E-FRC-0163`](../../../test/experiment/gauge/sigma-cooled-string.ts)** (L2) - fails the physics gates. The cooling moves hold beta0 near 13, but on D4 that point is deconfined (Polyakov 0.20 to 0.26), with no area law and a frozen meson.
- **[`E-FRC-0164`](../../../test/experiment/gauge/photon-links.ts)** (L2) - a U(1) link sector for the knit, Z_8192 angles beside integer flux as a leapfrog of two shears: exact reversal, Gauss's law at every dock and beat, and a frame change in every dock (0 mismatches). The per-link angle and flux are a modeling choice under test.
- **[`E-FRC-0165`](../../../test/experiment/gauge/photon-light.ts)** (L2) - partial. Light in the bulk substrate (the D4 box, not physical space): the longitudinal direction pinned, isotropic to 0.83 percent between two W(F4) orbits, and a Coulomb field for a static pair. The estimator counts 2 branches where the four-dimensional substrate has 3, and the Coulomb energy sits 7 to 10 percent high. Light on the 3D husk is not measured.
