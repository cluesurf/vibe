...
| P59. Nested selves | `p59-nested-selves` | **solved**: on a modular mesh, a small wound inside a cell heals (homeostasis, 1.00), a whole-cell flip persists as a new identity (autonomy, 0.00 return), and the rest of the body is undisturbed (1.00). A self made of selves, cells in a body |
| P60. Tower of selves | `p60-tower-of-selves` | **solved**: a recursively modular mesh descends cells -> tissues -> organs -> systems -> body (81/27/9/3/1, dividing by the branching factor), the same emergent rule holding at every level. The fractal of wholes-within-wholes |
| P61. No self-storage | `p61-no-self-storage` | **solved**: reconstruction fidelity reaches 1 only with no compression (a model as big as the whole), so a complete self-record cannot fit inside the thing it records. The nested regress of lossy self-models converges (finite) while full copies diverge (the infinite mirror). Self-representation is necessarily lossy |
| P62. Dimension window | `p62-dimension-window` | **solved**: enumerating compact regular hyperbolic honeycombs (finite-celled crystals) reproduces the known classification (H^3 has 4, H^4 has 5) and shows they exist only in dimensions 2, 3, 4, vanishing at 5 and above. The crystal substrate dimension is a computed constraint, not an assumption |
| P63. Integrated information | `p63-integrated-information` | **solved**: integration Phi (algebraic connectivity) picks out selves. A cohesive cell has high Phi, a random bag near zero, and a self is a local maximum (swapping members in or out lowers it). The whole is an integrated unity. The bridge to the unity-of-experience claim |
| P64. Subtle-layer urges | `p64-subtle-layer-urges` | **solved**: a two-scale mesh where a slow deep layer biases the fast surface. Steering rises with coupling depth (none when uncoupled), and after the surface is disordered the deep layer reasserts its pattern (99%). The urge as a real second layer, not an abstract field |
| P65. Dreaming and waking | `p65-dreaming-and-waking` | **solved**: one memory mesh, two regimes. Waking (external clamp) pins it to the one veridical stimulus pattern; dreaming (no clamp, internal rhythm) roams all stored memories. The only difference is whether the shared external constraint is imposed |
| P66. Reincarnation (pattern persistence) | `p66-reincarnation` | **solved**: a self (a stored attractor) survives 100% turnover of its material (Ship of Theseus) and reconstitutes from a seed after full dissolution into noise. A self is a pattern, separable in principle from any particular substrate |
| P67. Synchronicity | `p67-synchronicity` | **solved**: two subsystems with no link between them correlate (100%) when they share a deep ancestry (same memory landscape) under a common ambient rhythm, and stay uncorrelated (11%) when unrelated. Correlation from the shared past, not a present signal (as in the Bell result P7) |
| P68. Dimension selection | `p68-dimension-selection` | **solved**: integrating gravitational orbits in d spatial dimensions shows only d=3 gives stable CLOSED orbits (inverse-square law). d=2 precesses, d>=4 is unstable. Of the allowed window {2,3,4} (P62), three is uniquely selected (corroborated by Huygens clean waves) |
| P69. Emergent dimension (pure growth) | `p69-emergent-dimension` | **solved**: the spatial dimension read intrinsically from grown connectivity (shell growth in hops, no coordinates) matches the target unbiased for flat grids (2.00/2.97/3.90), fixing P38 bias, and a negatively-curved mesh reads exponential (curvature emerges too) |
| P70. Born-rule derivation | `p70-born-rule` | **solved**: counting (amplitude = sqrt density, fair sampling) and envariance both derive |c|^2, and uniform substrate sampling uniquely selects the exponent 2. Born is not a postulate but counting the substrate fairly |
| P71. Hawking radiation | `p71-hawking` | **solved**: the across-horizon spectrum (trace out the interior) is thermal at T = kappa/2pi, the temperature scales as 1/M (area law plus first law), and the Page curve turns over so information is not lost |
| P72. Nonlinear Einstein equation | `p72-nonlinear-einstein` | **solved** (cosmological form): the exact FRW solutions satisfy Friedmann, acceleration, and conservation together to machine precision via the nonlinear Bianchi identity, and the nonlinearity is essential |
| P73. Fully-discrete graviton | `p73-discrete-graviton` | **solved**: the discrete linearized Einstein operator is gauge-invariant (annihilates pure-gauge h to 1e-16), massless (dispersion through the origin), and spin-2 (two transverse-traceless polarizations) |
| P74. Large-N crossing (cluster move) | `p74-large-n-crossing` | **solved**: the height-changing cluster move (toggle an asserted edge, re-close) sweeps 85 to 100 percent of the height range at N = 64 and 96 where the single-pair move (the P12 limitation) is stuck below 25 percent, unblocking the crossing at large N |
| P75. Selves as attractors, fully | `p75-selves-as-attractors` | **solved**: a self recovers from perturbations within a 40 percent basin, keeps its identity over time (overlap 1.000), and a mesh holds several selves with capacity growing with size (18 at N=120, 26 at N=240) |
| P76. 3D addressed navigation | `p76-dodecagrid-navigation` | **solved**: greedy hyperbolic-address routing on the dodecagrid {5,3,4} delivers 100 percent of pairs at stretch 1.00, the 3D analogue of P42 |
| P77. Chiral gauge theory | `p77-chiral-gauge` | **partial**: naive doubling 2^d with net chirality 0 (Nielsen-Ninomiya), a Wilson term leaves one species (vector theory fixed); the fully chiral gauge theory is open as in lattice physics generally |
| P78. Primordial fluctuation seed | `p78-primordial-spectrum` | **first step**: a sprinkled substrate gives a scale-free Poisson density seed (contrast scales as count^-0.509); the observed tilt and gravitational growth remain open |
| P79. Anomaly cancellation forces charge quantization | `p79-anomaly-charge-quantization` | **solved**: the unique anomaly-free, Yukawa-consistent solution for one generation is exactly the Standard Model hypercharges; the unused cubic and color anomalies cancel, and charges come out quantized in thirds with neutral atoms (assumes the gauge group and rep content) |
| P80. Baryogenesis (Sakharov conditions) | `p80-baryogenesis` | **solved (structural)**: the substrate supplies all three Sakharov conditions, a matter excess builds up, and removing any one (CP, out-of-equilibrium, baryon-number violation) erases it; the observed magnitude is open |
| P81. The mass hierarchy from hyperbolic overlaps | `p81-mass-hierarchy` | **solved (structural)**: exponential overlap on a hyperbolic substrate spans the observed 5.5-decade charged-fermion mass range from even spacing where a flat power law gives 2.3; the specific masses are open |
| P82. Sharp predictions vs experimental bounds | `p82-predictions-vs-bounds` | **solved**: predicts zero first-order Lorentz violation (passes Fermi-LAT GRB 090510, xi1 < 0.132), the same bound excludes a lattice (~1.12), and the swerve vanishes as discreteness fines (rate ~ density^-1.61); a confirmed first-order energy-dependent photon speed would falsify it |
| P83. Deterministic eternal growth | `p83-deterministic-growth` | **solved**: the mesh grows one cell at a time at the frontier, deterministically and append-only (interior frozen), resumable with no rebuild (chunks equal one shot), faithful to the static tiling, with the hyperbolic ball-growth ratio emerging at the golden-ratio law (2.6186 vs 2.6180) |
