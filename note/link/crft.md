...
CRFT (Brent Borgers, 69 papers) proposes that reality is information projected from a
higher-dimensional source down to the 3D world we see, at a fixed cost. Several of its
ideas line up with structures vibe has measured. This sheet maps those, and notes which
parts sit outside what vibe's method uses.

## Shared invariants (the survivors)

Structures both frameworks arrive at, each from its own starting point. (An invariant here
is a structure that stays fixed: conserved, forced, or matching across both.)
...
- **The Borgers Impedance** (the fine-structure constant times root two, the fixed cost).
  Vibe sets its scales from the {3,4,3,4} growth rate and adds no tuned constants, so a
  single fixed impedance does not fit vibe's method.
- **The named-constant set** (137, 17, the golden ratio, root two) and the E = I w^2
  framing (energy as information times a frequency squared). These are numeric
  correspondences, and vibe's rule against fitted constants keeps them out.
- **Qualia as an octonionic projection.** Qualia is subjective experience. Vibe shares the
  octonion algebra (the eight-dimensional number system that runs D4 to E8), but treats
