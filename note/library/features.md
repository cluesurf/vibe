...
What `@cluesurf/vibe` solves for, in tables. The library builds discrete substrates and reads physics off them. The rows below are the physics it can model and measure, the substrates it can build, the operators and measures and dynamics it ships, and the headline results it has established. Each row points to where to look.

Status key for the physics rows: **solid** is verified with a control that could fail, **partial** is a real structure established with the final step open, **open** is a genuine frontier (open in physics, not just here), **honest negative** is a result correctly reporting a no.
...
| the coupling value (the 1/137 problem) | the bare rule fixes the form, not the value | honest negative, `gauge/coupling-not-fixed-3434` |
...
| curved-bulk Newton screening | clean 3D Newton is the flat cusp, the hyperbolic bulk screens | honest negative, `measure/gravity-potential` |
...
| the churn baseline | the honest no-persistence null result | `selves/bare-rule-persistence-3434` |
...
The proven, controlled results, distilled. The full scoreboard is in `note/research/vibe/notes/theory-v0.7.0/paper/recent-results.md`.
...
- `note/research/vibe/notes/theory-v0.7.0/paper/recent-results.md`, the full results scoreboard.
