...
export function directionIntention(): {
  scan: {
    K: number
    goalSteps: number
    undirectedSolved: boolean
    expectedUndirected: number
  }[]
  ...
  const scan = Ks.map(K => {
    ...
    for (let t = 0; t < trials; t++)
      if (solveUndirected({ target, rng, budget }).solved) solvedCount++
    return {
      K,
      goalSteps,
      undirectedSolved: solvedCount > trials / 2,
      expectedUndirected: Math.pow(2, K),
    }
  })
