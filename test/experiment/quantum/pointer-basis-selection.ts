...
import { pointerTrajectory, tailMean } from '@/code/dynamics/measurement'
import { experiment } from '@/test/scaffold/suite'
import { verdict } from '@/test/scaffold/verdict'

export default experiment({
  id: 'quantum/pointer-basis-selection',
  title: 'the pointer basis is einselected by the bath coupling, a record forms along the coupling axis but not the orthogonal one',
  category: 'quantum',
  substrates: ['3434'],
  depth: 'L3',
  paper: true,
  run() {
    ...
    const openX = tailMean(pointerTrajectory({ init, forward, table, beats, open: true, frontierX, axis: 0 }))
    const openY = tailMean(pointerTrajectory({ init, forward, table, beats, open: true, frontierX, axis: 1 }))
    const closedX = tailMean(pointerTrajectory({ init, forward, table, beats, open: false, frontierX, axis: 0 }))
    const closedY = tailMean(pointerTrajectory({ init, forward, table, beats, open: false, frontierX, axis: 1 }))
    ...
    const closedNoRecord = closedX < 0.5 * openX && closedY < 0.5 * openX // closed forms no record in either, no selection
    const ok = recordAlongCoupling && noRecordOrthogonal && closedNoRecord
    ...
    return verdict({
      ...
      metrics: { cells: mesh.cellCount, openAlongCoupling: Number(openX.toFixed(4)), openOrthogonal: Number(openY.toFixed(4)), closedAlongCoupling: Number(closedX.toFixed(4)), closedOrthogonal: Number(closedY.toFixed(4)) },
      ...
      control: { openOrthogonal: Number(openY.toFixed(4)), closedAlongCoupling: Number(closedX.toFixed(4)) },
      notes: 'Q5 completed, the pointer-basis selection by einselection (the bath-coupling axis).',
    })
  },
})
