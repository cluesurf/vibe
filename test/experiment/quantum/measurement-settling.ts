...
import { pointerTrajectory, loschmidtEcho, settlingTime, tailMean } from '@/code/dynamics/measurement'
import { experiment } from '@/test/scaffold/suite'
import { verdict } from '@/test/scaffold/verdict'

export default experiment({
  id: 'quantum/measurement-settling',
  title: 'measurement as deterministic settling, the open edge forms a definite irreversible record, the closed system stays coherent',
  category: 'quantum',
  substrates: ['3434'],
  depth: 'L2',
  paper: true,
  run() {
    ...
    const openTraj = pointerTrajectory({ init, forward, table, beats, open: true, frontierX })
    const closedTraj = pointerTrajectory({ init, forward, table, beats, open: false, frontierX })
    const openRecord = tailMean(openTraj) // the settled pointer (the record)
    const closedRecord = tailMean(closedTraj)
    const openSettle = settlingTime(openTraj, 0.1 * Math.max(openRecord, 1e-6))
    const openEcho = loschmidtEcho({ init, forward, inverse, table, beats: echoBeats, open: true, frontierX })
    const closedEcho = loschmidtEcho({ init, forward, inverse, table, beats: echoBeats, open: false, frontierX })

    const recordForms = openRecord > closedRecord + 0.1 && openRecord > 0.1 // the open pointer settles to a definite gradient
    ...
    return verdict({
      ...
      metrics: { cells: mesh.cellCount, openRecord: Number(openRecord.toFixed(4)), closedRecord: Number(closedRecord.toFixed(4)), settlingBeats: openSettle, openEcho: Number(openEcho.toFixed(4)), closedEcho },
      ...
      control: { closedEcho, closedRecord: Number(closedRecord.toFixed(4)) },
      notes: 'Q5, measurement-as-settling, the acknowledged frontier, an honest L2. The fine phase leaks to the wake, the coarse record settles and is irreversible. A full account still needs the pointer-basis selection shown from the lattice, noted honestly.',
    })
  },
})
