...
export default experiment({
  ...
  run() {
    ...
    const pSlow = landauZenerDiabaticProbability({
      mass: 0.25,
      force: 0.05,
    })

    const pFast = landauZenerDiabaticProbability({
      mass: 0.25,
      force: 0.16,
    })
    ...
    const masslessP = FORCES.map(force =>
      landauZenerDiabaticProbability({ mass: 0, force }),
    )
