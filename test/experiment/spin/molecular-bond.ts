...
export default experiment({
  ...
  title: 'the H2+ molecular bond, a stable molecule at the observed 2.0-Bohr length from the shared electron, the antibonding and no-resonance cases the controls',
  category: 'spin',
  substrates: ['3434'],
  depth: 'L3',
  paper: false,
  run() {
    ...
    const bondLengthMatches = Math.abs(bond.equilibriumSeparation - OBSERVED_BOND_LENGTH) < 0.1
    ...
    const electronContracts = bond.effectiveCharge > 1 && bond.effectiveCharge < 1.5

    // the bond is bound, the bonding energy at equilibrium is below the separated-atom limit
    const bondingAtEquilibrium = hydrogenMolecularIonBondingEnergy({ separation: bond.equilibriumSeparation })
    ...
    for (let r = 0.6; r < 8; r += 0.01) {
      antibondingMin = Math.min(antibondingMin, hydrogenMolecularIonAntibondingEnergy(r))
    }
    const antibondingRepulsive = antibondingMin > DISSOCIATION_LIMIT - 1e-3
    ...
    for (let r = 0.6; r < 10; r += 0.01) {
      noResonanceMin = Math.min(
        noResonanceMin,
        hydrogenMolecularIonBondingEnergy({ separation: r, includeResonance: false }),
      )
    }
    ...
    return verdict({
      ...
      metrics: {
        ...
        equilibriumSeparation: Number(bond.equilibriumSeparation.toFixed(3)),
        observedBondLength: OBSERVED_BOND_LENGTH,
        dissociationEnergyEv: Number(bond.dissociationEnergyEv.toFixed(3)),
        observedDissociationEv: OBSERVED_DISSOCIATION_EV,
        bondingEnergyAtEquilibrium: Number(bondingAtEquilibrium.toFixed(4)),
