...
const rotateZ180 = (vector: number[]): number[] => [
  -vector[0]!,
  -vector[1]!,
  vector[2]!,
] // a 2-fold axis of the icosahedron

const directionKey = (vector: number[]): string =>
  vector.map(value => Math.round(value * 1e4)).join(',')
...
export default experiment({
  ...
  run() {
    ...
    const permutesDirections = directions.every(direction =>
      keys.has(directionKey(rotateZ180(direction))),
    )
    ...
    const spinorFlipsAt2Pi =
      ...
      quaternionKey(quaternion(-1, 0, 0, 0))
    ...
    const dichotomyEstablished =
      permutesDirections && occupationReturnsAt2Pi && spinorFlipsAt2Pi
    ...
    return verdict({
      ...
      control: {
        occupationPlusOne: occupationReturnsAt2Pi ? 1 : 0,
        spinBundleMinusOne: spinorFlipsAt2Pi ? 1 : 0,
      },
      notes:
