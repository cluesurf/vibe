...
import { spinorRepEven8, vectorRep8 } from '@/code/algebra/group/so8-triality'
...
const antisymmetricNormSquared = (a: number[], b: number[]): number => 2 * (1 - overlap(a, b) ** 2)
// squared norm of the symmetric (boson) two-particle state, 2(1 + p^2). Nonzero even when the states coincide.
const symmetricNormSquared = (a: number[], b: number[]): number => 2 * (1 + overlap(a, b) ** 2)

export default experiment({
  id: 'spin/fermi-exclusion',
  title: 'two identical fermions cannot share a state (the antisymmetric amplitude vanishes) while bosons pile up, the Pauli principle measured',
  category: 'spin',
  substrates: ['3434'],
  depth: 'L3',
  paper: true,
  run() {
    ...
    const fermionSameState = antisymmetricNormSquared(fermionA, fermionA)
    // two DISTINCT fermions coexist (nonzero antisymmetric amplitude)
    const fermionDistinctState = antisymmetricNormSquared(fermionA, fermionB)
    ...
    const ok = exclusion && distinctCoexist && bosonsPileUp && fermionExchangeSign === -1 && bosonExchangeSign === +1
    ...
    return verdict({
      ...
      metrics: {
        fermionSameStateNormSquaredTimes1000: Math.round(fermionSameState * 1000),
        fermionDistinctStateNormSquared: Math.round(fermionDistinctState * 1000) / 1000,
        bosonSameStateNormSquared: Math.round(bosonSameState * 1000) / 1000,
        fermionExchangeSign,
        bosonExchangeSign,
      },
      control: { bosonSameStateNormSquared: Math.round(bosonSameState * 1000) / 1000 },
      notes:
