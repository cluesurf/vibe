...
export default defineExperiment({
  ...
  run() {
    ...
    const ops2I: GroupOps<Quaternion> = { multiply, inverse: conjugate, key: quaternionKey }
    ...
    for (const element of group) {
      ...
      if (!representativeByRotation.has(id)) representativeByRotation.set(id, element)
    }
    const rotations = [...representativeByRotation.keys()] // 60 rotations = A5
    const opsSplit: GroupOps<SplitElement> = {
      multiply: (left, right) => ({
        rotation: rotationKey(multiply(representativeByRotation.get(left.rotation)!, representativeByRotation.get(right.rotation)!)),
        sign: left.sign * right.sign,
      }),
      inverse: (value) => ({ rotation: rotationKey(conjugate(representativeByRotation.get(value.rotation)!)), sign: value.sign }),
    ...
    const centralElement: SplitElement = { rotation: identityRotation, sign: -1 }
    const centralIsCommutatorSplit = contains(commutatorSplit, centralElement, opsSplit)
    ...
    const loopHolonomyMinusOne = quaternionKey(pureSquared) === quaternionKey(minusOne)
    const rotationOrderTwo = rotationKey(pureSquared) === identityRotation && rotationKey(pure) !== identityRotation

    const ok =
      isPerfect && minusOneIsCommutator && !centralIsCommutatorSplit && loopHolonomyMinusOne && rotationOrderTwo
    ...
    return verdict({
      ...
      control: { splitCommutatorOrder: commutatorSplit.length, splitCentralIsCommutator: centralIsCommutatorSplit ? 1 : 0 },
      notes:
