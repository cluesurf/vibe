...
function diagonalVacuum(diagonal: number[]): Octonion[][] {
  ...
  for (let i = 0; i < diagonal.length; i++) matrix[i]![i] = octonionReal(diagonal[i]!)
  return matrix
}

export default experiment({
  id: 'spin/three-generations-breaking-search',
  title: 'three generation slots with an exact family symmetry, splittable only by an external vacuum, so the hierarchy stays open',
  category: 'spin',
  substrates: ['3434'],
  depth: 'L2',
  paper: true,
  run() {
    ...
    const threeIdempotents = frame.length === 3 && frame.every((e) => isJordanIdempotent(e))
    let pairwiseOrthogonal = true
    for (let i = 0; i < 3; i++) for (let j = i + 1; j < 3; j++) if (!areJordanOrthogonal(frame[i]!, frame[j]!)) pairwiseOrthogonal = false
    ...
    const masses = (vacuum: ReturnType<typeof octonionMatrixIdentity>): number[] =>
      frame.map((e) => octonionMatrixTrace(jordanProduct(e, vacuum)))
    const spread = (m: number[]): number => Math.max(...m) - Math.min(...m)
    const distinctCount = (m: number[]): number => new Set(m.map((x) => Math.round(x * 1e6) / 1e6)).size
    ...
    const asymmetricSplits = spread(asymmetricMasses) > 1e-3 && distinctCount(asymmetricMasses) === 3
    ...
    const ratioOf = (m: number[]): number => { const s = [...m].sort((a, b) => a - b); return s[0]! !== 0 ? Math.round((s[2]! / s[0]!) * 100) / 100 : 0 }
    ...
    const familyStructureReal = threeIdempotents && pairwiseOrthogonal && cyclicIsAutomorphism
    const hierarchyIsGeometric = false // Boyle's conjecture is NEGATIVE for this model, the hierarchy is not derived

    const ok = familyStructureReal && symmetricDegenerate && asymmetricSplits && ratioUnconstrained && dynamicsCannotBreak && !hierarchyIsGeometric
    return verdict({
      ...
      metrics: {
        ...
        symmetricSpreadTimes1000: Math.round(spread(symmetricMasses) * 1000),
        asymmetricSpreadTimes1000: Math.round(spread(asymmetricMasses) * 1000),
      ...
      control: { symmetricSpreadTimes1000: Math.round(spread(symmetricMasses) * 1000) },
      notes:
