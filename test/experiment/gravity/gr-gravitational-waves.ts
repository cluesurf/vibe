...
function radiatedPower(
  m1: number,
  m2: number,
  a: number,
): { measured: number; formula: number; ok: boolean } {
  ...
  const formula = quadrupoleRadiatedPower({
    mass1: m1,
    mass2: m2,
    separation: a,
  })
  ...
  return {
    measured,
    formula,
    ok: Math.abs(measured - formula) / formula < 1e-9,
  }
}
...
function chirp(
  ...
  m2: number,
  a0: number,
): { exponent: number; ok: boolean; chirpMass: number } {
  ...
  return {
    ...
    chirpMass: chirpMassOf({ mass1: m1, mass2: m2 }),
  }
}
