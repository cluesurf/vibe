...
export default experiment({
  ...
  title: 'curved-bulk gravity is exponentially screened, the 1/r Newton tail is a flat-cusp property not a bulk one',
  category: 'gravity',
  substrates: ['534'],
  depth: 'L2',
  paper: true,
  run() {
    ...
    const hyp = buildCoxeterMesh({ symbol: [5, 3, 4], depth: 20, maxChambers: 40000 })
    const n = hyp.cellCount
    let center = 0
    for (let i = 1; i < n; i++) if (hyp.neighbors[i]!.length > hyp.neighbors[center]!.length) center = i
    const dist = neighborDistances({ neighbors: hyp.neighbors, size: n, source: center })
    let maxD = 0
    for (let i = 0; i < n; i++) if (dist[i]! > maxD) maxD = dist[i]!
    const bulk = greensDecayClass({ neighbors: hyp.neighbors, size: n, center, rlo: 1, rhi: maxD - 1 })
    ...
    for (let x = 0; x < L; x++) for (let y = 0; y < L; y++) for (let z = 0; z < L; z++) {
      const i = idx(x, y, z)
      for (const [dx, dy, dz] of steps) { const a = x + dx!, b = y + dy!, c = z + dz!; if (a >= 0 && a < L && b >= 0 && b < L && c >= 0 && c < L) flatN[i]!.push(idx(a, b, c)) }
    }
    const flat = greensDecayClass({ neighbors: flatN, size: N, center: idx(L >> 1, L >> 1, L >> 1), rlo: 2, rhi: 10 })

    const bulkExponential = bulk.exponential && bulk.expR2 > 0.85 && bulk.expRate < -0.1 // curved bulk: screened, exponential wins
    const cuspPowerLaw = !flat.exponential && flat.powR2 > 0.9 && flat.powSlope < -0.6 // flat cusp: Newton power law wins
