...
import { spatialMetricBootstrap, staticMetricPhotonDeflection } from '@/code/dynamics/static-metric-photon'

const RS = 1

export default experiment({
  id: 'gravity/schwarzschild-from-bootstrap',
  title: 'the Schwarzschild metric derived by the gravity-gravitates bootstrap, the resummed spatial metric giving the factor-two light bending',
  category: 'gravity',
  substrates: ['3434'],
  depth: 'L2',
  paper: false,
  run() {
    ...
    const bootstrapResults = radii.map((r) => {
      const trail = spatialMetricBootstrap({ x: RS / r, iterations: 60 })
      ...
      return { r, converged, exact, linearFirstOrder, residual: Math.abs(converged - exact) }
    })
    const bootstrapConverges = bootstrapResults.every((b) => b.residual < 1e-6)
    // the resummation matters, the converged metric differs from the linear first-order one (the nonlinear terms)
    const resummationMatters = bootstrapResults.some((b) => Math.abs(b.converged - b.linearFirstOrder) > 0.1)

    // the derived metric A = 1/B = 1 - r_s/r matches Schwarzschild
    const aDerived = radii.map((r) => 1 / (1 / (1 - RS / r)))
    const aMatchesSchwarzschild = aDerived.every((a, i) => Math.abs(a - (1 - RS / radii[i]!)) < 1e-9)
    ...
    const weakB = 100
    const fullDeflection = staticMetricPhotonDeflection({ schwarzschildRadius: RS, impactParameter: weakB, spatialMetric: 'full' })!
    const timeOnlyDeflection = staticMetricPhotonDeflection({ schwarzschildRadius: RS, impactParameter: weakB, spatialMetric: 'flat' })!
    ...
    const factorTwoFromResummation = Math.abs(fullProduct - 2) < 0.1 && Math.abs(timeOnlyProduct - 1) < 0.1 && Math.abs(spatialHalfRatio - 2) < 0.05
    const ok = bootstrapConverges && resummationMatters && aMatchesSchwarzschild && factorTwoFromResummation
    ...
    return verdict({
      ...
      metrics: {
        bootstrapConvergedAt3: Number(bootstrapResults[1]!.converged.toFixed(6)),
        bootstrapExactAt3: Number(bootstrapResults[1]!.exact.toFixed(6)),
        maxResidual: Number(Math.max(...bootstrapResults.map((b) => b.residual)).toExponential(2)),
