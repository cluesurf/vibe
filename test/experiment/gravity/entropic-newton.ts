...
import { screenBitSeries, ballRegion, logLogExponent, verlindeForceLaw } from '@/code/measure/entropic-gravity'
...
export default experiment({
  ...
  title: 'the static Newtonian 1/r force from the measured area law, the Verlinde route, no new field',
  category: 'gravity',
  substrates: ['3434'],
  depth: 'L2',
  paper: false,
  run() {
    ...
    const groundPerVolume = ground.bits.map((bits, i) => bits / ground.volumes[i]!)

    // the volume-law control, a thermal (maximally mixed) state has C = (1/2) I, so S = volume * ln 2
    const thermalBits = RADII.map(radius => ballRegion({ side: SIDE, radius }).length * Math.log(2))
    const thermalExponent = logLogExponent(RADII, thermalBits)
    const thermalPerVolume = thermalBits.map((bits, i) => bits / ground.volumes[i]!)

    // the Verlinde force law implied by each scaling, F proportional to 1/N(r) proportional to r^(-exponent)
    const groundForce = verlindeForceLaw({ bitExponent: groundExponent, tolerance: 0.5 })
    const thermalForce = verlindeForceLaw({ bitExponent: thermalExponent, tolerance: 0.5 })
    ...
    const exponentsSeparated = groundExponent < 2.6 && thermalExponent > 2.9
    const newtonFromArea = groundForce.isNewtonian
    const wrongFromVolume = !thermalForce.isNewtonian
    const ok = groundSubExtensive && thermalExtensive && exponentsSeparated && newtonFromArea && wrongFromVolume
    ...
    return verdict({
      ...
      metrics: {
        ...
        groundForceExponent: Number(groundForce.forceExponent.toFixed(3)),
        groundPotentialExponent: Number(groundForce.potentialExponent.toFixed(3)),
        thermalForceExponent: Number(thermalForce.forceExponent.toFixed(3)),
        groundNewtonian: groundForce.isNewtonian ? 1 : 0,
        thermalNewtonian: thermalForce.isNewtonian ? 1 : 0,
        groundEntropyPerVolumeStart: Number(groundPerVolume[0]!.toFixed(3)),
        groundEntropyPerVolumeEnd: Number(groundPerVolume[groundPerVolume.length - 1]!.toFixed(3)),
        groundSubExtensive: strictlyDecreasing(groundPerVolume) ? 1 : 0,
        thermalEntropyPerVolume: Number(thermalPerVolume[0]!.toFixed(3)),
      },
      control: { thermalExponent: Number(thermalExponent.toFixed(3)), thermalNewtonian: thermalForce.isNewtonian ? 1 : 0 },
      notes:
