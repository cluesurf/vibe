...
import { screenBitSeries, ballRegion } from '@/code/measure/entropic-gravity'
...
export default experiment({
  ...
  title: 'Newton G is the single scale, the bounded area-law bit density fixes G in lattice units, the volume law has no definite G',
  category: 'gravity',
  substrates: ['3434'],
  depth: 'L2',
  paper: false,
  run() {
    ...
    const areaDensity = series.bits.map((bits, i) => bits / surfaces[i]!)
    // the volume-law thermal density (S = volume * ln 2 per boundary cell), should grow with the region
    const thermalDensity = RADII.map((radius, i) => (ballRegion({ side: SIDE, radius }).length * Math.log(2)) / surfaces[i]!)
    ...
    const thermalGrows = thermalDensity[thermalDensity.length - 1]! > thermalDensity[0]! * 1.2
    ...
    return verdict({
      ...
      metrics: {
        ...
        areaDensityFar: Number(areaDensity[areaDensity.length - 1]!.toFixed(3)),
        thermalDensityNear: Number(thermalDensity[0]!.toFixed(3)),
        thermalDensityFar: Number(thermalDensity[thermalDensity.length - 1]!.toFixed(3)),
