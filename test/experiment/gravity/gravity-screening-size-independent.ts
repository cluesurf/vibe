...
export default experiment({
  ...
  paper: true,
  run() {
    const bulkSmall = buildCellGraph({ symbol: [3, 4, 3, 4], maxCells: 9000 })
    const bulkLarge = buildCellGraph({ symbol: [3, 4, 3, 4], maxCells: 170000 })
    ...
    const flatRisesTowardOne = flatRhoLate > flatRhoEarly && flatRhoLate > 0.7
    ...
    return verdict({
      ...
      control: {
        ...
        flatRhoEarly: Number(flatRhoEarly.toFixed(4)),
        flatRhoLate: Number(flatRhoLate.toFixed(4)),
        bulkRhoSizeDelta: Number(Math.abs(bulkRhoSmall - bulkRhoLarge).toFixed(4)),
      },
      notes:
