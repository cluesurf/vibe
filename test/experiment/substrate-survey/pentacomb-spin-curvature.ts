...
const contains24Cell = (symbol: number[]): boolean =>
  symbolContainsSubdiagram(symbol, [3, 4, 3])
...
export default experiment({
  ...
  run() {
    ...
    const hyperbolic =
      pentaSignature.negative === 1 && pentaSignature.zero === 0
    ...
    const spinorDirections =
      has24Cell && splits && spinor8sCarries && spinor8cCarries
    ...
    const controlFlat =
      euclideanControl.negative === 0 && euclideanControl.zero >= 1
    ...
    return verdict({
      ...
      control: {
        euclideanControlNegative: euclideanControl.negative,
        euclideanControlZero: euclideanControl.zero,
      },
      notes:
