...
export function fdFoundational(): { neighborsOk: boolean; closes: boolean; countConserved: boolean; momentumConserved: boolean; reversible: boolean } {
  ...
  const step = (occ: number[]): number[] => streamD4({ occupancy: collide(occ), neigh })
  const stepInv = (occ: number[]): number[] => collide(streamD4Inverse({ occupancy: occ, neigh, opp }))
  ...
  const T = 40
  let occ = occ0.slice()
  const c0 = d4Count(occ), m0 = d4Momentum({ occupancy: occ, roots })
  let countConserved = true, momentumConserved = true
  for (let t = 0; t < T; t++) { occ = step(occ); if (d4Count(occ) !== c0) countConserved = false; const m = d4Momentum({ occupancy: occ, roots }); if (!m.every((x, q) => x === m0[q]!)) momentumConserved = false }
