...
export function unifiedEmergence(): { partitions: boolean; vectorIsBoson: boolean; spinorsAreFermions: boolean; oneSubstrateBothSectors: boolean } {
  // partition the 24 directions: 8v (axis), 8s (even-parity half), 8c (odd-parity half)
  const v8: Q[] = ([0, 1, 2, 3] as const).flatMap((i) => [1, -1].map((s) => { const q = [0, 0, 0, 0]; q[i] = s; return q as unknown as Q }))
  const half: Q[] = []
  ...
  const vectorSign = (v: Q): number => { const r = qmul(qmul(rot2pi, v), qconj(rot2pi)); return r.every((x, i) => Math.abs(x - v[i]!) < 1e-9) ? 1 : -1 }
  // 8s/8c transform as SPINORS (left multiply q psi): the -1 hits, spinor goes to -1 (FERMION)
  const spinorSign = (psi: Q): number => { const r = qmul(rot2pi, psi); return r.every((x, i) => Math.abs(x - psi[i]!) < 1e-9) ? 1 : (r.every((x, i) => Math.abs(x + psi[i]!) < 1e-9) ? -1 : 0) }
