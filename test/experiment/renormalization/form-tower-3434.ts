...
export function formTower(): { real: number[]; nul: number[]; tower: boolean } {
  ...
  const step = (): void => perceptionMatchingSweep3d({ tone, matched: m, length: L, rng })
  for (let f = 0; f < 60; f++) step()
  const blocks = [1, 2, 4, 8]
  const blockGroups = blocks.map((b) => cubicBlockGroups({ size: L, blockSize: b }))
  const coarse = (t: Int8Array, bi: number): Float64Array =>
    coarseFieldByGroup({ field: t, group: blockGroups[bi]!.group, groupCount: blockGroups[bi]!.groupCount })
  const LAG = 10, M = 30
  const realS: Float64Array[][] = blocks.map(() => []), nulS: Float64Array[][] = blocks.map(() => [])
  for (let f = 0; f < M + LAG; f++) { step(); const sh = shuffledToneField({ tone, rng }); blocks.forEach((_, bi) => { realS[bi]!.push(coarse(tone, bi)); nulS[bi]!.push(coarse(sh, bi)) }) }
  const persist = (ser: Float64Array[]): number => Math.round(lagAutocorrelation({ series: ser, lag: LAG, epsilon: 1e-9 }) * 100) / 100
