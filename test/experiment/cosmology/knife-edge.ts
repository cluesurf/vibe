...
export default experiment({
  ...
  title: 'the genesis threshold is exactly arrow zero at every lattice size, the void is a scale-free measure-zero dead point',
  category: 'cosmology',
  substrates: ['534'],
  depth: 'L2',
  paper: true,
  run() {
    ...
    for (const maxChambers of caps) {
      ...
      sizes.push(n)
      const z = chargeTrajectory({ neighbors: mesh.neighbors, initial: new Int8Array(n), beats: 150, arrow: 0, seed: 9 })
      const t = chargeTrajectory({ neighbors: mesh.neighbors, initial: new Int8Array(n), beats: 150, arrow: 0.005, seed: 9 })
    ...
    return verdict({
      ...
      metrics: { smallest: sizes[0]!, largest: sizes[sizes.length - 1]!, deadAtZeroAllSizes: thresholdAtZero ? 1 : 0, minTinyArrowLife: Number(Math.min(...aliveAtTiny).toFixed(3)) },
      // CONTROL: the arrow-zero void is exactly 0 (not merely small) at every size, an exact threshold.
      control: { deadCountsAtZero: deadAtZero.join(' ') === '0 0 0' ? 1 : 0 },
      notes: 'G9, the knife-edge exact at every size. Grounds nothing-cannot-hold as scale-free.',
    })
  },
})
