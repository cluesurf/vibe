...
export default experiment({
  ...
  title: 'the per-beat expansion is the growth-ratio invariant of the symbol, fixed and greater than 1, not a free knob',
  category: 'cosmology',
  substrates: ['534'],
  depth: 'L2',
  paper: true,
  run() {
    const ratioOf = (symbol: number[], cap: number) => lastCompleteShellRatio(buildCoxeterMatrixMesh(symbol, cap).shells)
    const symbols: Array<[string, number[]]> = [['{7,3}', [7, 3]], ['{8,3}', [8, 3]], ['{5,4}', [5, 4]], ['{5,3,4}', [5, 3, 4]], ['{3,4,3,4}', [3, 4, 3, 4]]]
    ...
    return verdict({
      ...
      metrics: Object.fromEntries(symbols.map(([name], i) => [name, Number(ratios[i]!.toFixed(3))])),
      ...
      control: { ratio73small: Number(r73small.toFixed(3)), ratio73large: Number(r73large.toFixed(3)), distinctRates: distinct ? 1 : 0 },
      notes: 'G4, the expansion rate is the growth-ratio invariant (why-the-mesh-expands, sub-question 3).',
    })
  },
})
