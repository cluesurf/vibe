...
import { chargeTrajectory, attractorSignature, balanceToZero } from '@/code/dynamics/genesis'
import { experiment } from '@/test/scaffold/suite'
import { verdict } from '@/test/scaffold/verdict'

export default experiment({
  id: 'cosmology/attractor-signature',
  title: 'the genesis attractor is canonical, a definite density and balance and clustering reached from any start',
  category: 'cosmology',
  substrates: ['534'],
  depth: 'L3',
  paper: true,
  run() {
    ...
    const sigFrom = (initial: Int8Array) => {
      const r = chargeTrajectory({ neighbors: mesh.neighbors, initial, beats: 250, arrow: 0.1, seed: 9 })
      return attractorSignature({ tone: r.end, neighbors: mesh.neighbors })
    }
    const a = sigFrom(new Int8Array(n)) // from the void
    const b = sigFrom(dense) // from a dense start

    const densityCanonical = Math.abs(a.density - b.density) < 0.05 && a.density > 0.1
    const balanced = Math.abs(a.netBalance) < 0.02 && Math.abs(b.netBalance) < 0.02 // conservation, net near zero
    const clusteringCanonical = Math.abs(a.sameSignFraction - b.sameSignFraction) < 0.08
    ...
    return verdict({
      ...
      metrics: { cells: n, densityVoid: Number(a.density.toFixed(3)), densityDense: Number(b.density.toFixed(3)), netBalanceVoid: Number(a.netBalance.toFixed(4)), sameSignVoid: Number(a.sameSignFraction.toFixed(3)), sameSignDense: Number(b.sameSignFraction.toFixed(3)) },
      ...
      control: { densityGap: Number(Math.abs(a.density - b.density).toFixed(4)), clusterGap: Number(Math.abs(a.sameSignFraction - b.sameSignFraction).toFixed(4)) },
