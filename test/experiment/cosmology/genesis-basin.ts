...
import { chargeTrajectory, balanceToZero } from '@/code/dynamics/genesis'
import { experiment } from '@/test/scaffold/suite'
import { verdict } from '@/test/scaffold/verdict'

export default experiment({
  id: 'cosmology/genesis-basin',
  title: 'the initial state is irrelevant, every start converges to the same living balance, the universe is an attractor',
  category: 'cosmology',
  substrates: ['534'],
  depth: 'L3',
  paper: true,
  run() {
    ...
    const dist = neighborDistances({ neighbors: mesh.neighbors, size: n, source: center })

    // four deterministic, total-charge-zero initial conditions
    const theVoid = new Int8Array(n)
    const pairSeed = (() => { const t = new Int8Array(n); t[center] = 1; t[mesh.neighbors[center]![0]!] = -1; return t })()
    const denseAlt = balanceToZero((() => { const t = new Int8Array(n); for (let i = 0; i < n; i++) t[i] = i % 2 === 0 ? 1 : -1; return t })())
    const block = balanceToZero((() => { const t = new Int8Array(n); for (let i = 0; i < n; i++) if ((dist[i] ?? 9) <= 3) t[i] = i % 2 === 0 ? 1 : -1; return t })())
    const ics: Array<[string, Int8Array]> = [['void', theVoid], ['pair', pairSeed], ['dense', denseAlt], ['block', block]]

    const lateFraction = (initial: Int8Array, arrow: number): number => {
      const r = chargeTrajectory({ neighbors: mesh.neighbors, initial, beats: 200, arrow, seed: 9 })
    ...
    return verdict({
      ...
      metrics: { cells: n, onFractions: Number(onSpread.toFixed(4)), minLiving: Number(Math.min(...onFracs).toFixed(3)), maxLiving: Number(Math.max(...onFracs).toFixed(3)) },
      ...
      notes: 'G1, the initial-state problem dissolved. Fractions: ' + onFracs.map((f) => f.toFixed(3)).join(' '),
    })
  },
})
