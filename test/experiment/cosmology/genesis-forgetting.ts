...
import { chargeTrajectory, differenceTrajectory, balanceToZero } from '@/code/dynamics/genesis'
import { experiment } from '@/test/scaffold/suite'
import { verdict } from '@/test/scaffold/verdict'

export default experiment({
  id: 'cosmology/genesis-forgetting',
  title: 'the deep past washes out, two different starts converge to the same macroscopic state under the arrow',
  category: 'cosmology',
  substrates: ['534'],
  depth: 'L2',
  paper: true,
  run() {
    ...
    const dense = balanceToZero((() => { const t = new Int8Array(n); for (let i = 0; i < n; i++) t[i] = i % 2 === 0 ? 1 : -1; return t })())
    const sparse = balanceToZero((() => { const t = new Int8Array(n); for (let i = 0; i < n; i += 9) t[i] = (i / 9) % 2 === 0 ? 1 : -1; return t })())

    const A = chargeTrajectory({ neighbors: mesh.neighbors, initial: dense, beats: 200, arrow: 0.1, seed: 9 })
    const B = chargeTrajectory({ neighbors: mesh.neighbors, initial: sparse, beats: 200, arrow: 0.1, seed: 9 })
    const fracDiff = (i: number) => Math.abs(A.trajectory[i]! - B.trajectory[i]!) / n
    ...
    const micro = differenceTrajectory({ neighbors: mesh.neighbors, initialA: dense, initialB: sparse, beats: 200, arrow: 0.1, seed: 9 })
    const same = differenceTrajectory({ neighbors: mesh.neighbors, initialA: dense, initialB: dense.slice(), beats: 30, arrow: 0.1, seed: 9 })
    ...
    return verdict({
      ...
      metrics: { cells: n, startFractionGap: Number(startFracDiff.toFixed(3)), endFractionGap: Number(endFracDiff.toFixed(3)), microStart: micro.startDiff, microEnd: micro.endDiff },
      ...
      notes: 'G2, the forgetting rate. The macroscopic observable forgets; the residual microscopic difference is the attractor manifold, not start-memory.',
    })
  },
})
