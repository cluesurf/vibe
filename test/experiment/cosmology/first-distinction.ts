...
export default experiment({
  ...
  title: 'the first distinction is a single balanced adjacent (+1,-1) pair born from peace, conserving zero',
  category: 'cosmology',
  substrates: ['534'],
  depth: 'L1',
  paper: true,
  run() {
    const mesh = buildCoxeterMesh({ symbol: [5, 3, 4], depth: 14, maxChambers: 4000 })
    const n = mesh.cellCount
    // a tiny arrow so the first creation is a single isolated event, not many at once
    const first = firstDistinction({ neighbors: mesh.neighbors, cells: n, arrow: 0.002, seed: 9, maxBeats: 5000 })
    const control = firstDistinction({ neighbors: mesh.neighbors, cells: n, arrow: 0, seed: 9, maxBeats: 5000 })
    ...
    const ok = happened && balanced && adjacent && minimal && controlSilent
    ...
    return verdict({
      ...
      metrics: { cells: n, beatsToFirst: first.beatsToFirst, plus: first.plus, minus: first.minus, adjacent: adjacent ? 1 : 0 },
