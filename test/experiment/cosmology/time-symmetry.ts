...
export default experiment({
  ...
  title: 'the arrow as broken time-reversal symmetry, the symmetric point is the unique dead one',
  category: 'cosmology',
  substrates: ['534'],
  depth: 'L2',
  paper: true,
  run() {
    ...
    const seedStruct = balanceToZero((() => { const t = new Int8Array(n); for (let i = 0; i < n; i++) if ((dist[i] ?? 9) <= 4) t[i] = i % 2 === 0 ? 1 : -1; return t })())

    const endFrac = (initial: Int8Array, arrow: number) => {
      const r = chargeTrajectory({ neighbors: mesh.neighbors, initial, beats: 250, arrow, seed: 9 })
    ...
    const ok = symmetricVoidDead && brokenVoidLives && symmetricStructDecays && brokenStructLives
    ...
    return verdict({
      ...
      metrics: { cells: n, voidAtZero: Number(voidAtZero.toFixed(3)), voidTinyArrow: Number(voidTiny.toFixed(3)), structAtZero: Number(structAtZero.toFixed(3)), structBroken: Number(structBroken.toFixed(3)) },
      ...
      notes: 'G6, the arrow as broken time-reversal symmetry (arrow Layer 5).',
    })
  },
})
