...
export default experiment({
  ...
  title: 'backward evolution reaches a unique single seed chamber in finite steps, the substrate began',
  category: 'cosmology',
  substrates: ['534'],
  depth: 'L2',
  paper: true,
  run() {
    ...
    const finitePast = peelSteps === maxDepth + 1 && peelSteps < Infinity // a finite geometric past
    const shrinksMonotonically = monotonic && counts[0]! > counts[counts.length - 1]!
    ...
    return verdict({
      ...
      metrics: { cells: n, maxDepth, peelSteps, seedSize, fullSize: counts[0]! },
      ...
      notes: 'G3. The tone history is irreversible (annihilation loses info), but the GEOMETRY has a definite beginning. The tone at the seed is peace, and genesis does the rest.',
    })
  },
})
