...
import { chargeTrajectory, gardenOfEdenFraction } from '@/code/dynamics/genesis'
import { experiment } from '@/test/scaffold/suite'
import { verdict } from '@/test/scaffold/verdict'

export default experiment({
  id: 'cosmology/minimal-seed',
  title: 'the minimal seed is nothing, every seed boots the same attractor, and the rule is non-injective (Garden-of-Eden)',
  category: 'cosmology',
  substrates: ['534'],
  depth: 'L2',
  paper: true,
  run() {
    ...
    const lateFrac = (seedPairs: number) => {
      ...
      for (let k = 0; k < seedPairs; k++) { t[2 * k] = 1; t[2 * k + 1] = -1 }
      const r = chargeTrajectory({ neighbors: mesh.neighbors, initial: t, beats: 200, arrow: 0.1, seed: 9 })
    ...
    const seedIrrelevant = Math.max(fromVoid, fromOne, fromMany) - Math.min(fromVoid, fromOne, fromMany) < 0.05 && fromVoid > 0.1

    // Garden-of-Eden / irreversibility on a small 8-cell ring (3^8 states enumerated)
    const ring = Array.from({ length: 8 }, (_, i) => [(i + 7) % 8, (i + 1) % 8])
    const goe = gardenOfEdenFraction({ neighbors: ring, cells: 8, arrow: 0.1, seed: 9 })
    ...
    return verdict({
      ...
      metrics: { cells: n, fromVoid: Number(fromVoid.toFixed(3)), fromOnePair: Number(fromOne.toFixed(3)), fromManyPairs: Number(fromMany.toFixed(3)), goeFraction: Number(goe.goeFraction.toFixed(3)), injectivity: Number(goe.injectivity.toFixed(3)) },
      ...
      notes: 'G8, the minimal seed is nothing; the Garden-of-Eden fraction shows the irreversibility.',
    })
  },
})
