...
export function controlsBattery(): { c1: boolean; c2: boolean; c4: boolean; c5: boolean } {
  ...
  function d4Shells(R: number): number[] {
    const roots: number[][] = []
    for (let i = 0; i < 4; i++) for (let j = i + 1; j < 4; j++) for (const si of [1, -1]) for (const sj of [1, -1]) { const v = [0, 0, 0, 0]; v[i] = si; v[j] = sj; roots.push(v) }
    const key = (p: number[]): string => p.join(','); const dist = new Map([['0,0,0,0', 0]]); let frontier = [[0, 0, 0, 0]]
    const shells = [1]
    for (let r = 1; r <= R; r++) { const next: number[][] = []; for (const p of frontier) for (const root of roots) { const q = p.map((x, k) => x + root[k]!); const kk = key(q); if (!dist.has(kk)) { dist.set(kk, r); next.push(q) } } shells.push(next.length); frontier = next }
    return shells
  }
  function betheShells(R: number, deg: number): number[] {
    const shells = [1]; let prev = 1
    for (let r = 1; r <= R; r++) { const cur = r === 1 ? deg : prev * (deg - 1); shells.push(cur); prev = cur }
    return shells
  }
  ...
  const L = 9
  const reflect = (v: number[]): number[] => v.map((_, i) => v[(L - i) % L]!)
  const symmetric = (v: number[]): number[] => v.map((_, i) => (v[(i + 1) % L]! + v[(i + L - 1) % L]!) / 2) // left+right
  const chiral = (v: number[]): number[] => v.map((_, i) => v[(i + L - 1) % L]!) // shift right only
  const commutes = (rule: (v: number[]) => number[]): boolean => {
    const v = Array.from({ length: L }, (_, i) => i * i % 7)
    const a = rule(reflect(v)), b = reflect(rule(v))
  ...
  const Ax2 = Ax.map((row, x) => row.map((a, y) => a + g[x]![y]! - g[wrap(x + 1)]![y]!))
  const Ay2 = Ay.map((row, x) => row.map((a, y) => a + g[x]![y]! - g[x]![wrap(y + 1)]!))
  const plaq = (ax: number[][], ay: number[][], x: number, y: number): number => ax[x]![y]! + ay[wrap(x + 1)]![y]! - ax[x]![wrap(y + 1)]! - ay[x]![y]!
  const massTerm = (ax: number[][], ay: number[][]): number => { let s = 0; for (let x = 0; x < Lg; x++) for (let y = 0; y < Lg; y++) s += ax[x]![y]! ** 2 + ay[x]![y]! ** 2; return s } // m^2 A^2 (Proca)
