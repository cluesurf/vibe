...
export default experiment({
  ...
  run() {
    ...
    return verdict({
      ...
      notes:
        "the open flag is the charge knit's CPT: within {C, P(single-axis mirror), T(velocity reversal)} no CPT-type conjugation is exact for it, and nature's CPT nulls are the strongest in physics, so either a restoring conjugation exists in the larger 24-cell point group (possibly combined with a clock-phase coset), or the committed table has a real CPT problem. That hunt is the named follow-up. E-FND-0011's T and CPT lines hold only on its linear special state under the naive convention and are corrected by this experiment. Sakharov scorecard: C violated (here), CP violated (here), departure from equilibrium (the growth arrow), all three present in the base. CORRECTED 2026-09-01 by E-FND-0101 (the full 96-conjugation sweep): the charge knit DOES have exact CP (negation with the antidiagonal mirror) and exact CPT (negation, point inversion, T), so the no-CPT flag here was an artifact of sweeping only the axis mirror, the group-sweep cousin of the window artifact. The violations measured in this file stand for the specific conjugations tested, and the located CP violation moves to the traveller knit, where E-FND-0101 shows no negation-containing conjugation is exact over the whole group.",
    })
  },
})
