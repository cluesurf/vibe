...
import { cartanMatrix, rootsD4, isRootSystem, icosahedronVertexDirections } from '@/code/algebra/group/root-system'
import { diagramAutomorphismOrder, outerAutomorphismOrder } from '@/code/algebra/group/automorphism'

// basis vector e_i in `dim` dimensions with the given nonzero entries.
function basis(dim: number, ...entries: Array<[number, number]>): number[] {
  const v = new Array<number>(dim).fill(0)
  for (const [index, value] of entries) v[index] = value
  return v
}
const chain = (dim: number, count: number) => Array.from({ length: count }, (_, i) => basis(dim, [i, 1], [i + 1, -1]))
const aRoots = (n: number) => chain(n + 1, n)
const dRoots = (n: number) => [...chain(n, n - 1), basis(n, [n - 2, 1], [n - 1, 1])]
const bRoots = (n: number) => [...chain(n, n - 1), basis(n, [n - 1, 1])]
const cRoots = (n: number) => [...chain(n, n - 1), basis(n, [n - 1, 2])]
const f4Simple = [[0, 1, -1, 0], [0, 0, 1, -1], [0, 0, 0, 1], [0.5, -0.5, -0.5, -0.5]]
const g2Simple = [[0, 1, -1], [1, -2, 1]]

export default experiment({
  id: 'foundations/generator-triality-unique',
  title: 'D4 is the unique simple root system with triality, which forces the {3,4,3,4} dock',
  ...
  run() {
    ...
    const trialitySystems = systems.filter((s) => diagramAutomorphismOrder(cartanMatrix(s.simple)) === 6).map((s) => s.label)
    const uniqueD4 = trialitySystems.length === 1 && trialitySystems[0] === 'D4'
    ...
    const icosaCrystallographic = isRootSystem(icosahedronVertexDirections())

    const ok = uniqueD4 && d4Outer === 6 && dockCrystallographic && !icosaCrystallographic
    return verdict({
      status: ok ? 'pass' : 'fail',
      claim:
      ...
      metrics: { trialitySystems: trialitySystems.length, d4OuterOrder: d4Outer, systemsSwept: systems.length },
