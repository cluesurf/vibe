...
import { Quaternion, binaryTetrahedral, multiply, conjugate, quaternionKey, quaternion } from '@/code/algebra/group/quaternion'
import { closure, commutatorSubgroup, contains, GroupOps } from '@/code/algebra/group/finite-group'
import { reflectionClosure, rootsF4, rootsD4, vectorKey, dotVec } from '@/code/algebra/group/root-system'
import { vectorRep8, spinorRepEven8, spinorRepOdd8, applyTriality, vectorSetsEqual } from '@/code/algebra/group/so8-triality'
...
export default experiment({
  ...
  title: 'the 24 Hurwitz quaternions are the group 2T with a genuine spinor (nonsplit double cover)',
  category: 'foundations',
  substrates: ['3434'],
  depth: 'L1',
  paper: true,
  run() {
    const ops: GroupOps<Quaternion> = { multiply, inverse: conjugate, key: quaternionKey } // unit quaternion inverse = conjugate
    ...
    const ok = units.length === 24 && group.length === 24 && nonsplit && quotient === 12
    return verdict({
      status: ok ? 'pass' : 'fail',
      claim:
      ...
      metrics: { units: units.length, groupOrder: group.length, quotientOrder: quotient, nonsplit: nonsplit ? 1 : 0 },
    })
  },
})
...
experiment({
  ...
  title: 'reflecting the four F4 simple roots rebuilds the 24-cell dock and its dual (48 roots)',
  category: 'foundations',
  substrates: ['3434'],
  depth: 'L1',
  paper: true,
  run() {
    ...
    const longRoots = closed.filter((v) => Math.abs(dotVec(v, v) - 2) < 1e-6)
    const shortRoots = closed.filter((v) => Math.abs(dotVec(v, v) - 1) < 1e-6)
    const longIsDock = sameSet(longRoots, rootsD4())
    const matchesF4 = sameSet(closed, rootsF4())
    const ok = closed.length === 48 && longRoots.length === 24 && shortRoots.length === 24 && longIsDock && matchesF4
    return verdict({
      status: ok ? 'pass' : 'fail',
      claim:
      ...
      metrics: { roots: closed.length, longRoots: longRoots.length, shortRoots: shortRoots.length },
    })
  },
})
...
experiment({
  ...
  title: 'octonion (SO(8)) triality splits the dock into 8v + 8s + 8c, a vector and two spinors',
  category: 'foundations',
  substrates: ['3434'],
  depth: 'L1',
  paper: true,
  run() {
    ...
    const octets = vector.length === 8 && spinorEven.length === 8 && spinorOdd.length === 8
    ...
    const trialityStep = vectorSetsEqual(applyTriality(vector), spinorEven) // 8v -> 8s under triality
    ...
    return verdict({
      ...
      metrics: { vector: vector.length, spinorEven: spinorEven.length, spinorOdd: spinorOdd.length, unionSize: new Set(union.map(vectorKey)).size },
    })
  },
})
