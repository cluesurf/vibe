...
import { rootsD4, ternaryShells } from '@/code/algebra/group/root-system'
import { isKissingConfiguration, maxPairwiseCosine, canExtendKissing, coordinationAtMinAngle, deterministicSpiral, relaxRiesz } from '@/code/geometry/packing'
import { gridRefinementRewrite, bulkDegree } from '@/code/substrate/graph-rewrite'
import { ballGrowthDimension } from '@/code/measure/dimension'

const cosToDeg = (c: number) => Math.round(Math.acos(Math.min(1, c)) * 180 / Math.PI)
...
export default experiment({
  ...
  title: 'the dock is the optimal 4D kissing shell, so densest growth is forced to the 24 directions',
  ...
  run() {
    ...
    const candidates = [...(shells.get(1) ?? []), ...(shells.get(2) ?? []), ...(shells.get(3) ?? [])]
    const cannotExtend = !canExtendKissing(dock, candidates, 60)
    const ok = dock.length === 24 && kissing && minAngle === 60 && cannotExtend
    return verdict({
      status: ok ? 'pass' : 'fail',
      claim:
      ...
      metrics: { directions: dock.length, minAngleDegrees: minAngle, kissing: kissing ? 1 : 0 },
      notes: 'the 4D kissing number is 24 (Musin); the no-25th check is over a dense deterministic candidate set',
    })
  },
})
...
experiment({
  ...
  title: 'a generic start does not self-assemble the dock under local energy minimization (honest open)',
  category: 'foundations',
  substrates: ['3434'],
  depth: 'L1',
  paper: true,
  run() {
    ...
    const relaxed = relaxRiesz(start, { steps: 6000, powerStart: 0.5, powerEnd: 4, stepSize: 0.01 })
    const relaxedMinAngle = cosToDeg(maxPairwiseCosine(relaxed))
    const coordination = coordinationAtMinAngle(relaxed)
    const eightRegular = Object.keys(coordination).length === 1 && coordination[8] === 24
    ...
    return verdict({
      ...
      metrics: { startMinAngleDegrees: startMinAngle, relaxedMinAngleDegrees: relaxedMinAngle, targetDegrees: 60, eightRegular: eightRegular ? 1 : 0 },
    })
  },
})
...
experiment({
  ...
  title: 'a content-free graph rewrite gives a definite geometry but not the dock (the long shot, honest open)',
  category: 'foundations',
  substrates: ['3434'],
  depth: 'L1',
  paper: true,
  run() {
    ...
    const dimension = ballGrowthDimension({ neighbors: graph.neighbors, centers, maxRadius: 8 })
    const degree = bulkDegree(graph)
    const reproducesDock = Math.abs(dimension - 4) < 0.3 && degree === 24
    return verdict({
      ...
      metrics: { emergentDimension: Math.round(dimension * 100) / 100, bulkDegree: degree, targetDimension: 4, targetDegree: 24 },
    })
  },
})
