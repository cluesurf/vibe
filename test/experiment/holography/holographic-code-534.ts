...
export default defineExperiment({
  ...
  run() {
    ...
    const recoveredAt30 = recoverByMajority(corruptConnectedRegion(size, 0.3, logical)) === logical
    const recoveredAt49 = recoverByMajority(corruptConnectedRegion(size, 0.49, logical)) === logical
    ...
    const lostAt51 = recoverByMajority(corruptConnectedRegion(size, 0.51, logical)) !== logical
    ...
    const localAlwaysRecoverable = localFraction < 0.5 && recoverByMajority(corruptConnectedRegion(size, localFraction, logical)) === logical
    ...
    const ok = recoveredAt30 && recoveredAt49 && lostAt51 && localAlwaysRecoverable && nonRedundantBeforeError && nonRedundantLostOnSingleError
    ...
    return verdict({
      ...
      control: { nonRedundantLostOnSingleError: nonRedundantLostOnSingleError ? 1 : 0 },
      notes:
