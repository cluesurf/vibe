...
export default experiment({
  ...
  run() {
    const roots = rootsD4()
    const grounded = [AXIS_A, AXIS_B, ...FOUR_DIRECTIONS].every(v =>
    ...
    const sameAxisResidual = maxPointGap(
      composedSameAxis,
      predicted,
      points,
    )
    ...
    const diffAxisResidual = maxPointGap(
      composedDiffAxis,
      predicted,
      points,
    )
    ...
    const ok =
      grounded && sameAxisResidual < 1e-9 && diffAxisResidual > 1e-2
