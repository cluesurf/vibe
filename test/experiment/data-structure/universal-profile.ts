...
export default experiment({
  ...
  run() {
    ...
    const flatSmall = tessellationDataProfile({
      symbol: [4, 3, 4],
      maxCells: 500,
    }).growthRatio
    ...
    const ok =
      allCapacityExponential &&
      allTreeDepthLogarithmic &&
      profiles.length === 42 &&
      flatDecreasesTowardOne &&
      hyperbolicStaysBounded
    ...
    return verdict({
      ...
      control: {
        flatGrowthSmall: flatSmall,
        flatGrowthLarge: flatLarge,
        flatDecreasesTowardOne: flatDecreasesTowardOne ? 1 : 0,
        hyperbolicStaysBounded: hyperbolicStaysBounded ? 1 : 0,
      },
      notes:
