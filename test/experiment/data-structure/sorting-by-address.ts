...
export default defineExperiment({
  ...
  paper: true,
  run() {
    const addressing = buildAddressing({ symbol: [3, 4, 3, 4], maxCells: 3000 })
    const stats = addressingStats(addressing)
    const order = addressing.address.map((_, cell) => cell).sort((x, y) => lexCompare(addressing.address[x]!, addressing.address[y]!))
    ...
    for (let i = 1; i < order.length; i++) if (lexCompare(addressing.address[order[i - 1]!]!, addressing.address[order[i]!]!) >= 0) strictlyOrdered = false
    ...
    return verdict({
      ...
      metrics: { cells: stats.cellCount, addressesUnique: stats.allUnique ? 1 : 0, strictlyOrdered: strictlyOrdered ? 1 : 0 },
      ...
      notes: 'SS7 of experiments/17. The address that sorts (here) also gives BFS order (SS8) and the trie prefixes (SS3). A comparison sort costs O(N log N), reading the address order costs O(N).',
    })
  },
})
