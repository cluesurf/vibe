...
export default defineExperiment({
  ...
  run() {
    ...
    const everyCellVisitedOnce = a.dist.every((d) => d >= 0) && histogram.reduce((s, h) => s + h, 0) === cells
    ...
    for (let d = 0; d <= reliable; d++) if (histogram[d] !== a.shellSizes[d]) frontierEqualsShell = false
    ...
    return verdict({
      ...
      metrics: { cells, depth: maxDepth, everyCellVisitedOnce: everyCellVisitedOnce ? 1 : 0, frontierEqualsShell: frontierEqualsShell ? 1 : 0 },
      ...
      notes: 'SS8 of experiments/17. BFS is the growth order, DFS is the radial parent chain. Sorting (SS7) reads this order by address.',
    })
  },
})
