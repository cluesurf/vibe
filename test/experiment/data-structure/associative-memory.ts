...
export default experiment({
  ...
  run() {
    ...
    const smallAccuracy = vsaRecallAccuracy({ dim: smallDimension, items })
    const largeAccuracy = vsaRecallAccuracy({ dim: largeDimension, items })
    ...
    const capacityScalesWithRadius = largeAccuracy > 0.9 && largeAccuracy > smallAccuracy + 0.2
    ...
    return verdict({
      ...
      metrics: { items, largeDimension, largeAccuracy, capacityScalesWithRadius: capacityScalesWithRadius ? 1 : 0 },
      ...
      notes: 'DS10 of experiments/16. Capacity equals the cell count in a radius (DS1), here read as binding capacity.',
    })
  },
})
