...
export default defineExperiment({
  ...
  run() {
    ...
    let running = 0
    for (const n of levels) { running += n; cumulative.push(running) }
    let strictlyNested = true
    for (let i = 1; i < cumulative.length; i++) if (cumulative[i]! < cumulative[i - 1]!) strictlyNested = false
    const total = cumulative[cumulative.length - 1]!
    // the number of bounding levels is logarithmic in the cells
    const logarithmicLevels = bins <= 4 * Math.log2(total)

    const ok = strictlyNested && logarithmicLevels && levels.length === bins
    ...
    return verdict({
      ...
      metrics: { bins, totalCells: total, strictlyNested: strictlyNested ? 1 : 0, logarithmicLevels: logarithmicLevels ? 1 : 0 },
      ...
      notes: 'SS12 of experiments/17. The horoball hierarchy is the radial structure, shared with the mipmap (DS7).',
    })
  },
})
