...
// world lines and read off its screen. E-FRC-0100 shows the color-symmetric pair unitaries form one
...
//
// The carrier. A color label cannot ride a tone of the committed rule in general, because the vacuum
...
// (code/dynamics/color-worldline). On a side-7 D4 mesh over 49 beats, 6 of the 48 (tone, direction)
...
//
// The shader. Each excitation carries a color qutrit, A red, B green, C blue. At every collision event
// (two excitations in one cell when the collision acts) the pair's qutrits get U(phi), the operator
// E-FRC-0100 built (pairExchangeUnitary). The tones never read the color, so every tone observable is
// the same at every phi. The screen observable is which excitation ends holding which color: the
...
// probability of U(phi), and is computed exactly over the six color assignments: it agrees with the
...
// cannot change is checked too: the total color content (each color held by exactly one excitation
// in total probability), the su(3) quadratic Casimir expectation, and the color-singlet weight, all
// fixed at every phi and every beat because U(phi) commutes with every global color rotation.
//
...
import {
  ...
  type ColorState,
  type Seed,
} from '@/code/dynamics/color-worldline'
import {
  combineOperators,
  epsilonState,
  liftGenerator,
...
function singletWeight(state: ColorState): number {
...
export default experiment({
  id: 'gauge/color-shader',
  code: 'E-FRC-0105',
  title:
    'a color qutrit on three exactly preserved excitations of the committed rule, with the color-symmetric unitary U(phi) at each of their 15 encounters, makes which excitation ends holding which color a continuous function of phi, a permutation at phi = 0 and pi and fractional between, differing from the matched classical random swap by interference between orders of encounters, while every tone observable, the color content, the Casimir weight and the singlet weight stay fixed',
  category: 'gauge',
  substrates: ['3434'],
  depth: 'L2',
  paper: false,
  run() {
    ...
    const C = casimir()
    const initial = colorBasisState({ colors: COLORS, d: 3 })
    ...
    const results = PHASES.map(phase => {
      ...
      for (const event of lines.events) {
        ...
        for (const color of COLORS) {
          const held = [0, 1, 2].reduce(
            (s, slot) =>
              s + colorProbability({ state, d: 3, slot, color }),
            0,
          )
      ...
      const quantum = colorProbability({
        state,
        d: 3,
        slot: 0,
        color: 0,
      })
      const classical = classicalColorProbability({
        events: lines.events,
    ...
    const firstEventOnly = PHASES.every(phase => {
      ...
      const quantum = colorProbability({
        ...
        color: COLORS[firstEvent.first] ?? 0,
      })
    ...
    return verdict({
      status: ok ? 'pass' : 'fail',
      claim:
        'three excitations the committed rule preserves exactly (support one at every beat, exact superposition through all 15 encounters) carry a color qutrit each, and the color-symmetric unitary at each encounter turns the probability that A ends red from 1 at phi = 0 to 0 or 1 at phi = pi through fractional values between, with A entangled in between and the matched classical random swap agreeing only at single encounters and the two classical points, while the color content, the su(3) Casimir weight and the singlet weight never move',
      metrics: {
      ...
      notes:
        'L2, stated plainly: U(phi) is added by hand, the one ingredient E-FND-0080 and E-FRC-0100 show the base lacks, and nothing in the committed rule chooses phi. What the rule supplies is the carrier (the only world lines it preserves exactly are single-slot excitations over the vacuum, 6 of 48 lone starts on this mesh, while a tone as such has no identity through the create, flip and annihilate moves) and the order and timing of encounters. A shared cell is taken as the encounter, although the collision acts on separate lines and the tones do not scatter: the classical dynamics is exactly linear here, so the color exchange is the only interaction. What phi changes on the screen is who carries which color and how entangled the carriers are. What it cannot change: every tone observable (the tones never read color), the color content, and every su(3) invariant of the color register, because U(phi) commutes with every global color rotation. Pair creation in the color singlet was not used: the committed table turns every empty wire line into a pair and every (-1, +1) wire state back into an empty line, so allowing annihilation only from the singlet would veto moves the committed table performs, which changes the rule.',
    })
  },
})
