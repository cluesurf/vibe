...
const MEASUREMENTS: Record<GaugeGroup, number> = { u1: 900, su2: 300, su3: 80, su4: 60 }
const BIN = 5

type Estimate = { value: number; error: number }
type Rates = Record<LoopRepresentation, { rate: Estimate; curvature: Estimate; chi2: number; points: { g: number; rho: Estimate }[] }>

function forceRatio(tables: readonly number[][][]): number {
  const table = [0, 1, 2].map(r =>
    [0, 1, 2].map(t => tables.reduce((sum, s) => sum + (s[r]?.[t] ?? 0), 0) / tables.length),
  )

  return creutzRatioFromTable({ table, r: 2, t: 2 }) / creutzRatioFromTable({ table, r: 1, t: 1 })
}

function runningRates(input: { group: GaugeGroup; seed: number; tree: number }): Rates {
  const { group } = input
  const n = groupSize({ group })
  const couplings = group === 'u1' ? ABELIAN_COUPLINGS : MATCHED.map(x => x / n)
  const representations: LoopRepresentation[] =
    group === 'u1' ? ['fundamental'] : ['fundamental', 'adjoint']
  const points: Record<LoopRepresentation, Estimate[]> = { fundamental: [], adjoint: [] }

  couplings.forEach((gSquared, index) => {
    const rng = makeRng({ seed: input.seed + index })
    const lattice = makeGaugeLattice({ group, lengths: [BOX, BOX, BOX, BOX], start: 'cold', rng })
  ...
  const fit = (list: Estimate[]): Rates['fundamental'] => {
    if (list.length === 0) {
      return { rate: { value: 0, error: 0 }, curvature: { value: 0, error: 0 }, chi2: 0, points: [] }
    }

    const result = weightedLinearFit({
      xs: couplings,
      ys: list.map((p, i) => (p.value - input.tree) / (couplings[i] ?? 1)),
  ...
  return { fundamental: fit(points.fundamental), adjoint: fit(points.adjoint) }
}
...
export default experiment({
  ...
  run() {
    ...
    const tree = treeLevelCreutz({ r: 2, box: BOX }) / treeLevelCreutz({ r: 1, box: BOX })
    ...
    const groups = NON_ABELIAN.map((group, index) => ({
      ...
      quarkCasimir: (groupSize({ group }) ** 2 - 1) / (2 * groupSize({ group })),
    ...
    const kappas = groups.map(g => ({
      ...
      error: Math.hypot(g.rates.fundamental.rate.error, alpha.error) / g.casimir,
    }))
    const kappaPulls = [pull(kappas[0]!, kappas[1]!), pull(kappas[0]!, kappas[2]!), pull(kappas[1]!, kappas[2]!)]
    ...
    const representation = groups.map(g => {
      ...
      return {
        ...
        sameRatePull:
          (g.rates.adjoint.rate.value - g.rates.fundamental.rate.value) /
          Math.hypot(g.rates.adjoint.rate.error, g.rates.fundamental.rate.error),
    ...
    const sameForAdjoint = representation.every(r => Math.abs(r.sameRatePull) < 3)
    // the three groups are independent measurements, so their pulls against the source-charge
    // alternative combine in quadrature
    const combinedSourceChargePull = Math.hypot(...representation.map(r => r.alternativePull))
    ...
    const ok =
      treeMatches && antiscreens && proportionalToCa && sameForAdjoint && sourceChargeExcluded && fitsHold
    ...
    const impliedScaleRatio = Math.exp(kappa / tree / ((2 * 11) / (48 * Math.PI ** 2)))
    ...
    return verdict({
      ...
      metrics: {
        ...
        largestSameRatePull: Math.max(...representation.map(r => Math.abs(r.sameRatePull))),
        smallestSourceChargePull: Math.min(...representation.map(r => Math.abs(r.alternativePull))),
