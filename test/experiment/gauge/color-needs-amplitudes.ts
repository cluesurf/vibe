// What would a local rule need in order to carry SU(3) color? E-FRC-0093 to 0097 found the committed
...
//
// 1. Every permutation of the 3^k joint slot states that commutes with the color algebra, found by an
//    exhaustive search (and for a pair, by brute force over all 9! = 362,880). Color-symmetric
...
// 2. The color-symmetric pair unitaries form a circle, U(phase) = P_sym + e^{i phase} P_anti (or with
...
//    (identity and swap), and every other member entangles color, with entangling power up to 1/4.
//    So amplitudes are the one addition that lets a color-symmetric rule DO something.
// 3. A tone that is both the clock and the color breaks color: the rule's vacuum clock (the tone
//    cycle t -> t + 1) commutes with only a 3-dimensional abelian part of u(3). A slot that is a clock
//    trit times a separate color qutrit keeps all nine.
//
...
export default experiment({
  id: 'gauge/color-needs-amplitudes',
  code: 'E-FRC-0100',
  title:
    'a color-symmetric permutation rule can only relabel slots (2 of 9! pair permutations, 1 for a color-anticolor pair, 6 for three slots), the color-symmetric pair unitaries are a circle whose only classical points are identity and swap, every other point entangles, and a tone that is also the clock keeps only 3 of the 9 color generators',
  category: 'gauge',
  substrates: 'any',
  depth: 'L1',
  paper: false,
  run() {
    const pairPlain = colorPermutations({
      d: 3,
      slots: ['plain', 'plain'],
    }).length
    const pairMixed = colorPermutations({
      d: 3,
      slots: ['plain', 'conjugate'],
    }).length
    const triple = colorPermutations({
      d: 3,
      slots: ['plain', 'plain', 'plain'],
    }).length
    const bruteForcePlain = bruteForceColorPermutations({
      d: 3,
      slots: ['plain', 'plain'],
    })
    const bruteForceMixed = bruteForceColorPermutations({
      d: 3,
      slots: ['plain', 'conjugate'],
    })
    ...
    // the rule's vacuum clock, the tone cycle t -> t + 1, as color, then as a separate factor
    const clock = permutationOperator({ map: [1, 2, 0] })
    const clockAsColor = symmetryDimension({
      operator: clock,
      d: 3,
      slots: ['plain'],
    })
    const colorIdentity = identityOperator({ size: 3 })
    const clockBesideColor = symmetryDimension({
      operator: kroneckerOperator(clock, colorIdentity),
      d: 3,
      slots: ['plain'],
      lift: (generator: Operator) =>
        kroneckerOperator(colorIdentity, generator),
    })

    const cartanPairs = colorPermutations({
      d: 3,
      slots: ['plain', 'plain'],
      diagonal: true,
    }).length
    ...
    const clockSplits = clockAsColor === 3 && clockBesideColor === 9
    const controlsWork =
    ...
    return verdict({
      status: ok ? 'pass' : 'fail',
      claim:
        "of the 9! = 362,880 reversible maps of a pair of three-state slots, exactly 2 (identity and swap) commute with SU(3), 1 does for a triplet and an antitriplet, and 6 of the 27! for three triplets, so a color-symmetric classical rule can only move color around and never mix it. The color-symmetric pair unitaries form a circle, every point of it keeps all 9 u(3) generators, and its only classical points are identity and swap: every other point entangles color, with entangling power up to 1/4. A tone that is also the rule's clock keeps 3 of the 9 generators, a separate color factor beside the clock keeps all 9",
      metrics: {
        ...
        clockAsColorSymmetry: clockAsColor,
        clockBesideColorSymmetry: clockBesideColor,
      },
      control: {
      ...
      notes:
        'L1, known mathematics measured exactly, with no random numbers (exhaustive search, a fixed phase grid of 360 points). It answers what the base would need for color. A deterministic permutation rule that respects SU(3) can only stream and exchange color, never rotate it, so gluon dynamics cannot be a symmetry of any tone permutation. The minimal addition is one phase on the exchange, which is an amplitude, the same missing ingredient E-FND-0080 found for the quantum sector. The clock measurement says the color cannot be the same three values the rule already uses as its period-three clock, it has to be a second factor beside it. The 1/4 maximum is for three-state slots and is reached where the antisymmetric part picks up a phase of pi / 2, the color analogue of the square root of swap.',
    })
  },
})
