...
import { lineWeave, pairCollision, passThrough, turningWeave } from '@/code/rule/collision'
import {
  ScheduleSymmetry,
  scheduleAnatomy,
  scheduleSymmetries,
...
function orderCounts(input: {
  ...
  for (const symmetry of input.symmetries) {
    const order = symmetryOrder({ symmetry, period: input.period, opposite: input.opposite })
...
function divisibleByThree(counts: Map<number, number>): number {
  return [...counts].filter(([order]) => order % 3 === 0).reduce((sum, [, n]) => sum + n, 0)
}
...
export default experiment({
  ...
  run() {
    ...
    const committed = scheduleSymmetries({
      anatomy: scheduleAnatomy({ schedule: turningWeave({ opposite }), period: PERIOD, degree: 24 }),
      permutations,
      opposite,
      relabellings,
    })
    const committedOrders = orderCounts({ symmetries: committed, period: PERIOD, opposite })

    const staticWeave = scheduleSymmetries({
      anatomy: scheduleAnatomy({ schedule: () => lineWeave({ opposite }), period: 1, degree: 24 }),
      permutations,
      opposite,
      relabellings,
    })
    const previousKnit = scheduleSymmetries({
      anatomy: scheduleAnatomy({ schedule: () => pairCollision({ opposite }), period: 1, degree: 24 }),
      permutations,
      opposite,
      relabellings,
    })
    const previousOrders = orderCounts({ symmetries: previousKnit, period: 1, opposite })
    const streaming = scheduleSymmetries({
      anatomy: scheduleAnatomy({ schedule: () => passThrough, period: 1, degree: 24 }),
      permutations,
      opposite,
    })
    const streamingOrders = orderCounts({ symmetries: streaming, period: 1, opposite })

    const groupIsComplete = permutations.length === 1152
    const committedTrivial = committed.length === 1 && (committedOrders.get(1) ?? 0) === 1
    const noOrderThree = divisibleByThree(committedOrders) === 0
    const detectorFindsOrderThree =
      streaming.length === 1152 && divisibleByThree(streamingOrders) > 0 && divisibleByThree(previousOrders) > 0
    const ok = groupIsComplete && committedTrivial && noOrderThree && detectorFindsOrderThree
    ...
    return verdict({
      ...
      metrics: {
        candidatesTested: permutations.length * relabellings.length * PERIOD,
        committedSymmetries: committed.length,
        committedOrderThreeOrMultiple: divisibleByThree(committedOrders),
        staticWeaveSymmetries: staticWeave.length,
      },
      control: {
        weylF4Order: permutations.length,
        streamingSymmetries: streaming.length,
        streamingOrderThreeOrMultiple: divisibleByThree(streamingOrders),
        previousKnitSymmetries: previousKnit.length,
        previousKnitOrderThreeOrMultiple: divisibleByThree(previousOrders),
      },
      notes:
