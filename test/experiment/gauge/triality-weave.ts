...
// for (E-FND-0117).
//
// The rule: each beat, three blocks of four lines (colour line f_i with orbit O_{(i + r_t) mod 3},
// r = 0, 1, 2, 2, 1, 0), and on each block X P X, with P the committed pair clock on every line and X
// a four-line vertex that swaps
//   f (s, 0) with the orbit empty  <->  f (-s, -s) with every orbit line (s, 0)   (triple creation)
//   f empty with one orbit line (s, 0)  <->  f (s, s) with that line (-s, 0)      (orbit exchange)
// and their (0, s) forms, s = +1 and -1. The triple creation is the classical three-at-once vertex the
// committed rule lacks (E-FRC-0094): one charge on the colour line turns into three identical charges,
// one on each line of the orbit, which a triality treats alike.
//
...
// - universality: a lone tone on any of the 24 directions disturbs all 12 lines within 24 beats, and
//   the line graph is connected
...
import { TRIALITY_WEAVE_PERIOD, trialityWeave, trialityWeaveLayout } from '@/code/rule/triality-weave'
...
export default experiment({
  ...
  code: 'E-FRC-0109',
  title:
    'a rule with one four-line vertex, a charge on a colour line turning into three identical charges on a triality orbit, keeps the colour-selecting triality, reverses exactly, conserves charge, is CPT exact and connects all twelve lines, which E-FRC-0107 showed no pairwise rule can do',
  category: 'gauge',
  substrates: ['3434'],
  depth: 'L2',
  paper: false,
  run() {
    ...
    const selectors = permutations.filter(
      p =>
        ...
        planes.has(keyOf(p.map((image, d) => (image === d ? d : -1)).filter(d => d >= 0))),
    )
    ...
    for (const start of cellMap === undefined ? [] : starts) {
      let a: Will = { mesh: box, data: Int8Array.from(start.data) }
      let b: Will = { mesh: box, data: transformState({ data: start.data, cellMap, permutation: sigma, degree: 24 }) }
      ...
      for (let t = 0; t < 12; t++) {
        ...
        commutes =
          commutes &&
          transformState({ data: a.data, cellMap, permutation: sigma, degree: 24 }).every((x, k) => x === b.data[k])
      }
    }
    ...
    for (let c = 0; c < TRIALITY_WEAVE_PERIOD && cptPhase < 0; c++) {
      ...
      for (let t = 0; t < TRIALITY_WEAVE_PERIOD && holds; t++) {
        const mirror = (((c - t) % TRIALITY_WEAVE_PERIOD) + TRIALITY_WEAVE_PERIOD) % TRIALITY_WEAVE_PERIOD
    ...
    const lineOf = (d: number): number => layout.lines.findIndex(([a, b]) => a === d || b === d)
    ...
    const lineGraph = (
      ...
      return { components: new Set(Array.from({ length: 12 }, (_, i) => find(i))).size, fewestReached: fewest }
    }
