...
function denseStep(input: { patterns: Int8Array[]; state: Int8Array; power: number }): Int8Array {
  ...
  for (let i = 0; i < n; i++) {
    let h = 0
    for (let m = 0; m < patterns.length; m++) h += (weights[m] ?? 0) * (patterns[m]![i] ?? 0)
...
function classicalRelax(input: { J: Int8Array[]; cue: Int8Array; beats: number }): Int8Array {
  const zero = new Float64Array(input.cue.length)
  let s = Int8Array.from(input.cue)
  for (let t = 0; t < input.beats; t++) s = hopfieldStep(input.J, s, zero, null)
  return s
}

// Dense relax, repeated higher-order beats.
function denseRelax(input: { patterns: Int8Array[]; cue: Int8Array; beats: number; power: number }): Int8Array {
  let s = Int8Array.from(input.cue)
  for (let t = 0; t < input.beats; t++) s = denseStep({ patterns: input.patterns, state: s, power: input.power })
  return s
}
...
function recallRate(input: { size: number; patternCount: number; fraction: number; relax: (cue: Int8Array, patterns: Int8Array[]) => Int8Array }): number {
  const { size, patternCount, fraction, relax } = input
  const patterns = storedPatterns(patternCount, size, makeRng({ seed: 1 }))
  let hits = 0
  for (let m = 0; m < patternCount; m++) {
    const rng = makeRng({ seed: 200 + m })
    const cue = Int8Array.from(patterns[m]!)
    for (let i = 0; i < size; i++) if (rng.next() < fraction) cue[i] = -(cue[i] ?? 0) as -1 | 1
    if (nearestPattern(relax(cue, patterns), patterns).index === m) hits++
  }
  return hits / patternCount
}

export function denseVsClassicalCapacity(input?: { size?: number; fraction?: number; power?: number }): {
  size: number
  ...
  const capacityOf = (relax: (cue: Int8Array, patterns: Int8Array[]) => Int8Array): number => {
    let best = 0
    for (let P = 2; P <= size; P++) {
      const rate = recallRate({ size, patternCount: P, fraction, relax })
      if (rate >= target) best = P
      else if (P > 4 && rate < target - 0.2) break
    }
    return best
  }
  const classicalCapacity = capacityOf((cue, patterns) => classicalRelax({ J: hebbianFills(patterns, size), cue, beats: 8 }))
  const denseCapacity = capacityOf((cue, patterns) => denseRelax({ patterns, cue, beats: 8, power }))
