...
export default experiment({
  ...
  title: 'a richer collision damps a shear but BALLISTICALLY (decay time scales as wavelength, not wavelength squared), so it is not a genuine viscosity',
  category: 'fluids',
  substrates: ['3434'],
  depth: 'L2',
  paper: true,
  run() {
    ...
    const decayTime = (collision: typeof viscous, wavelength: number): number => {
      ...
      for (let t = 1; t < series.length; t++) {
        if (series[t]! < 1 / Math.E) {
          const prev = series[t - 1]!, cur = series[t]!
    ...
    const logL: number[] = [], logTau: number[] = []
    const taus: number[] = []
    ...
    const validRule = conservesCharge(probe, viscous, 20) && conservesMomentum(probe, viscous, 20, directions) && isReversible(probe, viscous, 20)
    ...
    const ok = viscousDamps && committedInviscid && validRule && ballisticNotDiffusive
    ...
    return verdict({
      ...
      control: { committedShearDampsBelow1OverE: committedInviscid ? 0 : 1 },
      notes:
