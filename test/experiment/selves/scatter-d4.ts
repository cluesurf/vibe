...
export default defineExperiment({
  ...
  run() {
    ...
    const center = 4 + 4 * side + 4 * side * side + 4 * side * side * side
    ...
    const crossingStays = crossed.data[center * degree + dir] === 1 && crossed.data[center * degree + opp] === 1
    ...
    const reversible = isReversible({ mesh, data: approach.data.slice() }, mobile, beats)
    ...
    return verdict({
      ...
      control: { crossingStaysUnderPassThrough: crossingStays ? 1 : 0, interactedVsCrossingControl: interacted ? 1 : 0 },
      notes:
