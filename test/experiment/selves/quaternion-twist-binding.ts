...
export default defineExperiment({
  ...
  run() {
    ...
    const rotateBy = (v: Spin, axis: Spin, ang: number): Spin => { const u = unit(axis); const c = Math.cos(ang), s = Math.sin(ang), kxv = cross(u, v), kdv = dot(u, v); return [v[0] * c + kxv[0] * s + u[0] * kdv * (1 - c), v[1] * c + kxv[1] * s + u[1] * kdv * (1 - c), v[2] * c + kxv[2] * s + u[2] * kdv * (1 - c)] }
    ...
    const field = (s: Spin[], x: number, y: number): Spin => { const h: Spin = [0, 0, B]; for (const [dx, dy, ax] of bonds) { const r = rotateBy(s[idx(x + dx, y + dy)]!, ax, theta); h[0] += J * r[0]; h[1] += J * r[1]; h[2] += J * r[2] } return h }
    const pinEdge = (out: Spin[]): void => { for (let i = 0; i < L; i++) { out[idx(i, 0)] = [0, 0, 1]; out[idx(i, L - 1)] = [0, 0, 1]; out[idx(0, i)] = [0, 0, 1]; out[idx(L - 1, i)] = [0, 0, 1] } }
    const relaxStep = (s: Spin[], a: number): Spin[] => { const out: Spin[] = new Array(L * L); for (let y = 0; y < L; y++) for (let x = 0; x < L; x++) { const c = s[idx(x, y)]!; const h = field(s, x, y); out[idx(x, y)] = unit([c[0] + a * h[0], c[1] + a * h[1], c[2] + a * h[2]]) } pinEdge(out); return out }
    const precessStep = (s: Spin[], open: boolean): Spin[] => { const out: Spin[] = new Array(L * L); for (let y = 0; y < L; y++) for (let x = 0; x < L; x++) { const h = field(s, x, y); out[idx(x, y)] = unit(rotateBy(s[idx(x, y)]!, [h[0], h[1], h[2]], Math.hypot(h[0], h[1], h[2]) * dt)) } if (open) pinEdge(out); return out }
    ...
    let minQ = relaxedQ, maxQ = relaxedQ
    for (let t = 0; t < precessSteps; t++) { spins = precessStep(spins, false); const q = skyrmionDegree(spins, L); if (q < minQ) minQ = q; if (q > maxQ) maxQ = q }
    const precessedRadius = skyrmionRadius(spins, L)

    let pert = relaxed.map(v => [...v] as Spin)
    for (let y = 18; y < 21; y++) for (let x = 26; x < 29; x++) { const n = Math.hypot(1, 0, 0.2); pert[idx(x, y)] = [1 / n, 0, 0.2 / n] }
    for (let t = 0; t < precessSteps; t++) pert = precessStep(pert, false)
    const perturbedQ = skyrmionDegree(pert, L)

    const boundExists = Math.abs(relaxedQ + 1) < 0.1 && relaxedRadius > 1.5 && relaxedRadius < 8
    const stableUnderReversible = Math.abs(minQ + 1) < 0.1 && Math.abs(maxQ + 1) < 0.1 && Math.abs(precessedRadius - relaxedRadius) < 1.2
    ...
    return verdict({
      ...
      control: { relaxedDegreeTimes100: Math.round(relaxedQ * 100), precessMinQTimes100: Math.round(minQ * 100) },
      notes:
