...
export default experiment({
  ...
  run() {
    ...
    const accrete = (will: Will): void => {
      ...
      let total = 0, sx = 0, sy = 0, sz = 0, sw = 0
      for (let c = 0; c < coin.cellCount; c++) { const n = will.data[c * degree + rest]!; q[c] = n; if (n > 0) { const [x, y, z, w] = coord(c); total += n; sx += n * x; sy += n * y; sz += n * z; sw += n * w } }
      if (total === 0) return
      const cx = sx / total, cy = sy / total, cz = sz / total, cw = sw / total
      ...
      for (let c = 0; c < coin.cellCount; c++) {
        ...
        let bestNb = -1, bestDist = Infinity
        for (let d = 0; d < 24; d++) { const nb = base.neighbour(c, d); const [x, y, z, w] = coord(nb); const dd = (x - cx) ** 2 + (y - cy) ** 2 + (z - cz) ** 2 + (w - cw) ** 2; if (dd < bestDist) { bestDist = dd; bestNb = nb } }
        if (bestNb >= 0) moves.push([c, bestNb])
      }
      for (const [from, to] of moves) if (will.data[from * degree + rest]! > 0) { will.data[from * degree + rest] = 0; will.data[to * degree + rest] = will.data[to * degree + rest]! + 1 }
    }
    ...
    return verdict({
      ...
      control: { displacedStartExtent: displacedStart, displacedEndExtent: displacedEnd },
      notes:
