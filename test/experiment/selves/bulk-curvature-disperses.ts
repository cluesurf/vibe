...
function shellGrowthRatio(input: { mesh: Mesh; source: number; lo: number; hi: number }): number {
  const { mesh, source, lo, hi } = input
  const dist = shellDistances(mesh, source)
  const shell = new Map<number, number>()
  for (let c = 0; c < mesh.cellCount; c++) { const d = dist[c]!; if (d >= 0) shell.set(d, (shell.get(d) ?? 0) + 1) }
  let logSum = 0
  let count = 0
  for (let r = lo; r < hi; r++) {
    const a = shell.get(r) ?? 0
    const b = shell.get(r + 1) ?? 0
    if (a > 0 && b > 0) { logSum += Math.log(b / a); count++ }
  }
...
export default defineExperiment({
  ...
  run() {
    ...
    const treeRatio = shellGrowthRatio({ mesh: tree, source: 0, lo: 2, hi: 7 })
    ...
    const flatRatio = shellGrowthRatio({ mesh: flat, source: center, lo: 3, hi: 7 })
