...
export default experiment({
  ...
  title: 'a self of selves, the structured causal map keeps effective information across TWO recursive coarse-grainings, a random tower loses it',
  category: 'selves',
  ...
  run() {
    const fine = 36
    const traj = selfTrajectory({ L: 64, beats: 1200, bins: fine, seed: 24680 })
    ...
    const meso = coarseGrainTpm({ tpm: micro, groups: Array.from({ length: fine }, (_, i) => Math.floor(i / 3)) })
    const eiMeso = effectiveInformation(meso)
    const macro = coarseGrainTpm({ tpm: meso, groups: Array.from({ length: 12 }, (_, i) => Math.floor(i / 3)) })
    ...
    const shuffle = (n: number, m: number): number[] => {
      ...
      for (let i = n - 1; i > 0; i--) { const j = Math.floor(rng.next() * (i + 1)); const t = g[i]!; g[i] = g[j]!; g[j] = t }
      return g
    }
    const mesoR = coarseGrainTpm({ tpm: micro, groups: shuffle(fine, 12) })
    const eiMesoR = effectiveInformation(mesoR)
    const macroR = coarseGrainTpm({ tpm: mesoR, groups: shuffle(12, 4) })
    ...
    return verdict({
      ...
      metrics: { fine, eiMicro: Number(eiMicro.toFixed(3)), eiMeso: Number(eiMeso.toFixed(3)), eiMacro: Number(eiMacro.toFixed(3)), gapLevel1: Number(gapLevel1.toFixed(3)), gapLevel2: Number(gapLevel2.toFixed(3)) },
      ...
      control: { eiMesoRandom: Number(eiMesoR.toFixed(3)), eiMacroRandom: Number(eiMacroR.toFixed(3)) },
      notes: 'A6 / MS3 spatial, the self of selves at L3, two recursive coarse-grainings with the structured-vs-random control at each level.',
    })
  },
})
