...
export default defineExperiment({
  ...
  run() {
    ...
    for (let t = 0; t < relaxSteps; t++) spins = relaxSpins({ spins, params, rate: 0.08 })
    ...
    for (let t = 0; t < precessSteps; t++) { spins = precessSpins({ spins, params, dt, open: false }); const q = skyrmionDegree(spins, size); if (q < minQ) minQ = q; if (q > maxQ) maxQ = q }
    ...
    const at = (x: number, y: number): number => ((y + size) % size) * size + ((x + size) % size)
    for (let y = 15; y < 19; y++) for (let x = 27; x < 31; x++) { const n = Math.hypot(1, 0, 0.2); perturbed[at(x, y)] = [1 / n, 0, 0.2 / n] }
    let pert = perturbed
    for (let t = 0; t < precessSteps; t++) pert = precessSpins({ spins: pert, params, dt, open: false })
    ...
    const boundExists = Math.abs(relaxedQ + 1) < 0.1 && relaxedRadius > 2 && relaxedRadius < 8
    const stableUnderReversible = Math.abs(minQ + 1) < 0.1 && Math.abs(maxQ + 1) < 0.1 && Math.abs(precessedRadius - relaxedRadius) < 1
