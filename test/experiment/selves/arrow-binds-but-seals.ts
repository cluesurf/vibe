...
export default experiment({
  ...
  run() {
    ...
    for (let d = 0; d < degree; d++) { const o = opposite[d]!; if (d < o) lines.push([d, o]) }
    ...
    const body = (): Will => {
      ...
      for (let c = 0; c < mesh.cellCount; c++) {
        ...
        if ((x - half) ** 2 + (y - half) ** 2 + (z - half) ** 2 + (w - half) ** 2 <= 4) { const b = c * degree; for (let i = 0; i < 6; i++) { const [a, zz] = lines[i]!; will.data[b + a] = 1; will.data[b + zz] = 1 } }
      }
      return will
    }
    // a hit to the body's OWN charges at the centre (flip them), a real net-charge disturbance to the body itself.
    const hit = (w0: Will): Will => { const w = cloneWill(w0); for (let i = 0; i < 6; i++) { const [a, zz] = lines[i]!; w.data[center * degree + a] = -1; w.data[center * degree + zz] = -1 } return w }
    ...
    const netRms = (will: Will): number => {
      let total = 0, sx = 0, sy = 0, sz = 0, sw = 0; const q: number[] = []
      for (let c = 0; c < mesh.cellCount; c++) { let n = 0; const b = c * degree; for (let d = 0; d < degree; d++) n += will.data[b + d]!; q[c] = n; if (n > 0) { const [x, y, z, w] = coord(c); total += n; sx += n * x; sy += n * y; sz += n * z; sw += n * w } }
      if (total === 0) return 0
      const mx = sx / total, my = sy / total, mz = sz / total, mw = sw / total
      let v = 0
      for (let c = 0; c < mesh.cellCount; c++) { const n = q[c]!; if (n > 0) { const [x, y, z, w] = coord(c); v += n * ((x - mx) ** 2 + (y - my) ** 2 + (z - mz) ** 2 + (w - mw) ** 2) } }
    ...
    const arrowSeals = arrow.openFinal >= arrow.closedFinal * 0.5 && arrow.openFinal > 0
    // the momentum rule RADIATES (open difference healed to near zero) but does NOT BIND (rms disperses large).
    const momentumRadiates = momentum.openFinal <= momentum.closedFinal * 0.1
    const momentumNoBind = momentum.rmsMax >= 8

    const ok = arrowBinds && arrowSeals && momentumRadiates && momentumNoBind
    return verdict({
      status: ok ? 'pass' : 'fail',
      claim:
      ...
      control: { arrowOpenFinal: arrow.openFinal, momentumOpenFinal: momentum.openFinal },
      notes:
