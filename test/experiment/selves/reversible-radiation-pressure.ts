...
export default defineExperiment({
  ...
  run() {
    ...
    for (let d = 0; d < degree; d++) { const o = opposite[d]!; if (d < o) (d < 24 ? longLines : shortLines).push([d, o]) }
    // pair each short line with a long line (the slow-fast coupling), the rest long-long.
    const pairs: Array<[[number, number], [number, number]]> = []
    for (let i = 0; i < shortLines.length; i++) pairs.push([shortLines[i]!, longLines[i]!])
    for (let i = shortLines.length; i + 1 < longLines.length; i += 2) pairs.push([longLines[i]!, longLines[i + 1]!])
    ...
    const coupledRotate: Collision = (slots, base) => {
      for (const [A, B] of pairs) {
        const a0 = slots[base + A[0]] ?? 0, a1 = slots[base + A[1]] ?? 0
        const b0 = slots[base + B[0]] ?? 0, b1 = slots[base + B[1]] ?? 0
        const aHeadOn = a0 === a1 && a0 !== 0
        const bHeadOn = b0 === b1 && b0 !== 0
        const aEmpty = a0 === 0 && a1 === 0
        const bEmpty = b0 === 0 && b1 === 0
        if (aHeadOn && bEmpty) { slots[base + B[0]] = a0 as Will['data'][number]; slots[base + B[1]] = a0 as Will['data'][number]; slots[base + A[0]] = 0; slots[base + A[1]] = 0 }
        else if (bHeadOn && aEmpty) { slots[base + A[0]] = b0 as Will['data'][number]; slots[base + A[1]] = b0 as Will['data'][number]; slots[base + B[0]] = 0; slots[base + B[1]] = 0 }
      }
    }
    ...
    const setup = (): Will => {
      ...
      for (let c = 0; c < mesh.cellCount; c++) {
        const x = c % side, y = Math.floor(c / side) % side, z = Math.floor(c / (side * side)) % side, w = Math.floor(c / (side * side * side)) % side
        const r2 = (x - half) ** 2 + (y - half) ** 2 + (z - half) ** 2 + (w - half) ** 2
        if (r2 <= 2) { for (const [s0, s1] of shortLines) { will.data[c * degree + s0] = 1; will.data[c * degree + s1] = 1 } }
        else {
          const density = x < half ? 9 : 2 // fast bath dense at low x, sparse (depleted) at high x
          for (let li = 0; li < longLines.length; li++) if (((c * 31 + li * 17) % 12) < density) { const [l0, l1] = longLines[li]!; will.data[c * degree + l0] = 1; will.data[c * degree + l1] = 1 }
        }
      }
      return will
    }
    const slowCentroidX = (will: Will): { cx: number; rms: number } => {
      let total = 0, sx = 0
      for (let c = 0; c < mesh.cellCount; c++) { let m = 0; for (let d = 24; d < 32; d++) m += Math.abs(will.data[c * degree + d]!); if (m > 0) { total += m; sx += m * (c % side) } }
      if (total === 0) return { cx: half, rms: 0 }
      const cx = sx / total
      let v = 0
      for (let c = 0; c < mesh.cellCount; c++) { let m = 0; for (let d = 24; d < 32; d++) m += Math.abs(will.data[c * degree + d]!); if (m > 0) v += m * ((c % side) - cx) ** 2 }
    ...
    return verdict({
      ...
      control: { startRmsTimes100: Math.round(start.rms * 100), maxRmsTimes100: Math.round(maxRms * 100) },
      notes:
