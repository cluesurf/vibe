  ...
  run() {
    ...
    const diffuse = (): Will => {
      ...
      for (let c = 0; c < mesh.cellCount; c++) {
        ...
        if ((x - half) ** 2 + (y - half) ** 2 + (z - half) ** 2 + (w - half) ** 2 <= 36) {
          const b = c * degree
          for (let i = 0; i < 2; i++) { const [a, zz] = lines[i]!; will.data[b + a] = 1; will.data[b + zz] = 1 }
        }
      }
      return will
    }
    ...
    const noneContract = momentum.min >= rms0 * 0.9 && arrow.min >= rms0 * 0.9 && bindMove.min >= rms0 * 0.9
    ...
    return verdict({
      ...
      control: { rms0Times100: Math.round(rms0 * 100), momentumFinalTimes100: Math.round(momentum.final * 100) },
      notes:
