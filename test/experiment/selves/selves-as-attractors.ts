  ...
  for (let p = 0; p < k; p++) {
    ...
    const settled = settle(
      ...
      perturb(
        self,
        fraction,
        seed + 100 * p + Math.round(fraction * 50),
      ),
      40,
    )
...
export function selvesAsAttractors(input: { seed: number }): {
  ...
  const byFraction = fractions.map(f => ({
    fraction: f,
  ...
  return {
    ...
    solved:
      basinRadius >= 0.2 &&
      identityOverlap > 0.98 &&
      capacityGrows &&
      (capacityByN[0]?.capacity ?? 0) >= 4,
  }
}
...
export default experiment({
  ...
  run() {
    ...
    return verdict({
      ...
      metrics: {
        basinRadius: r.basinRadius,
        identityOverlap: r.identityOverlap,
      },
    })
  },
})
