...
export function Viewer({ title, mode, symbols, hint }: ViewerProps): React.ReactElement {
  const [symbol, setSymbol] = useState<number[]>(symbols[0]!.value)
  const is3D = mode !== '2d'

  return (
    <div className="viewer">
      <VibeView symbol={symbol} mode={mode} className="viewer-canvas" />

      <div className="panel">
        <Link to="/" className="back">← vibe</Link>
        <h1>{title}</h1>
        ...
        <label className="field">
          ...
          <select
            value={symbol.join('-')}
            onChange={(e) => setSymbol(symbols.find((s) => s.value.join('-') === e.target.value)!.value)}
          >
            {symbols.map((s) => (
        ...
        <div className="legend">
          ...
          <dl>
            <dt>W / ↑</dt><dd>forward</dd>
            <dt>S / ↓</dt><dd>back</dd>
            <dt>A / ←</dt><dd>turn left</dd>
            <dt>D / →</dt><dd>turn right</dd>
            {is3D ? <><dt>Q / E</dt><dd>strafe</dd><dt>Space / Shift</dt><dd>rise / sink</dd></> : null}
            <dt>drag</dt><dd>{is3D ? 'look around' : 'turn view'}</dd>
            <dt>wheel</dt><dd>{is3D ? 'dolly forward' : 'zoom'}</dd>
          </dl>
        </div>
      </div>
    </div>
