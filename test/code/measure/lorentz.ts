...
suite('measure/lorentz: lorentzIsotropy', [
  check(
    'a square lattice has a preferred frame with 4-fold anisotropy 1',
    () => {
      ...
      ok(
        out.preferredFrame,
        'a lattice must register a preferred frame',
      )
    },
  ),
  check(
    'a substrate with no embedding reports the isotropic null',
    () => {
      const g = makeGraph({
        size: 4,
        directed: false,
...
suite('measure/lorentz: latticeAnisotropy (group-speed)', [
  check(
    ...
    () => {
      ...
      ok(
        out.anisotropy < 1e-2,
        `long-wavelength anisotropy ${out.anisotropy} must be tiny`,
      )
    },
  ),
