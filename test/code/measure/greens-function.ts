...
function cubic(L: number): { neighbors: number[][]; coords: number[][]; center: number } {
...
suite('measure/greens-function: Dirichlet Jacobi solve', [
  ...
  check('the converged field satisfies (D - A) phi = delta', () => {
    ...
    const center = 2
    const phi = dirichletGreensFunction({ neighbors, center, degree: 2, iterations: 6000 })
...
suite('measure/greens-function: falloff exponent', [
  ...
  check('a 3D cubic lattice gives a positive exponent near 1', () => {
    ...
    ok(e > 0.5 && e < 1.6, `3D screened Coulomb should be near 1, got ${e}`)
  }),
])
...
suite('measure/greens-function: decay class', [
  ...
  check('the decay class is well-formed on a flat lattice', () => {
    ...
    ok(r.powSlope < 0, `potential should decay, got slope ${r.powSlope}`)
    ok(Number.isFinite(r.expR2) && Number.isFinite(r.powR2), 'both fits must produce finite R^2')
    ok(r.exponential === (r.expR2 > r.powR2), 'the verdict must match the better R^2')
  }),
])
