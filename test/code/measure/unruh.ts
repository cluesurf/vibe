...
suite('measure/unruh: temperature from detailed balance', [
  ...
  check('a synthetic detailed-balance response recovers its temperature', () => {
    const T0 = 0.37
  ...
  check('the recovered temperature does not depend on kappa', () => {
    ...
    close(temperatureFromDetailedBalance({ kappa: 0.5, response: r }), T0, TOL)
    close(temperatureFromDetailedBalance({ kappa: 3, response: r }), T0, TOL)
  }),
