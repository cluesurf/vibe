...
suite('measure/shell-growth-ratio', [
  ...
  check('the mean is rounded to two places', () => {
    equal(
      shellGrowthRatio({ shellCounts: [3, 4, 5], from: 0, to: 3 }),
      1.29,
    )
  }),
  // A window with a single entry and no safeDenominator returns 0.
  check(
    'a single-entry window returns 0 without safeDenominator',
    () => {
      equal(
        shellGrowthRatio({ shellCounts: [5, 10, 20], from: 1, to: 2 }),
        0,
      )
    },
  ),
  ...
  check(
    'safeDenominator keeps a single-entry window defined at 0',
    () => {
      equal(
        shellGrowthRatio({
          shellCounts: [5, 10, 20],
  ...
  check('an over-long window is clamped to the series', () => {
    equal(
      shellGrowthRatio({ shellCounts: [1, 2, 4], from: 0, to: 99 }),
      2,
    )
  }),
])
