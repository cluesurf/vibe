...
suite('measure/shell-growth: Euclidean L1 shells', [
  ...
  check('the L1 ratio is n/(n-1) and tends to 1', () => {
    close(
      euclideanL1ShellRatio({ dimension: 2, shell: 5 }),
      5 / 4,
      1e-9,
    )
    close(
...
suite('measure/shell-growth: growth-rate algebra', [
  ...
  check('the shell-separation exponent inverts lambda^p', () => {
    close(
      shellSeparationExponent({ ratio: 8, growthRate: 2 }),
      3,
      1e-12,
    )
    close(
  ...
  check(
    'Aitken on a clean geometric sequence returns the ratio',
    () => {
  ...
  check(
    'Aitken accelerates a converging ratio sequence to its limit',
    () => {
      // counts chosen so the interior ratios (indices 2..len-2) are 1.5, 1.25, 1.125
      close(
        extrapolatedGrowthRate([1, 8, 12, 15, 16.875, 999]),
        1,
        1e-9,
      )
    },
