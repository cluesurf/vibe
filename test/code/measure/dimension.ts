...
suite('measure/dimension: Myrheim-Meyer inversion fixed points', [
  ...
  check('ordering fraction 1/2 inverts to dimension 2 (f(2) = 1/2)', () => {
    close(dimensionFromOrderingFraction(0.5), 2, 1e-3)
  }),
  check('the inversion is monotone: smaller fraction -> larger dimension', () => {
...
suite('measure/dimension: Myrheim-Meyer on causal sets', [
  check('a total chain (ordering fraction 1) reads dimension ~ 1', () => {
    const pairs: [number, number][] = []
    for (let a = 0; a < 6; a++) {
      for (let b = a + 1; b < 6; b++) pairs.push([a, b])
    }
    close(myrheimMeyerDimension({ poset: posetFromPairs(6, pairs) }), 1, 1e-2)
  }),
  check('a star order with ordering fraction 1/2 reads dimension ~ 2', () => {
...
suite('measure/dimension: box-counting (Minkowski) dimension', [
  check('a fully filled 32x32 sheet has box-counting dimension exactly 2', () => {
    const cells = Array.from({ length: 32 * 32 }, (_, i) => i)
    close(boxCountingDimension({ cells, sideLength: 32 }), 2, 1e-9)
  }),
  check('a single filled row (a line) has box-counting dimension exactly 1', () => {
...
suite('measure/dimension: ball-growth dimension', [
  check('a path graph reads ball-growth dimension ~ 1', () => {
    ...
    const d = ballGrowthDimension({ neighbors, centers: [30], maxRadius: 25 })
    close(d, 1, 0.2)
  }),
