...
import { suite, check, close, equal, exactArray, ok } from '@/test/code/harness'
...
suite('measure/probe-directions: non-degeneracy (plastic-number fix)', [
  check('REGRESSION: coord 3 is never the exact negative of coord 2 (dim 3, 4)', () => {
    for (const dimension of [3, 4]) {
  ...
  check('the directions span all axes (no axis collapses to a constant)', () => {
    ...
    for (let axis = 0; axis < 4; axis++) {
      ...
      ok(variance > 0.01, `axis ${axis} must carry real spread, got var ${variance}`)
    }
  }),
  check('the mean direction is ~ 0 (a well-spread set)', () => {
    ...
    ok(Math.hypot(...mean) < 0.1, `mean direction ${mean} should be near origin`)
  }),
])
