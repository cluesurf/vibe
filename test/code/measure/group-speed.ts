...
import { groupSpeed, groupVelocity1d, groupSpeedAnisotropy } from '@/code/measure/group-speed'
...
suite('measure/group-speed: groupVelocity1d', [
  ...
  check('omega = sin k has v = cos k', () => {
    close(groupVelocity1d({ omega: Math.sin, k: 0.7 }), Math.cos(0.7), 1e-6)
  }),
...
suite('measure/group-speed: groupSpeed (2D)', [
  check('omega = |k| has group speed 1 everywhere off the origin', () => {
...
suite('measure/group-speed: groupSpeedAnisotropy', [
  check('the |k| cone is isotropic: anisotropy ~ 0, mean speed ~ 1', () => {
    const out = groupSpeedAnisotropy({ omega: (kx, ky) => Math.hypot(kx, ky), kMag: 1 })
