...
import { algebraicConnectivity, toneIntegration } from '@/code/measure/integration'
...
suite('measure/integration: algebraicConnectivity (Fiedler value)', [
  check('K4 has algebraic connectivity 4 (Laplacian spectrum {0, 4, 4, 4})', () => {
  ...
  check('path P3 has algebraic connectivity 1 (spectrum {0, 1, 3})', () => {
  ...
  check('a disconnected 2-node region has algebraic connectivity 0', () => {
    const adj = adjacency([[], []])
    const value = algebraicConnectivity({
      adjacency: adj,
      region: new Set([0, 1]),
...
suite('measure/integration: toneIntegration (Phi)', [
  check('REGRESSION (odd n): a disconnected 5-node region has Phi = 0', () => {
  ...
  check('a complete K4 region is integrated (Phi > 0)', () => {
    ...
    ok(phi > 0, `a fully coupled region must have positive Phi, got ${phi}`)
  }),
])
