...
function cubicNeighbors(L: number): number[][] {
  const at = (x: number, y: number, z: number): number => x + L * y + L * L * z
...
suite('measure/gravity-exponent: 3D Coulomb falloff', [
  check('alpha is finite, positive, and near 1 (Newtonian, z=1)', () => {
    const L = 21
    const c = (L - 1) / 2
    ...
    ok(alpha > 0, `a decaying potential must give a positive exponent, got ${alpha}`)
