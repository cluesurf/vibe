...
suite('measure/bell: chsh (local hidden variable)', [
  check('constant outcomes (+1,+1) give every correlator 1, so S = 2 exactly', () => {
    const out = chsh({
  ...
  check('the local model with independent settings obeys |S| <= 2 (Bell bound)', () => {
    const out = chsh({
      drawHidden: ({ rng }) => rng.next() * 2 * Math.PI,
      settingCorrelation: 0,
      angles: OPTIMAL,
    ...
    ok(Math.abs(out.s) <= 2.2, `local |S| = ${out.s} must stay at/under the classical bound`)
    ok(Math.abs(out.s) < 2.5, `local |S| = ${out.s} must not reach the Tsirelson value`)
  }),
])

suite('measure/bell: chshShared (superdeterminism loophole)', [
  check('aligned settings with eta = 1 reach S ~ 4 (algebraic maximum)', () => {
    // Region analysis over lambda in [0,pi): the four equal-measure intervals give
    // E(0,0)=+1, E(0,1)=-1, E(1,0)=+1, E(1,1)=+1, so S = 1-(-1)+1+1 = 4.
    const s = chshShared({ eta: 1, mode: 'aligned', trials: 40000, seed: 7 })
    close(s, 4, 0.1)
  }),
  check('random (decorrelated) settings with eta = 1 stay within |S| <= 2', () => {
    const s = chshShared({ eta: 1, mode: 'random', trials: 40000, seed: 7 })
    ok(Math.abs(s) <= 2.05, `decorrelated |S| = ${s} must respect the classical bound`)
  }),
  check('aligned settings with eta = 0 (purely local) stay within |S| <= 2', () => {
    const s = chshShared({ eta: 0, mode: 'aligned', trials: 40000, seed: 7 })
    ok(Math.abs(s) <= 2.05, `local |S| = ${s} must respect the classical bound`)
  }),
])
