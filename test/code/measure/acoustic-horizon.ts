...
suite('measure/acoustic-horizon: speed profile', [
  ...
  check('the profile vanishes inside and is tanh outside', () => {
    close(tanhHorizonSpeed({ radius: 0.5, horizon: 1, gradient: 0.5 }), 0, 1e-12)
    close(tanhHorizonSpeed({ radius: 1, horizon: 1, gradient: 0.5 }), 0, 1e-12)
    close(tanhHorizonSpeed({ radius: 3, horizon: 1, gradient: 0.5 }), Math.tanh(1), 1e-12)
  }),
  // The slope at the horizon equals the gradient (surface gravity = gradient).
  check('the slope at the horizon is the gradient', () => {
    const eps = 1e-5
    const slope = tanhHorizonSpeed({ radius: 1 + eps, horizon: 1, gradient: 0.5 }) / eps
...
suite('measure/acoustic-horizon: ray freeze surface gravity', [
  ...
  check('the freeze rate recovers the surface gravity', () => {
    const kappa = rayFreezeSurfaceGravity({ horizon: 1, gradient: 0.5, start: 2 })
    close(kappa, 0.5, 0.05)
  }),
  check('a steeper gradient gives a larger surface gravity', () => {
    const kappa = rayFreezeSurfaceGravity({ horizon: 1, gradient: 0.8, start: 2 })
