...
suite('measure/regression: powerLawExponent', [
  ...
  check('diffusive spread ~ t^2 has exponent 2 (and drops the t=1 zero point)', () => {
  ...
  check('guard: fewer than two usable (positive) points returns 0', () => {
    equal(powerLawExponent({ times: [1, 2], spreads: [0, 5] }), 0)
    equal(powerLawExponent({ times: [1, 2, 3], spreads: [0, 0, 0] }), 0)
  }),
])

suite('measure/regression: linearFit', [
  check('y = 3x + 1 recovers slope 3, intercept 1, r2 = 1, residual 0', () => {
...
suite('measure/regression: fitForm', [
  check('y = 2*f(x) + 3 with f = identity recovers a = 2, r2 = 1', () => {
    const x = [1, 2, 3, 4, 5]
...
suite('measure/regression: loglogExponentWindow', [
  ...
  check('a non-positive sample inside the window is dropped, slope stays 2', () => {
  ...
  check('guard: an all-zero window returns 0 not NaN', () => {
    equal(loglogExponentWindow({ values: [0, 0, 0, 0], lo: 1, hi: 3 }), 0)
  }),
])

suite('measure/regression: powerLawFit', [
  check('a clean y = x^2 gives exponent 2 and ~zero max deviation', () => {
    const xs = [1, 2, 3, 4, 5, 6]
    const ys = xs.map(x => x * x)
...
suite('measure/regression: localForceLawExponent', [
  check('an inverse-square force (potential 1/r) reads exponent -2', () => {
    // force = -dG/dr of G = 1/r is 1/r^2, whose local log-log slope is -2.
    close(localForceLawExponent({ potential: r => 1 / r, r: 1 }), -2, 1e-3)
  }),
  check('a 4D short-range force (potential 1/r^2) reads exponent -3', () => {
    close(localForceLawExponent({ potential: r => 1 / (r * r), r: 1 }), -3, 1e-3)
  }),
])
