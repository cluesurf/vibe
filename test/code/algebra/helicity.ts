...
suite('algebra/helicity: the polarization tensors', [
  check('plus and cross are traceless, unit-Frobenius^2 = 2, orthogonal', () => {
    const plusTrace =
    ...
    equal(tensorInner(PLUS_POLARIZATION, PLUS_POLARIZATION), 2, '|plus|^2 = 2')
    equal(tensorInner(CROSS_POLARIZATION, CROSS_POLARIZATION), 2, '|cross|^2 = 2')
    equal(
  ...
  check('conjugating by the identity rotation is a no-op', () => {
    ...
    for (let i = 0; i < 3; i++) {
      for (let j = 0; j < 3; j++) {
        close(out[i]![j]!, PLUS_POLARIZATION[i]![j]!, TOL, 'R(0) e R(0)^T = e')
      }
    }
  }),
])
...
suite('algebra/helicity: spin-2 overlaps', [
  check('plusSelfOverlap(theta) = cos(2 theta), period 180 deg', () => {
    ...
    close(plusSelfOverlap(Math.PI), 1, 1e-12, '180 deg -> back to +1 (period)')
  }),
  check('plusToCrossOverlap(theta) = sin(2 theta), = 1 at 45 deg', () => {
    for (const t of [0, 0.4, Math.PI / 6, 1.2]) {
      close(plusToCrossOverlap(t), Math.sin(2 * t), 1e-12, 'cross overlap')
    }
    close(plusToCrossOverlap(0), 0, 1e-12, 'theta=0 -> 0')
    close(plusToCrossOverlap(Math.PI / 4), 1, 1e-12, 'plus rotates into cross at 45')
  }),
