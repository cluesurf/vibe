...
suite('algebra/linear/kernel-polynomial: |x| coefficients', [
  check('c0 = 2/pi and the even coefficients match the closed form', () => {
...
suite('algebra/linear/kernel-polynomial: Chebyshev moment recurrence', [
  check('on H=[y] the moments are T_n(y) = cos(n arccos y)', () => {
    const y = 0.5
    const operator: HermitianOperator = (input: Cx, output: Cx): void => {
      output.re[0] = y * input.re[0]!
      output.im[0] = y * input.im[0]!
    }
    const probe: Cx = { re: Float64Array.from([1]), im: Float64Array.from([0]) }
    const count = 6
    const mu = chebyshevMoments({ operator, scale: 1, probe, count, dim: 1 })
    for (let n = 0; n < count; n++) {
      const tn = Math.cos(n * Math.acos(y))
      close(mu[n]!, tn, 1e-12, `mu_${n} = T_${n}(0.5)`)
    }
  }),
  check('the scale factor folds H into [-1,1]: moments of H/scale = T_n(h/scale)', () => {
    const h = 2
    ...
    const probe: Cx = { re: Float64Array.from([1]), im: Float64Array.from([0]) }
    const mu = chebyshevMoments({ operator, scale, probe, count: 5, dim: 1 })
    for (let n = 0; n < 5; n++) {
...
suite('algebra/linear/kernel-polynomial: spectral bound', [
  check('spectralBound of diag(-3,1,2) is the largest lambda^2 = 9', () => {
    const d = [-3, 1, 2]
    const operator: HermitianOperator = (input: Cx, output: Cx): void => {
