...
suite('algebra/linear/power-iteration: lowest eigenpairs', [
  check('lowest pair of [[2,1],[1,2]]: energy 1, eigenvector verified', () => {
    ...
    for (let i = 0; i < hs.length; i++) {
      close(hs[i]!, pair!.energy * pair!.state[i]!, 1e-6, `H s component ${i}`)
    }
  }),
  check('deflation recovers {1, 3} from the 3x3 block, states orthogonal', () => {
    const op = operatorFrom([
      [2, 1, 0],
      [1, 2, 0],
      [0, 0, 5],
