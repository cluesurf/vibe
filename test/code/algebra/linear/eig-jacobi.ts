...
import { makeDense, denseSet, DenseMatrix } from '@/code/algebra/linear/dense'
...
suite('algebra/linear/eig-jacobi: jacobiEigenvalues3 (3x3, eigenvalues only)', [
  ...
  check('J + 2I has a doubly-degenerate eigenvalue: {3, 3, 6}', () => {
...
suite('algebra/linear/eig-jacobi: jacobiEigenvalues (n x n, ascending)', [
...
suite('algebra/linear/eig-jacobi: eigSymmetric (values + vectors)', [
  check('[[2,1],[1,2]] eigenpairs: values {1,3}, vectors orthonormal', () => {
    const eig = eigSymmetric({ matrix: denseFrom([
      [2, 1],
      [1, 2],
    ]) })
  ...
  check('A v_j = lambda_j v_j for every eigenpair', () => {
    ...
    for (let j = 0; j < n; j++) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        close(av, lambda * eig.vectors[i * n + j]!, 1e-8, `row ${i} of A v${j}`)
      }
    }
  }),
  check('reconstruction: sum_j lambda_j v_j v_j^T = A', () => {
    ...
    for (let i = 0; i < n; i++) {
      for (let k = 0; k < n; k++) {
        let acc = 0
        for (let j = 0; j < n; j++) {
          acc += eig.values[j]! * eig.vectors[i * n + j]! * eig.vectors[k * n + j]!
        }
