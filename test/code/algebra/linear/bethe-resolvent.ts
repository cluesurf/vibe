...
suite('algebra/linear/bethe-resolvent: cavity decay closed forms', [
  check('massless point E = z gives mu = 1/b exactly', () => {
    // z = 3, b = 2 -> mu = 1/2
    close(betheCavityDecay({ coordination: 3, energy: 3 }), 1 / 2, 1e-12, 'z=3')
    // z = 4, b = 3 -> mu = 1/3
    close(betheCavityDecay({ coordination: 4, energy: 4 }), 1 / 3, 1e-12, 'z=4')
    // z = 5, b = 4 -> mu = 1/4
    close(betheCavityDecay({ coordination: 5, energy: 5 }), 1 / 4, 1e-12, 'z=5')
  }),
  check('the decay solves b mu^2 - E mu + 1 = 0', () => {
    ...
    close(b * mu * mu - energy * mu + 1, 0, 1e-12, 'satisfies the cavity quadratic')
  }),
  check('band edge E = 2 sqrt(b) gives mu = 1/sqrt(b)', () => {
    const b = 2
    const z = b + 1
    const mu = betheCavityDecay({ coordination: z, energy: 2 * Math.sqrt(b) })
...
suite('algebra/linear/bethe-resolvent: boundary exponent', [
  check('massless point gives the universal alpha = 2', () => {
    close(betheBoundaryExponent({ coordination: 3, energy: 3 }), 2, 1e-12, 'z=3')
    close(betheBoundaryExponent({ coordination: 4, energy: 4 }), 2, 1e-12, 'z=4')
    close(betheBoundaryExponent({ coordination: 5, energy: 5 }), 2, 1e-12, 'z=5')
  }),
  check('alpha = 2 ln(1/mu) / ln(b) off the massless point', () => {
    ...
    close(betheBoundaryExponent({ coordination: z, energy }), expected, 1e-12, 'formula')
  }),
])

suite('algebra/linear/bethe-resolvent: finite-tree validation', [
  check('finite-tree ratio matches the exact finite-size decay (z=3)', () => {
    const z = 3
    const depth = 10
    const b = z - 1
    // exact: phi_k = A b^-k + B with B = -A b^-(d+1); ratio phi_3/phi_2.
    const tail = Math.pow(b, -(depth + 1))
    const expected = (Math.pow(b, -3) - tail) / (Math.pow(b, -2) - tail)
    const measured = finiteTreeResolventRatio({ coordination: z, depth })
    close(measured, expected, 2e-3, 'finite-tree decay ratio')
  }),
  check('the finite-tree ratio approaches the cavity mu = 1/b as depth grows', () => {
    const z = 3
    const mu = betheCavityDecay({ coordination: z, energy: z }) // 1/2
    const measured = finiteTreeResolventRatio({ coordination: z, depth: 12 })
