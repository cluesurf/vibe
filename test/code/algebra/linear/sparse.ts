...
suite('algebra/linear/sparse: CSR matvec', [
  check('[[2,1],[1,2]] x = hand product', () => {
    ...
    closeArray(sparseMatVec(m, { x: Float64Array.from([1, 1]) }), [3, 3], 1e-12, 'x=(1,1)')
    closeArray(sparseMatVec(m, { x: Float64Array.from([1, -1]) }), [1, -1], 1e-12, 'x=(1,-1)')
  }),
  check('duplicate (row,col) triplets are summed', () => {
    ...
    const m = sparseFromTriplets({ rows: 2, cols: 2, triplets })
    closeArray(sparseMatVec(m, { x: Float64Array.from([1, 1]) }), [2, 3], 1e-12, 'summed diagonal')
  }),
  check('operatorFromSparse exposes the size and the same action', () => {
...
suite('algebra/linear/sparse: Aubry-Andre potential', [
  ...
  check('nonzero strength adds strength*cos(2 pi golden i) to the diagonal', () => {
