...
suite('algebra/jordan: the Jordan product and the rank-3 frame', [
  check('the test elements are Hermitian and the Jordan product preserves Hermiticity', () => {
  ...
  check('the diagonal frame: three primitive idempotents, each of trace 1', () => {
  ...
  check('the frame idempotents are pairwise orthogonal and sum to the identity', () => {
    ...
    ok(octonionMatrixEquals(sum, octonionMatrixIdentity(3)), 'E0 + E1 + E2 = I')
  }),
])

suite('algebra/jordan: the Jordan identity holds at n<=3, fails at n>=4', [
  check('a single residual at n=3 is ~0 (the Jordan identity holds)', () => {
    const a = deterministicHermitian(3, 0)
    const b = deterministicHermitian(3, 1)
    close(jordanIdentityResidual(a, b), 0, 1e-9, 'one H3(O) residual ~ 0')
  }),
  check('the worst residual at n=2 and n=3 is ~0', () => {
    close(maxJordanIdentityResidual(2), 0, 1e-9, 'H2(O) is a Jordan algebra')
    close(maxJordanIdentityResidual(3), 0, 1e-9, 'H3(O) is a Jordan algebra')
  }),
  check('the residual at n=4 is large (H4(O) is NOT a Jordan algebra)', () => {
...
suite('algebra/jordan: S_3 slot permutations are automorphisms', [
  check('there are exactly 6 permutations of 3 slots (|S_3| = 6)', () => {
    ...
    for (const p of perms) {
      equal([...p].sort().join(','), '0,1,2', 'is a permutation of {0,1,2}')
    }
  }),
  check('every slot permutation is a Jordan automorphism of H3(O)', () => {
