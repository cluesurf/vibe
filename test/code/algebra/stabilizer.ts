...
suite('algebra/stabilizer: symplectic primitives', [
  ...
  check(
    'X and Z on the same qubit anticommute; on different qubits commute',
    () => {
      notOk(
  ...
  check('a Y (x and z set on one qubit) has weight 1', () => {
    ...
    equal(
      pauliWeight({ x: 0b101, z: 0b010 }),
      3,
      'X0 Z1 X2 has weight 3',
    )
  }),
])
...
suite('algebra/stabilizer: the [[5,1,3]] perfect code', [
  check(
    'the four generators pairwise commute (a valid stabilizer)',
    () => {
...
suite('algebra/stabilizer: erasure correctability from distance 3', [
  ...
  check(
    'some 3-qubit erasure is NOT correctable (distance exactly 3)',
    () => {
      const span = stabilizerGroup({
        generators: GENERATORS,
        qubits: QUBITS,
      ...
      ok(
        foundUncorrectable,
        'a weight-3 logical fits inside some 3-qubit set',
      )
    },
  ),
