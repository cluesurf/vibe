...
import { suite, check, equal, close, closeArray, exactArray } from '@/test/code/harness'
import {
  dot,
  norm,
  add,
...
suite('algebra/vector: euclidean primitives', [
  ...
  check('sub defaults to plain difference and scales the second arg', () => {
  ...
  check('normalize gives an exact unit direction', () => {
    ...
    close(norm(normalize([2, -3, 6])), 1, 1e-12, 'normalized norm = 1 (3d)')
  }),
  check('normalize leaves the zero vector at zero (guard against /0)', () => {
...
suite('algebra/vector: minkowski (J) inner product', [
  check('innerJ weights each axis by its metric sign', () => {
    ...
    equal(innerJ([1, 2, 3], [4, 5, 6], [1, -1, 1]), 12, 'metric (+,-,+)')
  }),
  check('a timelike unit vector has J-square -1, a spacelike one +1', () => {
    const metric = [-1, 1, 1, 1] // mostly-plus with timelike axis 0
    equal(innerJ([1, 0, 0, 0], [1, 0, 0, 0], metric), -1, 'timelike^2 = -1')
    equal(innerJ([0, 1, 0, 0], [0, 1, 0, 0], metric), 1, 'spacelike^2 = +1')
  }),
  check('normJ is the square root of the absolute J-square', () => {
    const metric = [-1, 1, 1, 1]
    // innerJ((2,1,0,0)) = -4 + 1 = -3, normJ = sqrt(3)
    close(normJ([2, 1, 0, 0], metric), Math.sqrt(3), 1e-12, 'sqrt(|-3|)')
  }),
])
