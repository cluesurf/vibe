...
const closeQuat = (a: Quaternion, b: Quaternion, message: string): void => {
...
suite('algebra/group/rotation: vector action (SO(3) by conjugation)', [
  ...
  check('a 180 deg rotor applied twice (a full turn) returns the vector', () => {
    closeQuat(rotateVectorTimes(K, I, 2), I, 'vector after 2 pi = itself')
  }),
])
...
suite('algebra/group/rotation: spinor action and the double cover', [
  check('a 2 pi turn flips a spinor sign (k^2 = -1)', () => {
    ...
    closeQuat(rotateSpinorTimes(K, ONE, 2), negate(ONE), 'spinor after 2 pi = -psi')
  }),
  ...
  check('a 4 pi turn restores the spinor (the cover closes)', () => {
    closeQuat(rotateSpinorTimes(K, ONE, 4), ONE, 'spinor after 4 pi = +psi')
  }),
  check('q and -q give the same SO(3) rotation key (2-to-1 cover)', () => {
    for (const g of [zRotor(0.9), I, K, quaternion(1, 1, 1, 1)]) {
      equal(rotationKey(g), rotationKey(negate(g)), 'q and -q same rotation')
    }
  }),
])
