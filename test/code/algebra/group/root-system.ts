...
suite('algebra/group/root-system: D4 / B4 / F4 counts and norms', [
  check('D4 has 24 roots, all of norm^2 = 2, closed under negation', () => {
    const roots = rootsD4()
  ...
  check('B4 has 32 roots: 24 long (norm^2 2) + 8 short (norm^2 1)', () => {
    const roots = rootsB4()
  ...
  check('F4 has 48 roots: 24 long (norm^2 2) + 24 short (norm^2 1)', () => {
    const roots = rootsF4()
    equal(roots.length, 48, '|F4 roots| = 48')
    equal(distinct(roots), 48, 'distinct')
    const counts = byNorm(roots)
    equal(counts.get(2), 24, '24 long F4 roots')
    equal(counts.get(1), 24, '24 short F4 roots (8 axis + 16 half-integer)')
...
suite('algebra/group/root-system: spinor weights and E8', [
  check('D_n has 2^(n-1) even-parity spinor weights, norm^2 = n/4', () => {
    const w4 = spinorWeightsDn(4)
    equal(w4.length, 8, '|spinor(D4)| = 8')
    for (const w of w4) {
      ok(Math.abs(normSquared(w) - 1) < 1e-12, 'D4 spinor weight norm^2 = 1')
      equal(w.filter(x => x < 0).length % 2, 0, 'even number of minus signs')
    }
    const w5 = spinorWeightsDn(5)
    equal(w5.length, 16, '|spinor(D5)| = 16 (the so(10) generation)')
    for (const w of w5) {
      ok(Math.abs(normSquared(w) - 1.25) < 1e-12, 'D5 spinor weight norm^2 = 5/4')
    }
  }),
  check('E8 has 8 simple roots of norm^2 = 2', () => {
    ...
    for (const r of simple) {
      ok(Math.abs(normSquared(r) - 2) < 1e-12, 'E8 simple root norm^2 = 2')
    }
  }),
  check('the E8 Cartan matrix is a valid Dynkin diagram with det = 1', () => {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      for (let j = 0; j < 8; j++) {
        if (i !== j) {
          ok(cartan[i]![j]! === 0 || cartan[i]![j]! === -1, 'off-diag in {0,-1}')
          equal(cartan[i]![j]!, cartan[j]![i]!, 'Cartan symmetric (simply laced)')
          if (i < j && cartan[i]![j]! === -1) {
            edges++
          }
        }
  ...
  check('E8 has 240 roots, all of norm^2 = 2, closed under negation', () => {
    const roots = rootsE8()
...
suite('algebra/group/root-system: ternary shells and code lattice', [
  check('the {-1,0,1}^4 shells are the regular 4-polytopes', () => {
    ...
    equal(shells.get(4)!.length, 16, 'norm^2=4: the tesseract, 1*16 = 16')
  }),
  check('the norm-2 ternary shell is exactly the 24 D4 roots', () => {
    ...
    ok([...d4].every(k => shell.has(k)), 'D4 roots = norm-2 ternary shell')
  }),
  ...
  check('Construction A on the even-weight code rebuilds the 24 D4 minimal vectors', () => {
...
suite('algebra/group/root-system: probe and lattice direction sets', [
  ...
  check('the 12 icosahedron directions are unit and negation-closed', () => {
    ...
    ok(closedUnderNegation(directions), '12 directions form 6 antipodal axes')
  }),
])
