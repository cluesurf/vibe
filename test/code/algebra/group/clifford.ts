...
import { type Complex, complex, cMul } from '@/code/algebra/linear/complex'
import {
  type ComplexMatrix,
  pauli,
  diracGamma,
...
const scaleByComplex = (matrix: ComplexMatrix, z: Complex): ComplexMatrix =>
  matrix.map(row => row.map(value => cMul(value, z)))
...
suite('algebra/group/clifford: Pauli algebra', [
  check('sigma_1, sigma_2, sigma_3 each square to the identity', () => {
    ...
    ok(cmEquals(cmMultiply(s1!, s1!), cmIdentity(2), TOL), 'sigma_1^2 = I')
    ok(cmEquals(cmMultiply(s2!, s2!), cmIdentity(2), TOL), 'sigma_2^2 = I')
    ok(cmEquals(cmMultiply(s3!, s3!), cmIdentity(2), TOL), 'sigma_3^2 = I')
  }),
  ...
  check('the Pauli commutator: [sigma_1, sigma_2] = 2 i sigma_3', () => {
    const [, s1, s2, s3] = pauli()
    ok(
      cmEquals(cmCommutator(s1!, s2!), scaleByComplex(s3!, c(0, 2)), TOL),
      '[s1,s2] = 2 i s3',
    )
  }),
])
...
suite('algebra/group/clifford: Dirac gamma algebra', [
  ...
  check('the Clifford relation {gamma_mu, gamma_nu} = 2 eta_mu_nu I', () => {
  ...
  check('gamma5^2 = I and gamma5 anticommutes with every gamma_mu', () => {
    const gamma = diracGamma()
    const gamma5 = diracGamma5()
    ok(cmEquals(cmMultiply(gamma5, gamma5), cmIdentity(4), TOL), 'gamma5^2 = I')
    const zero = cmZero(4, 4)
    for (let mu = 0; mu < 4; mu++) {
      ok(
...
suite('algebra/group/clifford: spin double cover', [
  check('the spin generator squares to (1/4) I (eigenvalues +/- 1/2)', () => {
  ...
  check('a 2pi spinor rotation is -I, a 4pi rotation is +I', () => {
    ...
    const rotor2pi = cliffordRotor({ angle: 2 * Math.PI, bivector, size: 4 })
    const rotor4pi = cliffordRotor({ angle: 4 * Math.PI, bivector, size: 4 })
    ok(
