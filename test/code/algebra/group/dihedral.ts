...
const totalDimension = (
  ...
  Object.entries(multiplicities).reduce(
    (sum, [name, m]) => sum + (name === 'triv' || name === 'sign' ? 1 : 2) * m,
    0,
  )

suite('algebra/group/dihedral: face permutation decomposition (odd n)', [
  check('D7 face rep = triv + E1 + E2 + E3 (heptagon, the {7,3} case)', () => {
    const { multiplicities, hasSpinor } = dihedralFacePermutationDecomposition(7)
    ...
    equal(hasSpinor, false, 'a point-group perm rep carries no spinor')
  }),
  check('D7 dimensions sum to 7 (a 7-point transitive permutation rep)', () => {
    const { multiplicities } = dihedralFacePermutationDecomposition(7)
    equal(totalDimension(multiplicities), 7, 'sum of dim*mult = n')
  }),
  check('D5 face rep = triv + E1 + E2, dimensions sum to 5', () => {
    const { multiplicities, hasSpinor } = dihedralFacePermutationDecomposition(5)
  ...
  check('every multiplicity is a non-negative integer', () => {
    for (const n of [5, 7, 9, 11]) {
      const { multiplicities } = dihedralFacePermutationDecomposition(n)
      for (const [name, m] of Object.entries(multiplicities)) {
        ok(Number.isInteger(m), `${name} multiplicity ${m} is an integer`)
        ok(m >= 0, `${name} multiplicity ${m} >= 0`)
      }
      equal(totalDimension(multiplicities), n, `D${n} dimensions sum to n`)
    }
  }),
])
