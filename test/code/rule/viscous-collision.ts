...
suite('rule/viscous-collision: quads are disjoint and momentum-matched', [
  check('the two pairs of each quad carry equal total momentum', () => {
  ...
  check('no slot index is reused across quads (disjoint involution factors)', () => {
    ...
    equal(seen.size, quads.length * 4, 'every quad contributes four fresh slots')
  }),
])
...
suite('rule/viscous-collision: viscousRotate is a conserving involution', [
  check('applying it twice is the identity', () => {
    const start = pattern()
    const twice = apply(viscousRotate({ directions: D4 }), apply(viscousRotate({ directions: D4 }), start))
    equal(JSON.stringify(Array.from(twice)), JSON.stringify(Array.from(start)), 'self-inverse')
  }),
  check('it conserves mass and the full momentum vector', () => {
    ...
    equal(JSON.stringify(momentum(after)), JSON.stringify(momentum(start)), 'momentum conserved')
  }),
  check('it actually moves occupancy when a quad fires (not a no-op)', () => {
    const [a, b, c, d] = quads[0] as [number, number, number, number]
    const slots = new Int8Array(DEGREE)
    slots[a] = 1
...
suite('rule/viscous-collision: controlled variant stays a conserving involution', [
  ...
  check('the controlled collision conserves mass and momentum', () => {
    const start = pattern()
    const after = apply(controlledViscousRotate({ directions: D4 }), start)
