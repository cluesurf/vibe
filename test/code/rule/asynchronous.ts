...
const sumLocal: LocalMap = ({ neighborhood }) => neighborhood.reduce((a, b) => a + b, 0)
...
suite('rule/asynchronous: running-state sequential sweep', [
  check('sequential sweep matches the hand result WITH running state', () => {
    // input [1,0,0,1], order 0,1,2,3, reads the running clone:
    //   0 <- {v1=0}=0          -> [0,0,0,1]
    //   1 <- {v0=0,v2=0}=0     -> [0,0,0,1]
    //   2 <- {v1=0,v3=1}=1     -> [0,0,1,1]
    //   3 <- {v2=1}=1          -> [0,0,1,1]
    equal(JSON.stringify(stepAsync('sequential', [1, 0, 0, 1], 1)), JSON.stringify([0, 0, 1, 1]))
  }),
  check('the running-state result differs from the synchronous result', () => {
    const async = stepAsync('sequential', [1, 0, 0, 1], 1)
    const sync = (() => {
      const out = synchronousRule({ name: 's', local: sumLocal }).step({
        substrate: graph,
    ...
    ok(JSON.stringify(async) !== JSON.stringify(sync), 'async propagates within the sweep')
  }),
  check('the identity local map is a no-op', () => {
    equal(
      JSON.stringify(stepAsync('sequential', [1, -1, 0, 1], 1, identityLocal)),
...
suite('rule/asynchronous: random order is deterministic per seed', [
  check('same seed gives the same result', () => {
    ...
    equal(JSON.stringify(a), JSON.stringify(b), 'reproducible under a fixed seed')
  }),
])
