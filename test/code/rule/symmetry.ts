...
import { makeWill, fillWillPattern, charge, cellTone, Will } from '@/code/tone/will'
import {
  chargeConjugate,
  timeReverse,
  parityReflect,
...
suite('rule/symmetry: charge conjugation C', [
  check('C is an involution', () => {
    const w = sample()
    exactArray(chargeConjugate(chargeConjugate(w)).data, w.data, 'C(C(w)) = w')
  }),
  check('C flips the total charge', () => {
    const w = sample()
    equal(charge(chargeConjugate(w)), -charge(w), 'C negates the charge')
  }),
...
suite('rule/symmetry: time reversal T', [
  ...
  check('T preserves the total charge and every per-cell tone', () => {
    ...
    for (let cell = 0; cell < mesh.cellCount; cell++) {
      equal(cellTone(t, cell), cellTone(w, cell), `cell ${cell} tone preserved`)
    }
  }),
])
...
suite('rule/symmetry: parity P', [
  check('P is an involution', () => {
    const w = sample()
    const p = parityReflect({ will: w, directions, side: SIDE, axis: 0 })
    const pp = parityReflect({ will: p, directions, side: SIDE, axis: 0 })
...
suite('rule/symmetry: combined CPT', [
  check('CPT negates the charge (C flips, P and T preserve)', () => {
    const w = sample()
    const cpt = chargeParityTime({ will: w, directions, side: SIDE, axis: 0 })
