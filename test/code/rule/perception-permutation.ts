...
suite('rule/perception-permutation: the 9-state core', [
  check('the signed pair sum is conserved on all nine transitions', () => {
    for (const a of TONES) {
      for (const b of TONES) {
        const [na, nb] = perceptionPermutation(a, b)
        equal(na + nb, a + b, `pair (${a},${b}) must preserve the signed sum`)
      }
    }
  }),
  check('the permutation is a bijection on the nine ordered pairs', () => {
    ...
    equal(seen.size, 9, 'all nine inputs must map to distinct outputs')
  }),
  check('PERCEPTION_FORWARD matches perceptionPermutation exactly', () => {
    for (const a of TONES) {
      for (const b of TONES) {
        const [na, nb] = perceptionPermutation(a, b)
        equal(
  ...
  check('PERCEPTION_INVERSE is the exact functional inverse of FORWARD', () => {
    for (let i = 0; i < 9; i++) {
      equal(PERCEPTION_INVERSE[PERCEPTION_FORWARD[i]!], i, `inverse undoes index ${i}`)
      equal(PERCEPTION_FORWARD[PERCEPTION_INVERSE[i]!], i, `forward undoes index ${i}`)
    }
  }),
...
suite('rule/perception-permutation: reversible sweeps and charge', [
  check('block CA forward then inverse recovers the ring exactly', () => {
    const length = 8
    const start = Int8Array.from([0, 0, 1, -1, 1, 0, -1, 0])
    for (const parity of [0, 1]) {
      const tone = Int8Array.from(start)
      perceptionBlockBeat({ tone, length, parity, table: PERCEPTION_FORWARD })
      perceptionBlockBeat({ tone, length, parity, table: PERCEPTION_INVERSE })
      for (let i = 0; i < length; i++) {
        equal(tone[i], start[i], `parity ${parity}: slot ${i} must return`)
      }
    }
  }),
  check('block CA conserves the total signed charge', () => {
    ...
    perceptionBlockBeat({ tone, length, parity: 0, table: PERCEPTION_FORWARD })
    equal(sum(tone), before, 'a matching of the rule conserves charge')
  }),
  check('edge-colouring sweep forward then reversed-inverse recovers the start', () => {
  ...
  check('CSR matching sweep conserves charge and is deterministic', () => {
    const n = 6
    ...
    perceptionMatchingSweepCsr({ tone: a, offsets, adj, matched, start: 0 })
    perceptionMatchingSweepCsr({ tone: b, offsets, adj, matched, start: 0 })

    equal(sum(a), before, 'the matching must conserve total charge')
    for (let i = 0; i < n; i++) {
      equal(a[i], b[i], `deterministic: slot ${i} must match across runs`)
    }
  }),
])
