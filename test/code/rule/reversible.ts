...
suite('rule/reversible: even from old odd, then odd from new even', [
  check('the two half-steps match the hand derivation', () => {
    ...
    equal(JSON.stringify(stepRev(sumLocal, [1, 0, 0, 1])), JSON.stringify([0, 1, 1, 1]))
  }),
  check('the result differs from the synchronous step (odd sees the new even)', () => {
    ...
    ok(JSON.stringify(rev) !== JSON.stringify(sync), 'block ordering changes the result')
  }),
  check('the identity local map is a no-op', () => {
    equal(JSON.stringify(stepRev(identityLocal, [1, -1, 0, 1])), JSON.stringify([1, -1, 0, 1]))
  }),
])
