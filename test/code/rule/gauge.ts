...
suite('rule/gauge: structural invariants of a sweep', [
  check('the tone configuration passes through unchanged', () => {
    const field = makeGaugeField({ graph: triangle, group: { form: 'u1', q: Q } })
    const config = makeConfiguration({ alphabet: { form: 'ternary' }, size: 3 })
    ...
    equal(getTone(out.configuration, { element: 0 }), 1, 'tone 0 unchanged')
    equal(getTone(out.configuration, { element: 2 }), -1, 'tone 2 unchanged')
  }),
  check('every link stays a valid Z_q element in [0, q)', () => {
    ...
    for (let sweep = 0; sweep < 10; sweep++) {
      rule.step({
        substrate: triangle,
        configuration: makeConfiguration({ alphabet: { form: 'ternary' }, size: 3 }),
        beat: sweep,
  ...
  check('a triangle-free graph admits no move (links stay at identity)', () => {
    const field = makeGaugeField({ graph: square, group: { form: 'u1', q: Q } })
    gaugeRule({ name: 'g', field, beta: 1 }).step({
      substrate: square,
      configuration: makeConfiguration({ alphabet: { form: 'ternary' }, size: 4 }),
      beat: 0,
      rng: makeRng({ seed: 3 }),
    })
...
suite('rule/gauge: determinism', [
  check('the same seed produces the same sweep', () => {
    ...
    equal(JSON.stringify(run()), JSON.stringify(run()), 'reproducible under a fixed seed')
  }),
])
