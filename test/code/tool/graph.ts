...
function csrByHand(neighbors: number[][]): { offsets: number[]; adj: number[] } {
  const offsets = [0]
  const adj: number[] = []
...
suite('tool/graph: adjacency and CSR', [
  ...
  check('mostConnectedNode picks max degree, ties to lowest index', () => {
  ...
  check('edgesFromCsr returns each undirected edge once with eu < ev', () => {
  ...
  check('adjacencyListsEqual is order-insensitive and detects a difference', () => {
    ok(adjacencyListsEqual(PATH, [[1], [2, 0], [3, 1], [2]]), 'same up to order')
    ok(!adjacencyListsEqual(PATH, [[1], [0], [1, 3], [2]]), 'missing edge differs')
  }),
])

suite('tool/graph: BFS traversal', [
  check('csrDistances on the path equal the hop count from node 0', () => {
    const { offsets, adj } = toCsr(PATH)
    const dist = csrDistances({ offsets, adj, size: 4, source: 0 })
    exactArray(dist, Int32Array.from([0, 1, 2, 3]), 'path distances')
  }),
  check('csrDistances with maxRadius stops after that many rings', () => {
    const { offsets, adj } = toCsr(PATH)
    const dist = csrDistances({ offsets, adj, size: 4, source: 0, maxRadius: 1 })
    // only node 0 (0) and node 1 (1) reached; 2 and 3 stay -1
    exactArray(dist, Int32Array.from([0, 1, -1, -1]), 'radius-1 distances')
  }),
  check('csrDistances on a 6-cycle matches hand distances (min of both arcs)', () => {
    // cycle 0-1-2-3-4-5-0
    const cyc: number[][] = [
      [1, 5],
      [0, 2],
    ...
    exactArray(dist, Int32Array.from([0, 1, 2, 3, 2, 1]), 'cycle distances')
  }),
  ...
  check('largestComponentNodes returns a full component of a disconnected graph', () => {
    ...
    exactArray(Int32Array.from([...comp].sort((a, b) => a - b)), Int32Array.from([0, 1]), 'members')
  }),
])
