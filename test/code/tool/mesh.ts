...
suite('tool/mesh: opposite is an involution', [
  ...directionalMeshes.map(({ name, mesh }) =>
    check(`${name}: opposite(opposite(d)) === d`, () => {
      ok(
        oppositeInvolutes(mesh),
        `${name} opposite must be an involution`,
      )
    }),
  ),
  ...directionalMeshes
    ...
    .map(({ name, mesh }) =>
      check(`${name}: opposite has no fixed direction`, () => {
        for (let d = 0; d < mesh.degree; d++) {
          ok(
            mesh.opposite(d) !== d,
            `${name} direction ${d} is its own opposite`,
          )
        }
      }),
    ),
  check(
    'd4-rest: only the rest direction (24) is its own opposite',
    () => {
      const m = d4MeshWithRest({ side: 3 })
      ...
      for (let d = 0; d < 24; d++) {
        ok(
          m.opposite(d) !== d,
          `moving direction ${d} should not be self-opposite`,
        )
      }
...
suite('tool/mesh: streaming round-trips and symmetry', [
  ...directionalMeshes.map(({ name, mesh }) =>
    check(
      `${name}: neighbour(neighbour(c,d), opposite(d)) === c`,
      () => {
        ok(
          neighbourRoundTrips(mesh),
          `${name} round-trip must return home`,
        )
      },
    ),
  ),
  ...directionalMeshes.map(({ name, mesh }) =>
    check(
      `${name}: streaming is symmetric (a->b implies b->a back)`,
      () => {
        ok(
          streamingSymmetric(mesh),
          `${name} streaming must be symmetric`,
        )
      },
    ),
  ),
  check(
    ...
    () => {
      ...
      equal(
        ...
        24,
        'all 24 directions land on different cells',
      )
    },
  ),
])
...
suite('tool/mesh: shell distances', [
  check(
    'shellDistances gives the source distance 0 and reaches every cell',
    () => {
      const m = squareMesh({ side: 5 })
  ...
  check(
    'square shell distance is the L1 (taxicab) torus distance',
    () => {
      const side = 5
