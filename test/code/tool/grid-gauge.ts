...
suite('tool/grid-gauge: plaquette flux is the lattice curl', [
  check('the zero field has zero flux everywhere', () => {
    const g: GridGauge = {
      Ax: makeGridGrid(SIDE),
      Ay: makeGridGrid(SIDE),
    }
  ...
  check(
    'flux equals the hand-computed curl on an interior cell',
    () => {
...
suite(
  'tool/grid-gauge: Wilson loop is the enclosed flux (discrete Stokes)',
  [
    check(
      'loop around a rectangle equals the sum of inner plaquette fluxes',
      () => {
        const g = integerField()
...
suite('tool/grid-gauge: gauge invariance', [
  check(
    'A -> A + d(lambda) leaves every plaquette flux unchanged',
    () => {
      const g = integerField()
  ...
  check(
    'the transform leaves an interior Wilson loop unchanged',
    () => {
      const g = integerField()
      const lambda = makeGridGrid(SIDE)
...
suite('tool/grid-gauge: single-vortex field', [
  check(
    'a loop encircling the vortex returns exactly the flux (holonomy)',
    () => {
      const flux = 1.3
  ...
  check('a plaquette far from the vortex carries ~0 flux', () => {
    ...
    close(
      plaquetteFlux(g, { x: 0, y: 0, side: SIDE }),
      0,
      1e-9,
      'smooth region has zero flux',
    )
  }),
  check(
    'the total flux over the torus is zero (topological constraint)',
    () => {
      const flux = 0.8
      const g = vortexGaugeField({
        side: SIDE,
