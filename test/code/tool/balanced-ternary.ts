...
suite('tool/balanced-ternary: hand-derived expansions', [
  check('known little-endian expansions match the hand derivation', () => {
  ...
  check('every produced digit is a tone in {-1, 0, +1}', () => {
    for (let v = -13; v <= 13; v++) {
      for (const d of toBalancedTernary(v, 3)) {
        ok(d === -1 || d === 0 || d === 1, `digit ${d} of ${v} must be a tone`)
      }
    }
  }),
])

suite('tool/balanced-ternary: fromBalancedTernary is the positional value', [
  check('fromBalancedTernary equals the independent positional sum', () => {
...
suite('tool/balanced-ternary: round-trip over the whole range', [
  ...[1, 2, 3, 4].map(K =>
    check(`from(to(v)) === v for every representable v at K=${K}`, () => {
      const cap = balancedTernaryCap(K)

      for (let v = -cap; v <= cap; v++) {
        equal(fromBalancedTernary(toBalancedTernary(v, K)), v, `round-trip ${v}`)
      }
    }),
  ),
  check('out-of-range values clamp to the nearest representable bound', () => {
    ...
    equal(fromBalancedTernary(toBalancedTernary(99, 2)), 4, 'clamp to +4 at K=2')
    equal(fromBalancedTernary(toBalancedTernary(-99, 2)), -4, 'clamp to -4 at K=2')
  }),
])

suite('tool/balanced-ternary: field representability', [
  check('a field within the cap is representable, one over is not', () => {
    ok(isBalancedTernaryField([-13, 0, 7, 13], 3), 'all within +/-13 at K=3')
    ok(!isBalancedTernaryField([0, 14], 3), '14 exceeds the K=3 cap of 13')
    ok(!isBalancedTernaryField([-14, 0], 3), '-14 exceeds the K=3 cap')
    ok(isBalancedTernaryField([1, -1, 0], 1), 'single trits all within +/-1')
    ok(!isBalancedTernaryField([2], 1), '2 exceeds the single-trit cap')
  }),
])
