...
suite('substrate/modular-group: matrix arithmetic', [
  check('the identity is a unit and multiplication is associative', () => {
    equal(multiplyIntegerMatrix(I, T).join(','), T.join(','), 'I*T = T')
    equal(multiplyIntegerMatrix(T, I).join(','), T.join(','), 'T*I = T')
  ...
  check('sign-normalization collapses M and -M', () => {
    ...
    equal(normalizeModularMatrix(T), normalizeModularMatrix(negT), 'T ~ -T')
  }),
  check('S has order 2 and ST has order 3 in PSL(2,Z)', () => {
    const s2 = multiplyIntegerMatrix(S, S)
    equal(normalizeModularMatrix(s2), normalizeModularMatrix(I), 'S^2 = 1')
    const st = multiplyIntegerMatrix(S, T)
    const st3 = multiplyIntegerMatrix(multiplyIntegerMatrix(st, st), st)
    equal(normalizeModularMatrix(st3), normalizeModularMatrix(I), '(ST)^3 = 1')
    // ST is not itself the identity (order is exactly 3, not 1).
    notOk(normalizeModularMatrix(st) === normalizeModularMatrix(I), 'ST != 1')
  }),
])

suite('substrate/modular-group: graph and continued fractions', [
  check('the Cayley graph is symmetric and embedded inside the disc', () => {
  ...
  check('continued fractions reach the right rationals', () => {
    equal(rationalFromContinuedFraction([2]).num, 2, '[2] = 2/1 numerator')
    equal(rationalFromContinuedFraction([2]).den, 1, '[2] = 2/1 denominator')
