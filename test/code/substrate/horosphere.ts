...
suite('substrate/horosphere: ideal direction and Busemann height', [
  check('the ideal direction is the unit-normalized farthest cell', () => {
    const xi = idealDirection([[0.1, 0], [0.5, 0], [0.9, 0]])
...
suite('substrate/horosphere: the horosphere frame', [
  check('the 2D frame is one unit vector perpendicular to the ideal point', () => {
  ...
  check('the 3D frame is orthonormal and perpendicular to the ideal point', () => {
    const ideal = [0, 0, 1]
  ...
  check('extractBand keeps the cells near the level', () => {
    const band = extractBand({ busemann: [-0.4, 0, 0.4, 1.2], level: 0, half: 0.5 })
