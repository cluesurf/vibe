...
suite('substrate/coxeter/coxeter-growth: finite Poincare polynomials', [
  check('a finite group growth sums to the group order', () => {
    for (const [symbol, order] of [
      ...
      equal(seq[0], 1n, `[${symbol}] identity coefficient`)
      ok(sum(seq) === order, `[${symbol}] W(1) = |W| = ${order}`)
    }
  }),
  check('the finite growth polynomial is palindromic (Poincare duality)', () => {
    for (const symbol of [[3], [4], [3, 3], [4, 3]]) {
      ...
      for (let i = 0; i < poly.length; i++) {
        ok(poly[i] === poly[poly.length - 1 - i], `[${symbol}] palindrome at ${i}`)
      }
    }
  }),
])
...
suite('substrate/coxeter/coxeter-growth: self-consistent expansion', [
  check('expanding via the denominator recurrence reproduces the series', () => {
    for (const symbol of [[7, 3], [5, 4], [5, 3, 4]]) {
      ...
      for (let i = seed; i < seq.length; i++) {
        ok(rebuilt[i] === seq[i], `[${symbol}] recurrence reproduces term ${i}`)
      }
    }
  }),
  check('an infinite (hyperbolic) group growth is monotone and unbounded', () => {
