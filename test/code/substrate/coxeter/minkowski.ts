...
suite('substrate/coxeter/minkowski: matrices and the metric', [
  ...
  check('the Minkowski inner product carries the diagonal signature', () => {
    const metric = [1, 1, -1]
    close(innerJ([0, 0, 1], [0, 0, 1], metric), -1, 1e-12, 'timelike norm')
    close(innerJ([1, 0, 0], [1, 0, 0], metric), 1, 1e-12, 'spacelike norm')
    close(innerJ([1, 2, 3], [0, 0, 0], metric), 0, 1e-12, 'zero vector')
  }),
])
...
suite('substrate/coxeter/minkowski: reflections are involutions', [
  ...
  check('matVec of identity is the vector', () => {
    closeArray(matVec(identity(3), [7, -1, 4]), [7, -1, 4], 1e-12, 'I*v')
  }),
])
