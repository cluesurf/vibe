...
import { suite, check, equal, ok, notOk, close } from '@/test/code/harness'
import {
  gramMatrix,
  classifyGeometry,
  honeycombCell,
...
suite('substrate/coxeter/schlafli: Gram matrix and geometry', [
  ...
  check('classifyGeometry separates spherical, euclidean, hyperbolic', () => {
    for (const s of [[3, 3], [4, 3], [5, 3], [3, 3, 3]]) {
      equal(classifyGeometry(s), 'spherical', `{${s}}`)
    }
    for (const s of [[4, 4], [3, 6], [6, 3], [4, 3, 4]]) {
      equal(classifyGeometry(s), 'euclidean', `{${s}}`)
    }
    for (const s of [[7, 3], [5, 3, 4], [3, 5, 3], [3, 4, 3, 4]]) {
      equal(classifyGeometry(s), 'hyperbolic', `{${s}}`)
    }
  }),
  check('cell and vertex figure drop the last/first entry', () => {
    equal(honeycombCell([5, 3, 4]).join(','), '5,3', 'cell')
    equal(honeycombVertexFigure([5, 3, 4]).join(','), '3,4', 'vertex figure')
  }),
])
...
suite('substrate/coxeter/schlafli: honeycomb taxonomy', [
  check('{5,3,4} is compact; {3,4,3,4} is ideal-finite-cell', () => {
    ...
    ok(isIdealFiniteCellHoneycomb([3, 4, 3, 4]), '{3,4,3,4} ideal finite cell')
    notOk(isIdealFiniteCellHoneycomb([5, 3, 4]), '{5,3,4} not ideal')
  }),
  check('exactly four compact regular hyperbolic honeycombs exist in H^3', () => {
    const found = enumerateCompactHoneycombs({ dimension: 3, maxEntry: 5 })
...
suite('substrate/coxeter/schlafli: dihedral angles and edge regime', [
  check('dihedral angles match the Platonic solids', () => {
    ...
    close(dihedralAngleDegrees({ p: 3, q: 3 }), 70.528779, 1e-4, 'tetrahedron')
    close(dihedralAngleDegrees({ p: 5, q: 3 }), 116.565051, 1e-4, 'dodecahedron')
  }),
  check('edge regime: cubic honeycomb is flat, others curve', () => {
    equal(edgeRegime({ p: 4, q: 3, r: 4 }).regime, 'euclidean', '{4,3,4} flat')
    equal(edgeRegime({ p: 4, q: 3, r: 3 }).regime, 'spherical', '{4,3,3} tesseract')
    equal(edgeRegime({ p: 5, q: 3, r: 4 }).regime, 'hyperbolic', '{5,3,4}')
    // r copies of the cube (90 deg each) close at exactly 360 for r=4.
    close(edgeRegime({ p: 4, q: 3, r: 4 }).totalAngleDegrees, 360, 1e-6, '4*90')
  }),
])
