...
suite(
  'substrate/coxeter/exact-modular: face reflections are involutions',
  [
    check(
      ...
      () => {
        ...
        for (const nc of neighbors) {
          const back = engine
            .neighbors(nc)
            .map(c => engine.fingerprint(c))

          ok(back.includes(home), 'stepping back returns the origin')
          notOk(
            engine.fingerprint(nc) === home,
            'a neighbour is not the origin',
          )
        }
      },
    ),
...
suite('substrate/coxeter/exact-modular: the built graph', [
  ...
  check('{7,3} builds with facet count 7', () => {
    equal(
      buildTilingExact({ symbol: [7, 3], maxCells: 50 }).facetCount,
      7,
      '{7,3}',
    )
  }),
