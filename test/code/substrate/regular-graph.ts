...
suite('substrate/regular-graph: determinism', [
  check('the same seed yields an identical graph', () => {
    const a = buildRegularGraph({ n: 200, degree: 4, rng: makeRng({ seed: 7 }) })
    const b = buildRegularGraph({ n: 200, degree: 4, rng: makeRng({ seed: 7 }) })
  ...
  check('a different seed yields a different graph', () => {
    const a = buildRegularGraph({ n: 200, degree: 4, rng: makeRng({ seed: 7 }) })
    const c = buildRegularGraph({ n: 200, degree: 4, rng: makeRng({ seed: 8 }) })
    let differs = false
...
suite('substrate/regular-graph: structural invariants', [
  check('no self-loops and degree never exceeds the request', () => {
    const g = buildRegularGraph({ n: 300, degree: 6, rng: makeRng({ seed: 11 }) })
  ...
  check('CSR adjacency is symmetric and consistent with the edge list', () => {
    const g = buildRegularGraph({ n: 150, degree: 4, rng: makeRng({ seed: 3 }) })
    ...
    const sets: Set<number>[] = Array.from({ length: 150 }, () => new Set<number>())
    for (let i = 0; i < 150; i++) {
