...
suite('substrate/bethe-tree: coordination and shells', [
  ...
  check('the root has degree q and interior cells have degree q', () => {
    const adj = betheTree(3, 3)
    ...
    for (let i = 0; i < adj.length; i++) {
      ...
      ok(deg === 1 || deg === 3, `node ${i} degree is 1 (leaf) or q=3`)
    }
  }),
])

suite('substrate/bethe-tree: tree structure', [
  check('it is a tree: edges = nodes - 1, and adjacency is symmetric', () => {
