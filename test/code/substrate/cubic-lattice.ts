...
suite('substrate/cubic-lattice: degrees and adjacency', [
  check('a 3x3 mesh has the interior degree 2d and corner degree d', () => {
  ...
  check('a 3x3x3 mesh centre has degree 6 and degree sum is 2*edges', () => {
...
suite('substrate/cubic-lattice: centre and distance', [
  ...
  check('Euclidean distance is the integer-coordinate metric', () => {
    ...
    close(cubicLatticeDistance({ lattice: lat, from: 0, to: 8 }), Math.sqrt(8), 1e-12, '(0,0)-(2,2)')
    close(cubicLatticeDistance({ lattice: lat, from: 0, to: 1 }), 1, 1e-12, 'adjacent')
    close(cubicLatticeDistance({ lattice: lat, from: 4, to: 4 }), 0, 1e-12, 'self')
  }),
])
