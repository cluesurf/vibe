...
import { suite, check, equal, ok, notOk, exactArray, close } from '@/test/code/harness'
import {
  gramSignature,
...
suite('substrate/coxeter: Gram signature classifies the geometry', [
  check('spherical (finite) honeycombs are positive-definite', () => {
    for (const symbol of [[3, 3], [3, 3, 3]]) {
      const sig = gramSignature(symbol)
      equal(sig.negative, 0, `{${symbol}} negative`)
      equal(sig.zero, 0, `{${symbol}} zero`)
    }
  }),
  check('Euclidean honeycombs are positive-semidefinite (one zero)', () => {
    for (const symbol of [[4, 4], [3, 6], [6, 3], [4, 3, 4]]) {
      const sig = gramSignature(symbol)
      equal(sig.negative, 0, `{${symbol}} negative`)
      equal(sig.zero, 1, `{${symbol}} zero`)
    }
  }),
  check('hyperbolic honeycombs are Lorentzian (exactly one negative)', () => {
    for (const symbol of [
      [7, 3],
      [5, 3, 4],
      [3, 5, 3],
      ...
      const sig = gramSignature(symbol)
      equal(sig.negative, 1, `{${symbol}} negative`)
      equal(sig.zero, 0, `{${symbol}} zero`)
    }
  }),
])
...
suite('substrate/coxeter: subdiagram detection', [
  check('the [3,4,3] (F4 / 24-cell) substructure is found where present', () => {
  ...
  check('it is absent where it should be', () => {
    ...
    notOk(symbolContainsSubdiagram([3, 4, 3], [3, 4, 3, 4]), 'longer pattern')
...
suite('substrate/coxeter: reflections and the -cos(pi/m) entry', [
  check('reflecting twice in any D4 root is the identity', () => {
    ...
    for (const a of roots) {
      exactArray(reflectRoot(reflectRoot(v, a), a), v, `R_${a}^2`)
    }
  }),
  ...
  check('the product of two reflections has order m (m=2 and m=3)', () => {
    const v = [1, 2, 3, 4]
    // Orthogonal pair (dot 0, m=2): (R_a R_b)^2 = identity.
    const a2 = [1, 1, 0, 0]
    const b2 = [1, -1, 0, 0]
    const rr2 = (x: number[]): number[] => reflectRoot(reflectRoot(x, b2), a2)
    exactArray(rr2(rr2(v)), v, '(R_a R_b)^2 for m=2')

    // Pair at dot -1 (m=3): order is 3, not 2.
    const a3 = [1, 1, 0, 0]
    const b3 = [0, -1, 1, 0]
    const rr3 = (x: number[]): number[] => reflectRoot(reflectRoot(x, b3), a3)
    exactArray(rr3(rr3(rr3(v))), v, '(R_a R_b)^3 for m=3')
    notOk(arraysEqual(rr3(rr3(v)), v), '(R_a R_b)^2 is not identity for m=3')
  }),
])
