...
suite('substrate/margenstern/fibonacci-tree: father and sons are inverse', [
  ...
  check('a node has 3 sons (white) or 2 sons (black) matching nodeType', () => {
    for (let n = 1; n <= 60; n++) {
      equal(sons(n).length, nodeType(n), `son count = nodeType for ${n}`)
      ok(nodeType(n) === 2 || nodeType(n) === 3, `nodeType in {2,3} for ${n}`)
    }
  }),
  check('the continuator is the middle/first preferred son', () => {
    for (let n = 2; n <= 60; n++) {
      ok(sons(n).includes(continuator(n)), `continuator is a son of ${n}`)
    }
  }),
...
suite('substrate/margenstern/fibonacci-tree: depth, paths and routes', [
  ...
  check('a route walks only true tree edges and reaches its target', () => {
