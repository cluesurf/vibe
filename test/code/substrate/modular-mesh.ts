...
const cfg = { numCells: 5, cellSize: 8, intraDegree: 3, interPerCell: 2 }

suite('substrate/modular-mesh: structure', [
  check('node count is numCells * cellSize and the cell labels are exact', () => {
    const { g, cellOf } = modularMesh({ ...cfg, rng: makeRng({ seed: 1 }) })
    equal(g.size, cfg.numCells * cfg.cellSize, 'node count')
    for (let c = 0; c < cfg.numCells; c++) {
      for (let i = 0; i < cfg.cellSize; i++) {
        equal(cellOf[c * cfg.cellSize + i], c, `cell label of ${c}.${i}`)
      }
    }
  }),
  check('adjacency is symmetric with no self-loops and all fills are +1', () => {
    const { g, fills } = modularMesh({ ...cfg, rng: makeRng({ seed: 1 }) })
