...
suite('substrate/radial-tree: root and boundary', [
  check('the innermost cell has the smallest radius (ties to lowest index)', () => {
    equal(innermostCell([0.5, 0.1, 0.9, 0.3]), 1, 'smallest radius')
    equal(innermostCell([0.2, 0.2, 0.9]), 0, 'tie to lowest index')
  }),
  check('boundaryByRadius keeps cells beyond a fraction of the max radius', () => {
    const out = boundaryByRadius({ radii: [0, 1, 2, 3, 4], fraction: 0.5 })
...
suite('substrate/radial-tree: the BFS tree and LCA', [
  ...
  check('on a balanced binary tree the LCA depth is the branch point', () => {
    //        0
    //      /   \
    //     1     2
    //    / \   / \
