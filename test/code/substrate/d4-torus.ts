...
suite('substrate/d4-torus: the 24 D4 roots', [
  ...
  check('opposite is a fixed-point-free involution on the 24 directions', () => {
    const roots = rootsD4()
    for (let i = 0; i < roots.length; i++) {
      ...
      ok(vecEqExact(roots[o]!, negate(roots[i]!)), 'opposite is negation')
    }
  }),
])
...
suite('substrate/d4-torus: the M=4 finite torus', [
  ...
  check('every cell has degree exactly 24 with no repeated neighbour', () => {
    const torus = buildD4Torus(4)
  ...
  check('every neighbour index is a valid in-lattice cell', () => {
    ...
    for (const row of torus.neigh) {
      for (const j of row) {
        ok(Number.isInteger(j) && j >= 0 && j < n, `neighbour ${j} in range`)
      }
    }
  }),
