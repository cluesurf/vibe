...
const flatNeighbors = (g: Graph): number[] => g.neighbors.flatMap(r => Array.from(r))

suite('substrate/hyperbolic-graph: embedding and symmetry', [
  check('the sunflower graph embeds inside the disc and is symmetric', () => {
    const g = hyperbolicSunflower({ count: 80, radius: 5, connectThreshold: 1.2 })
    assertInsideDisc(g, 'sunflower')
    assertSymmetric(g, 'sunflower')
  }),
  check('the regular {5,4} tiling embeds inside the disc and is symmetric', () => {
    const g = hyperbolicTiling({ p: 5, q: 4, depth: 3, connectThreshold: 1.2 })
...
suite('substrate/hyperbolic-graph: determinism', [
  check('the sunflower (no rng) rebuilds identically', () => {
    const a = hyperbolicSunflower({ count: 60, radius: 4, connectThreshold: 1 })
    const b = hyperbolicSunflower({ count: 60, radius: 4, connectThreshold: 1 })
    exactArray(flatNeighbors(a), flatNeighbors(b), 'sunflower adjacency')
  }),
  check('the Halton graph rebuilds identically', () => {
    const a = hyperbolicHalton({ count: 60, radius: 4, connectThreshold: 1 })
    const b = hyperbolicHalton({ count: 60, radius: 4, connectThreshold: 1 })
    exactArray(flatNeighbors(a), flatNeighbors(b), 'halton adjacency')
  }),
  check('the seeded random graph reproduces under the same seed', () => {
    const a = hyperbolicGraph({ count: 60, radius: 4, connectThreshold: 1, rng: makeRng({ seed: 7 }) })
    const b = hyperbolicGraph({ count: 60, radius: 4, connectThreshold: 1, rng: makeRng({ seed: 7 }) })
    equal(a.size, b.size, 'same size')
    exactArray(flatNeighbors(a), flatNeighbors(b), 'random adjacency under fixed seed')
  }),
])
