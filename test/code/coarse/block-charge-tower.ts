...
suite('coarse/block-charge-tower: charge conservation up the tower', [
  ...
  check(
    'a uniform field conserves total charge and has zero compressibility',
    () => {
      const levels = blockChargeTower({
        field: [1, 1, 1, 1],
        levels: 2,
      })
      ...
      for (const lv of levels) {
        ...
        close(
          ...
          0,
          1e-12,
          'uniform field has zero variance',
        )
      }
  ...
  check(
    'compressibility is Var/blockSize and total is conserved',
    () => {
      const levels = blockChargeTower({
        field: [2, 0, 2, 0],
        levels: 1,
