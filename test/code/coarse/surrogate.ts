...
import { suite, check, close, equal, ok, exactArray } from '@/test/code/harness'
import {
  fitMarkovSurrogate,
...
suite('coarse/surrogate: fit and marginal', [
  ...
  check('the fit surrogate is row-stochastic and matches the counts', () => {
  ...
  check('add-alpha smoothing keeps rows stochastic', () => {
    ...
    for (const row of tpm) {
      close(row.reduce((a, b) => a + b, 0), 1, TOL, 'row sums to 1')
    }
  }),
...
suite('coarse/surrogate: prediction beats the baseline', [
  ...
  check('the marginal baseline is strictly worse on structured data', () => {
    const test = [0, 1, 0, 1, 0]
    const ll = predictiveLogLikelihood({ tpm: [[0, 1], [1, 0]], test, lag: 1 })
    ...
    ok(ll > mll, 'the learned surrogate must beat the memoryless marginal')
  }),
])
