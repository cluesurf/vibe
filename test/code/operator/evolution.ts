...
import { suite, check, equal, close, ok, notOk } from '@/test/code/harness'
import {
  makeStateSpace,
...
suite('operator/evolution: state space', [
  check('dimension is valueCount^cells', () => {
    equal(makeStateSpace({ cells: 2, alphabet: { form: 'ternary' } }).dimension, 9, '3^2')
    equal(makeStateSpace({ cells: 3, alphabet: { form: 'boolean' } }).dimension, 8, '2^3')
    equal(makeStateSpace({ cells: 2, alphabet: { form: 'clock', q: 4 } }).dimension, 16, '4^2')
  }),
])
...
suite('operator/evolution: Hamiltonian from a permutation', [
  check('a non-permutation is rejected', () => {
    ...
    const out = hamiltonianFromPermutation({ perm: Int32Array.from([0, 0, 1, 2]) })
  ...
  check('the identity has all-zero eigen-phases', () => {
    const out = hamiltonianFromPermutation({ perm: Int32Array.from([0, 1, 2, 3]) })
  ...
  check('a single 4-cycle gives the four 4th-root phases', () => {
    const out = hamiltonianFromPermutation({ perm: Int32Array.from([1, 2, 3, 0]) })
    const expected = [0, Math.PI / 2, Math.PI, (3 * Math.PI) / 2]
    equal(out.eigenvalues.length, 4, 'four phases')
    for (let k = 0; k < 4; k++) {
      close(out.eigenvalues[k]!, expected[k]!, 1e-12, `phase 2*pi*${k}/4`)
    }
  }),
  check('two transpositions give {0,0,pi,pi}', () => {
    const out = hamiltonianFromPermutation({ perm: Int32Array.from([1, 0, 3, 2]) })
