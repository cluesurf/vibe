...
suite('operator/screened-greens-function: Yukawa fixed point', [
  ...
  check('the screened potential is positive everywhere and peaks at the source', () => {
    const phi = screenedGreensFunction({
      neighbors: chain,
      start: 4,
      mass2: 0.5,
  ...
  check('a larger mass gives stronger screening (smaller potential one step out)', () => {
    const light = screenedGreensFunction({ neighbors: chain, start: 4, mass2: 0.2, iterations: 10000 })
    const heavy = screenedGreensFunction({ neighbors: chain, start: 4, mass2: 2.0, iterations: 10000 })
    // at a fixed distance from the source the heavier mass decays faster.
    ok((heavy[6] ?? 0) / (heavy[4] ?? 1) < (light[6] ?? 0) / (light[4] ?? 1), 'heavier mass decays faster')
  }),
])

suite('operator/screened-greens-function: clamped leaky diffusion', [
  check('the stationary occupancy is a fixed point of the clamped leaky map', () => {
    ...
    for (let i = 0; i < chain.length; i++) {
      ...
      close(p[i] ?? 0, base + inflow, 1e-9, `leaky fixed point at ${i}`)
    }
  }),
  check('the occupancy is positive and largest at the source', () => {
    ...
    let argmax = 0
    for (let i = 1; i < p.length; i++) if ((p[i] ?? 0) > (p[argmax] ?? 0)) argmax = i
    equal(argmax, 4, 'peak at source')
    for (let i = 0; i < p.length; i++) ok((p[i] ?? 0) > 0, `p[${i}] positive`)
  }),
])
