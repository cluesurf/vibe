...
suite('operator/linearized-einstein: index maps', [
  check('site index and coords round-trip', () => {
    ...
    for (let i = 0; i < n; i++) {
      equal(gravitonSiteIndex(gravitonCoordsOf(i, L), L), i, `roundtrip ${i}`)
    }
  }),
])

suite('operator/linearized-einstein: lattice gauge invariance', [
  check('a pure-gauge field h = d xi + (d xi)^T is annihilated exactly', () => {
    const L = 4
    ...
    for (let s = 0; s < Math.pow(L, D); s++) {
      ...
      for (let nu = 0; nu < D; nu++) v[nu] = (amplitude[nu] ?? 0) * phase
      xi.push(v)
    }

    const centralDiff = (s: number, axis: number, fn: (site: number) => number): number => {
      const c = gravitonCoordsOf(s, L)
      const plus = gravitonSiteIndex(gravitonShift(c, axis, 1, L), L)
      const minus = gravitonSiteIndex(gravitonShift(c, axis, -1, L), L)
    ...
    ok(tensorFieldMaxAbs(h) > 0.1, 'the pure-gauge field is nontrivial')
    close(tensorFieldMaxAbs(linearizedEinstein(h)), 0, 1e-12, 'pure-gauge residual')
  }),
])

suite('operator/linearized-einstein: polarization count from spectrum', [
  check('2 physical polarizations and 4 gauge zero modes', () => {
    const result = gravitonPolarizationsFromSpectrum({ side: 4, mode: 1 })
