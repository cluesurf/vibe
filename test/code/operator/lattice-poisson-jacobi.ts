...
suite('operator/lattice-poisson-jacobi: relaxed field solves the stencil', [
  ...
  check('interior cells satisfy deg*phi - sum_neighbors = 4pi*source (fixed point)', () => {
    const source = new Float64Array(7)
    source[3] = 1
  ...
  check('a custom source coefficient scales the stencil right-hand side', () => {
    const source = new Float64Array(7)
    source[2] = 1
    source[4] = -1
    const coeff = 1.5
