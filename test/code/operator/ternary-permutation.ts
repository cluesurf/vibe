...
suite('operator/ternary-permutation: the pair update', [
  check('is a bijection on the 9 pair-states only when creating', () => {
    ...
    ok(isBijection(true), 'permutes all 9 states when creating (reversible CA mode)')
    ok(!isBijection(false), 'NOT a permutation without creation (annihilation has no inverse source)')
  }),
  ...
  check('committed table entries match the spec', () => {
    equal(ternaryPairPermutation(0, 0, true).join(','), '1,-1', 'two peaces create +,- when creating')
    equal(ternaryPairPermutation(0, 0, false).join(','), '0,0', 'two peaces hold when not creating')
    equal(ternaryPairPermutation(-1, 1, true).join(','), '0,0', '-,+ annihilates to 0,0')
    equal(ternaryPairPermutation(1, -1, true).join(','), '-1,1', '+,- swaps to -,+')
    equal(ternaryPairPermutation(1, 0, true).join(','), '0,1', 'a charge hops into the 0')
    equal(ternaryPairPermutation(1, 1, true).join(','), '1,1', 'like charges hold')
  }),
])
...
suite('operator/ternary-permutation: 3D parity-block beat', [
  check('a full beat conserves total charge', () => {
    ...
    equal(after, before, 'every pair move conserves charge, so the beat does too')
  }),
])
