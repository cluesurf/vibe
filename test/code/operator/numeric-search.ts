...
suite('operator/numeric-search: extrema', [
  check('maxIndex and minIndex match brute force, ties to lowest index', () => {
    equal(maxIndex({ field }).index, bruteMax(field), 'argmax')
    equal(maxIndex({ field }).value, 9, 'max value')
    equal(minIndex({ field }).index, 1, 'first index of the minimum (value 1)')
...
suite('operator/numeric-search: next value', [
  check('nextHigherIndex finds the smallest value strictly above the target', () => {
    const r = nextHigherIndex({ field, target: 4 })
    equal(r.value, 5, 'next value above 4 is 5')
    equal(r.index, 4, 'at index 4')
  }),
  check('nextLowerIndex finds the largest value strictly below the target', () => {
  ...
  check('returns index -1 when none qualifies', () => {
    equal(nextHigherIndex({ field, target: 9 }).index, -1, 'nothing above the max')
    equal(nextLowerIndex({ field, target: 1 }).index, -1, 'nothing below the min')
  }),
])
