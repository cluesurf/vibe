...
suite('operator/coxeter-mesh-gas: streaming', [
  check('streaming twice is the identity on an involutive adjacency', () => {
    const state = [
      [3, 5],
      [7, 9],
    ]
    const once = streamCoxeterMeshGas({ state, adjacency, rank })
    const twice = streamCoxeterMeshGas({ state: once, adjacency, rank })
    ok(stateEqual(twice, state), 'each generator is an involution, so stream^2 = id')
  }),
  check('streaming generator 0 swaps cells, generator 1 stays put', () => {
    const state = [
      [1, 2],
      [10, 20],
    ]
  ...
  check('streaming conserves total charge', () => {
    ...
    const out = streamCoxeterMeshGas({ state, adjacency, rank })
    equal(countCoxeterMeshGas(out), countCoxeterMeshGas(state), 'charge conserved')
  }),
])
...
suite('operator/coxeter-mesh-gas: collide', [
  check('collide forward then backward is the identity', () => {
    ...
    const back = collideCoxeterMeshGas({ state: fwd, rank: r, forward: false })
    ok(stateEqual(back, state), 'forward shift undone by backward shift')
  }),
  ...
  check('collide conserves total charge', () => {
    ...
    equal(countCoxeterMeshGas(out), countCoxeterMeshGas(state), 'cyclic shift conserves charge')
  }),
])

suite('operator/coxeter-mesh-gas: erase control', [
  check('erase destroys exactly the charge that lands in slot 0', () => {
    const r = 3
    const state = [
      [1, 2, 3],
      [4, 5, 6],
    ]
    const collided = collideCoxeterMeshGas({ state, rank: r, forward: true })
    const erased = eraseCoxeterMeshGas({ state, rank: r, forward: true })
