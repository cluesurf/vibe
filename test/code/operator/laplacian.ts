...
import { suite, check, equal, ok, close, exactArray, closeArray } from '@/test/code/harness'
...
const cycle4Spectrum = [0, 1, 2, 3].map(k => 2 - 2 * Math.cos((2 * Math.PI * k) / 4)).sort((a, b) => a - b)
const path5Spectrum = [0, 1, 2, 3, 4].map(k => 2 - 2 * Math.cos((Math.PI * k) / 5)).sort((a, b) => a - b)
...
suite('operator/laplacian: L = D - A structure', [
  ...
  check('L * 1 = 0: the constant vector is the zero eigenvector (exact)', () => {
    for (const g of [cycle4, path5]) {
      const L = laplacian({ substrate: g })
      ...
      for (let r = 0; r < L.rows; r++) {
        ...
        for (let p = L.rowPtr[r] ?? 0; p < (L.rowPtr[r + 1] ?? 0); p++) {
...
suite('operator/laplacian: spectrum', [
  check('the dense spectrum matches the analytic cycle eigenvalues', () => {
    const L = toDense(laplacian({ substrate: cycle4 }))
    const eig = eigSymmetric({ matrix: L })
    closeArray(eig.values, cycle4Spectrum, 1e-9, 'cycle4 spectrum')
  }),
  check('the dense spectrum matches the analytic path eigenvalues', () => {
    const L = toDense(laplacian({ substrate: path5 }))
    const eig = eigSymmetric({ matrix: L })
    closeArray(eig.values, path5Spectrum, 1e-9, 'path5 spectrum')
  }),
  check('the production Lanczos path: smallest eigenvalue is 0, all non-negative', () => {
    for (const g of [cycle4, path5]) {
      const values = laplacianSpectrum({ substrate: g, count: 3 })
      close(values[0] ?? NaN, 0, 1e-8, `smallest eigenvalue of ${g.size}-node L`)
