...
suite('operator/lattice-fermion: Clifford algebra', [
  ...
  check(
    '{gamma1, gamma2} = 0 (distinct gammas anticommute to zero)',
    () => {
  ...
  check('the Wilson projectors (I +/- gamma)/2 are idempotent', () => {
    for (const g of [PAULI_X, PAULI_Y]) {
      for (const sign of [1, -1]) {
        const p = matScaleReal(
          matAdd(IDENTITY2, matScaleReal(g, sign)),
          0.5,
        )
...
suite('operator/lattice-fermion: matrix sign', [
  check(
    'sign(H) of a traceless Hermitian H is H / |H| and squares to I',
    () => {
  ...
  check('sign(c * sigma_z) = sigma_z for c > 0', () => {
    close(
      dist(hermitianSign(matScaleReal(PAULI_Z, 3)), PAULI_Z),
      0,
      1e-12,
    )
  }),
...
suite('operator/lattice-fermion: doublers and dispersion', [
  check(
    'the naive operator has 2^d doublers and zero net chirality',
    () => {
      for (const [d, species] of [
        [1, 2],
        [2, 4],
        [3, 8],
      ] as const) {
  ...
  check(
    'the naive operator has 4 zero modes on a 4x4 grid (the doublers)',
    () => {
  ...
  check(
    'minSingularValue: zero at k=0, one at k=(pi/2, 0) for the naive operator',
    () => {
      close(
        minSingularValue(naiveDirac2D({ k1: 0, k2: 0 })),
        0,
        1e-12,
  ...
  check('1D lattice Dirac energy is sqrt(m^2 + sin^2 k)', () => {
    close(
      latticeDiracEnergy1d({ k: 0, m: 0.7 }),
      0.7,
      1e-12,
      'gap at k=0 is m',
    )
...
suite('operator/lattice-fermion: Ginsparg-Wilson', [
  check(
    'the overlap operator satisfies Ginsparg-Wilson to machine precision',
    () => {
      const points = [
        { k1: 0.5, k2: 0.7 },
        { k1: 1.3, k2: 2.1 },
      ...
      for (const k of points) {
        ...
        close(
          ...
          0,
          1e-9,
          `GW residual at (${k.k1},${k.k2})`,
        )
      }
    },
  ),
  check(
    'the naive operator does NOT satisfy Ginsparg-Wilson (residual > 0)',
    () => {
      const residual = ginspargWilsonResidual(
        naiveDirac2D({ k1: 0.5, k2: 0.7 }),
      )
  ...
  check(
    'the overlap operator has a single massless mode on a 4x4 grid, GW max ~ 0',
    () => {
      const { species, gwResidualMax } = scanBrillouin({
        operator: ({ k1, k2 }) =>
          overlapDirac2D({ k1, k2, m0: 1, r: 1 }),
        gridSize: 4,
      })
