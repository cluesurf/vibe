...
suite('operator/signed-majority: symmetric edge fills', [
  check('both half-edges of an undirected edge carry the same coupling', () => {
    const fills = symmetricEdgeFills({ neighbors: ring4, rng: makeRng({ seed: 4 }) })
    for (let v = 0; v < ring4.length; v++) {
      const row = ring4[v]!
      for (let k = 0; k < row.length; k++) {
        const w = row[k]!
        const kk = ring4[w]!.indexOf(v)
        equal(fills[v]![k], fills[w]![kk]!, `edge ${v}-${w} is symmetric`)
      }
    }
  }),
  check('every coupling is ternary', () => {
    const fills = symmetricEdgeFills({ neighbors: ring4, rng: makeRng({ seed: 99 }) })
...
suite('operator/signed-majority: synchronous step', [
  ...
  check('one step equals the sign of the hand-computed local field', () => {
    const neighbors = [[1], [0, 2], [1]]
    const fills = [Int8Array.from([1]), Int8Array.from([1, -1]), Int8Array.from([-1])]
  ...
  check('keepOnTie holds the current tone on a tied field', () => {
    ...
    const fills = [Int8Array.from([1]), Int8Array.from([1, -1]), Int8Array.from([-1])]
    const tone = Int8Array.from([1, 1, 1])
    const next = signedMajorityStep({ neighbors, fills, tone, keepOnTie: true })
...
suite('operator/signed-majority: asynchronous driver', [
  check('is deterministic in its seed', () => {
    const a = runAsynchronousSignedMajority({ neighbors: ring4, beats: 20, seed: 5 })
    const b = runAsynchronousSignedMajority({ neighbors: ring4, beats: 20, seed: 5 })
    equal(a.settledFraction, b.settledFraction, 'same seed -> same settled fraction')
    equal(a.toneHistogram.minus, b.toneHistogram.minus, 'same minus count')
  ...
  check('the histogram covers every cell and settled fraction is in [0,1]', () => {
    const r = runAsynchronousSignedMajority({ neighbors: ring4, beats: 30, seed: 8 })
    const { minus, zero, plus } = r.toneHistogram
    equal(minus + zero + plus, ring4.length, 'histogram sums to the cell count')
    ok(r.settledFraction >= 0 && r.settledFraction <= 1, 'settled fraction is a fraction')
  }),
])
