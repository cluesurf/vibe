...
const occEqual = (a: readonly number[], b: readonly number[]): boolean =>
...
suite('operator/d4-lattice-gas: streaming', [
  ...
  check('streamD4 is a permutation of the slots (count conserved)', () => {
  ...
  check('streamD4 conserves total momentum', () => {
    ...
    const m1 = d4Momentum({ occupancy: streamD4({ occupancy: occ, neigh }), roots })
    for (let q = 0; q < 4; q++) {
...
suite('operator/d4-lattice-gas: collision', [
  check('the collision involution is its own inverse', () => {
    ...
    ok(occEqual(collide(collide(occ)), occ), 'collide applied twice is the identity')
  }),
  check('collision conserves count and momentum on a seeded pair', () => {
    const collide = d4CollisionInvolution({ roots, opp })
    // Reconstruct the two swapped pair masks the operator uses.
    const k = roots.findIndex(
      (s, i) =>
        i !== 0 && i !== opp[0] && s.every((x, q) => x !== -roots[0]![q]!),
    )
    const A = (1 << 0) | (1 << opp[0]!)
    const B = (1 << k) | (1 << opp[k]!)
    ...
    for (let q = 0; q < 4; q++) {
      equal(m1[q], m0[q], `momentum component ${q} conserved (each pair has zero momentum)`)
    }
  }),
])
...
suite('operator/d4-lattice-gas: count', [
  check('d4Count equals an independent popcount', () => {
    ...
    equal(d4Count(occ), popcountTotal(occ), 'd4Count is a popcount over all slots')
  }),
])
