...
suite('operator/maxwell-lattice: gauge zero modes', [
  check('the spectrum is positive semidefinite with the right degrees of freedom', () => {
    const L = 2
    const spectrum = maxwellLatticeSpectrum({ side: L, mass: 0 })
    equal(spectrum.length, 3 * L * L * L, 'degrees of freedom = 3 L^3')
    for (const v of spectrum) ok(v > -1e-8, `eigenvalue ${v} >= 0`)
  }),
  check('the kernel dimension is L^3 + 2 = 10 (gauge modes + b1(T^3))', () => {
    const L = 2
    const spectrum = maxwellLatticeSpectrum({ side: L, mass: 0 })
...
suite('operator/maxwell-lattice: Proca mass', [
  check('a photon mass lifts every mode by m^2: no zero modes, minimum eigenvalue m^2', () => {
    const L = 2
    const mass = 0.5
    const spectrum = maxwellLatticeSpectrum({ side: L, mass })
    const zeroModes = spectrum.filter(v => Math.abs(v) < 1e-8).length
    equal(zeroModes, 0, 'no zero modes with a mass')
    close(Math.min(...spectrum), mass * mass, 1e-9, 'lifted gauge modes sit at m^2')
  }),
  check('the massive spectrum is the massless spectrum shifted up by m^2', () => {
    const L = 2
    const mass = 0.7
    ...
    for (let i = 0; i < massless.length; i++) {
      close(massive[i] ?? NaN, (massless[i] ?? 0) + mass * mass, 1e-9, `shifted eigenvalue ${i}`)
    }
  }),
])
