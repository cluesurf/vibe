...
const { regions, ground } = carveRegisters({ cells, numRegisters: 4, perRegister })

function machine(): RegisterMachine {
  return new RegisterMachine({ tone: new Int8Array(cellCount), regions, ground })
}
...
suite('operator/register-machine: layout', [
  check('carveRegisters partitions the cells exactly', () => {
    ...
    for (const region of regions) {
      equal(region.length, perRegister, 'each region has perRegister cells')
    }
    equal(ground.length, cellCount - 4 * perRegister, 'ground gets the remainder')
...
suite('operator/register-machine: conserving primitives', [
  check('INC and DEC keep total charge at zero', () => {
    ...
    m.inc(0)
    m.inc(0)
    equal(m.charge(), 0, 'two INCs still net zero (each +1 has a -1 in ground)')
...
suite('operator/register-machine: Minsky programs', [
  check('ADD leaves R0 = a + b with R1 drained, charge conserved', () => {
    for (const [a, b] of [
      [2, 3],
      [5, 0],
      [0, 4],
