...
suite('operator/ising-rg: exact decimation', [
  check('direct summation equals the closed form (1/2) ln cosh(2K)', () => {
    for (const k of [0, 0.1, 0.3, 0.5, 1.0, 1.7]) {
      ...
      close(isingDecimationFormula(k), 0.5 * Math.log(Math.cosh(2 * k)), 1e-12, `formula at K=${k}`)
    }
  }),
  check('the formula vanishes at K = 0', () => {
    close(isingDecimationFormula(0), 0, 1e-12, 'K=0 is a fixed point of decimation')
  }),
])
...
suite('operator/ising-rg: correlation', [
  check('nearestNeighborCorrelation matches a hand value', () => {
    ...
    close(nearestNeighborCorrelation(Int8Array.from([1, 1, -1, -1])), 1 / 3, 1e-12, 'hand-computed')
    close(nearestNeighborCorrelation(Int8Array.from([1, 1, 1, 1])), 1, 1e-12, 'fully aligned')
  }),
  check('a sampled chain correlation approaches tanh K', () => {
    ...
    close(acc / trials, Math.tanh(k), 0.03, 'mean correlation near tanh K (Monte Carlo)')
  }),
])

suite('operator/ising-rg: block-spin recursion', [
  check('the measured block-spin coupling obeys tanh K\' = tanh^2 K', () => {
    const k = 0.5
    const measured = measuredBlockSpinCoupling({ length: 400, coupling: k, samples: 400, seed: 9 })
    const expected = Math.atanh(Math.tanh(k) ** 2)
    close(measured, expected, 0.03, 'keep-every-other coupling matches the exact recursion (Monte Carlo)')
  }),
])
