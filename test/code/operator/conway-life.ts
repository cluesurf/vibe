...
const translate = (s: Set<string>, dx: number, dy: number): Set<string> => {
...
suite('operator/conway-life: still life and oscillator', [
  check('the 2x2 block is a fixed point', () => {
    ...
    ok(cellSetEqual(lifeStep(block), block), 'the block does not change')
  }),
  check('the blinker has period 2', () => {
    ...
    ok(cellSetEqual(lifeStep(horizontal), vertical), 'one step rotates to vertical')
    ok(cellSetEqual(steps(horizontal, 2), horizontal), 'two steps returns to horizontal')
  }),
])

suite('operator/conway-life: glider', [
  check('the glider is a rigid translation by (1,1) after 4 steps', () => {
    const glider = set([
      [1, 0],
      [2, 1],
      [0, 2],
    ...
    const dx = mx1 - mx0
    const dy = my1 - my0
    ok(cellSetEqual(after, translate(glider, dx, dy)), 'the pattern moves rigidly')
