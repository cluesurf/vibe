...
suite('operator/linearized-curvature: gauge invariance', [
  check('pure-gauge perturbations h = k xi + xi k are annihilated', () => {
    ...
    for (const { k, xi } of cases) {
      ...
      for (let i = 0; i < 3; i++) {
        for (let j = 0; j < 3; j++) {
          h[i]![j] = (k[i] ?? 0) * (xi[j] ?? 0) + (k[j] ?? 0) * (xi[i] ?? 0)
        }
      }
      close(maxAbs(linearizedEinsteinTensor(h, k)), 0, 1e-12, `gauge G for k=${k}`)
    }
  }),
])

suite('operator/linearized-curvature: transverse-traceless eigenvalue', [
  check('G[h_TT] = (1/2)|k|^2 h_TT for k along z', () => {
    ...
    for (let i = 0; i < 3; i++) {
      for (let j = 0; j < 3; j++) {
        close(G[i]![j] ?? 0, lambda * (h[i]![j] ?? 0), 1e-12, `TT(z) [${i}][${j}]`)
      }
    }
  }),
  check('G[h_TT] = (1/2)|k|^2 h_TT for k along x', () => {
    ...
    for (let i = 0; i < 3; i++) {
      for (let j = 0; j < 3; j++) {
        close(G[i]![j] ?? 0, lambda * (h[i]![j] ?? 0), 1e-12, `TT(x) [${i}][${j}]`)
      }
    }
  }),
  check('the operator is linear in h', () => {
    ...
    const sum = zero()
    for (let i = 0; i < 3; i++) for (let j = 0; j < 3; j++) sum[i]![j] = 2 * (a[i]![j] ?? 0) - 3 * (b[i]![j] ?? 0)
    ...
    for (let i = 0; i < 3; i++) {
      for (let j = 0; j < 3; j++) {
        close(Gsum[i]![j] ?? 0, 2 * (Ga[i]![j] ?? 0) - 3 * (Gb[i]![j] ?? 0), 1e-12, `linearity [${i}][${j}]`)
      }
    }
  }),
])
