...
const scalarSpec = (): number[] =>
  Array.from(
    eigSymmetric({
      matrix: scalarLandauSquared({ levels, fieldStrength: qB, mass }),
    }).values,
  ).sort((a, b) => a - b)
...
suite('operator/landau: Dirac Landau levels', [
  check(
    'the Dirac Hamiltonian is Hermitian (re symmetric, im antisymmetric)',
    () => {
      const h = diracLandauHamiltonian({
        levels,
        fieldStrength: qB,
        mass,
      })
  ...
  check(
    'eigenvalues squared are the relativistic levels m^2 + 2 qB n (with a zero mode at m^2)',
    () => {
