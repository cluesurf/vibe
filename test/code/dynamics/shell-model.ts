...
suite('dynamics/shell-model: spectrumSlope closed form', [
  check('a k^(-2/3) spectrum has log-log slope exactly -2/3', () => {
    ...
    const spectrum = Array.from({ length: 10 }, (_, i) => 5 * Math.pow(2, i) ** (-2 / 3))
    close(spectrumSlope({ spectrum, lo: 1, hi: 8 }), -2 / 3, 1e-9, 'slope = -2/3')
  }),
  check('a k^(-2) spectrum has slope -2', () => {
    const spectrum = Array.from({ length: 10 }, (_, i) => 3 * Math.pow(2, i) ** -2)
    close(spectrumSlope({ spectrum, lo: 1, hi: 8 }), -2, 1e-9, 'slope = -2')
  }),
])
...
suite('dynamics/shell-model: dynamics', [
  ...
  check('goyShellSpectrum is deterministic and finite', () => {
    const opts = { shells: 12, viscosity: 1e-7, dt: 1e-4, steps: 4000, nonlinear: true }
  ...
  check('the nonlinear cascade gives a different spectrum than the linear control', () => {
    const common = { shells: 12, viscosity: 1e-7, dt: 1e-4, steps: 4000 }
    const cascade = goyShellSpectrum({ ...common, nonlinear: true })
    const control = goyShellSpectrum({ ...common, nonlinear: false })
    let diff = 0
    for (let i = 0; i < cascade.length; i++) diff += Math.abs(cascade[i]! - control[i]!)
