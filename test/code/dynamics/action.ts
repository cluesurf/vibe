...
const antichain3 = makePosetFromRelation({ size: 3, precedes: () => false })
...
function sharp2d(poset: Poset): number {
  return benincasaDowkerAction({ epsilon: 1, dimension: 2 }).value({ poset })
}
...
suite('dynamics/action: sharp Benincasa-Dowker (2D)', [
  ...
  check('chain has strictly higher action than the V (monotone in relations)', () => {
    ok(sharp2d(chain3) > sharp2d(vPoset), 'more curvature content -> larger action')
  }),
])
...
suite('dynamics/action: smeared 2D kernel', [
  check('closed forms at epsilon 0.1', () => {
    ...
    close(smearedKernel2D({ n: 1, epsilon: e }), 1 - 3 * e, 1e-12, 'f(1) = 1 - 3e')
    close(
...
suite('dynamics/action: smeared Benincasa-Dowker action (2D)', [
  check('3-chain value matches -N/2 + eps*(2 f(0) + f(1))', () => {
    const e = 0.1
    const value = smearedBenincasaDowker({ epsilon: e, dimension: 2 }).value({ poset: chain3 })
  ...
  check('3-antichain value is -N/2 (no related pairs)', () => {
    const e = 0.25
    const value = smearedBenincasaDowker({ epsilon: e, dimension: 2 }).value({
      poset: antichain3,
    })
...
suite('dynamics/action: dimension-target action', [
  check('value is the squared deviation of the Myrheim-Meyer dimension', () => {
    ...
    close(
      dimensionTargetAction({ target: d + 2 }).value({ poset: chain3 }),
      4,
      1e-9,
