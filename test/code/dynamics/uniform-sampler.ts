...
suite('dynamics/uniform-sampler: toggle and validity', [
  ...
  check('adding a pair to an antichain keeps it valid', () => {
    const state = antichainState(4)
    ok(toggleKeepsValid(state, 0, 1, false), 'adding 0<1 to an antichain is transitive')
  }),
  check('removing a mediated relation (0<1<2, drop 0<2) is rejected', () => {
...
suite('dynamics/uniform-sampler: smeared action', [
  check('3-chain smeared action matches -N/2 + eps*(2 f(0) + f(1))', () => {
    const e = 0.1
    // f(0) = 1, f(1) = 1 - 3e (the 2D smeared kernel).
    const expected = -3 / 2 + e * (2 * 1 + (1 - 3 * e))
    close(smearedAction(chainState(3), e), expected, 1e-12, 'chain smeared action')
  }),
  check('3-antichain smeared action is -N/2', () => {
    close(smearedAction(antichainState(3), 0.25), -3 / 2, 1e-12, 'antichain = -N/2')
  }),
])
...
suite('dynamics/uniform-sampler: determinism', [
  check('two runs with the same seed give identical summaries', () => {
    ...
    equal(a.meanOrderingFraction, b.meanOrderingFraction, 'mean ordering fraction')
