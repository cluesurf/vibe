...
suite('dynamics/quantum-walk: dispersion', [
  check('omega(theta, 0) = theta (the mass gap)', () => {
    for (const theta of [0.1, 0.5, 1.0]) {
      close(coinedWalkDispersion({ theta, k: 0 }), theta, 1e-12, `gap at theta=${theta}`)
    }
  }),
  check('massless dispersion omega(0, k) = |k| (the lightcone)', () => {
    for (const k of [0.2, 0.7, 1.3]) {
      close(coinedWalkDispersion({ theta: 0, k }), Math.abs(k), 1e-12, `lightcone at k=${k}`)
    }
  }),
])

suite('dynamics/quantum-walk: continuous-time t=0 limit', [
  check('continuous quantum and classical MSD are zero at t=0 (orthonormal basis)', () => {
    ...
    close(continuousQuantumWalkMsd({ eig, n, center: 1, t: 0 }), 0, 1e-12, 'quantum MSD(0)=0')
    close(continuousClassicalWalkMsd({ eig, n, center: 1, t: 0 }), 0, 1e-12, 'classical MSD(0)=0')
  }),
])

suite('dynamics/quantum-walk: ballistic spreading', [
  check('the coined walk MSD grows super-linearly (exponent > 1.5)', () => {
    const { msd } = coinedWalkMSD({ size: 201, steps: 60, theta: Math.PI / 4 })
  ...
  check('a massless single particle drifts at ~ the light speed, a massive one slower', () => {
    const massless = singleParticleQuantumWalk({ mass: 0, momentum: 0.8, size: 240, steps: 80 })
    const massive = singleParticleQuantumWalk({ mass: 0.7, momentum: 0.8, size: 240, steps: 80 })
