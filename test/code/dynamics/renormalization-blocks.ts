...
function ring(): { g: Graph; offsets: Int32Array; adj: Int32Array } {
  const neighbors = Array.from({ length: N }, (_, i) => [(i - 1 + N) % N, (i + 1) % N])
...
suite('dynamics/renormalization-blocks: partition validity', [
  check('csrVoronoiBlocks assigns every cell a block in range', () => {
    ...
    const { blockOf, numBlocks } = csrVoronoiBlocks({ offsets, adj, size: N, targetSize: 6, rng: makeRng({ seed: 1 }) })
    for (let c = 0; c < N; c++) ok(blockOf[c]! >= 0 && blockOf[c]! < numBlocks, `cell ${c} in range`)
  }),
  check('geometricBlocks assigns every cell a block', () => {
    const { g } = ring()
    const { cl, K } = geometricBlocks(g, 6, makeRng({ seed: 2 }))
    ok(K >= 2, 'at least two blocks')
    for (let c = 0; c < N; c++) ok(cl[c]! >= 0 && cl[c]! < K, `cell ${c} assigned`)
  }),
])

suite('dynamics/renormalization-blocks: domain uniformity and fill symmetry', [
  check('domainBlocks are internally uniform in tone', () => {
    const { g } = ring()
    const tone = Int8Array.from({ length: N }, (_, i) => (Math.floor(i / 7) % 2 === 0 ? 1 : -1))
    const { cl, K } = domainBlocks(g, tone)
    const blockTone = new Map<number, number>()
    for (let c = 0; c < N; c++) {
      const b = cl[c]!
      if (blockTone.has(b)) equal(blockTone.get(b)!, tone[c]!, `block ${b} uniform`)
  ...
  check('coherentFills are symmetric across each edge', () => {
    ...
    for (let v = 0; v < N; v++) {
      ...
      for (let k = 0; k < row.length; k++) {
        ...
        equal(fills[v]![k]!, fills[w]![kk]!, `edge ${v}-${w} symmetric`)
      }
    }
  }),
])
...
suite('dynamics/renormalization-blocks: determinism', [
  check('csrVoronoiBlocks is reproducible under a fixed seed', () => {
    const { offsets, adj } = ring()
    const a = csrVoronoiBlocks({ offsets, adj, size: N, targetSize: 6, rng: makeRng({ seed: 9 }) }).blockOf
    const b = csrVoronoiBlocks({ offsets, adj, size: N, targetSize: 6, rng: makeRng({ seed: 9 }) }).blockOf
