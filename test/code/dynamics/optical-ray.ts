...
suite('dynamics/optical-ray: refractive deflection', [
  check('a uniform index (strength 0) bends nothing', () => {
    close(
      refractiveDeflection({ impactParameter: 10, strength: 0 }),
      0,
      1e-9,
      'straight ray',
    )
  ...
  check(
    'a weak well bends the ray toward the mass by ~ 2 strength / b',
    () => {
      const b = 20
      const strength = 1
  ...
  check('the deflection is deterministic', () => {
    const run = (): number =>
      refractiveDeflection({ impactParameter: 15, strength: 0.5 })
