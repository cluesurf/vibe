...
suite('dynamics/gravity-field: bulkMass', [
  check('an isolated mass is not a source', () => {
    ...
    equal(source.reduce((s, v) => s + v, 0), 0, 'lone mass sources nothing')
  }),
...
suite('dynamics/gravity-field: bounded fields', [
  check('relaxPotential stays within [-cap, cap]', () => {
    ...
    source[12] = 1
    const cap = 6
    const phi = relaxPotential({ source, ...base, sweeps: 30, strength: 3, cap })
    for (let c = 0; c < N; c++) ok(phi[c]! >= -cap && phi[c]! <= cap, `phi in range at ${c}`)
  }),
  check('vacuumDensity stays within [-cap, cap]', () => {
    ...
    for (let c = 0; c < N; c++) ok(v[c]! >= -cap && v[c]! <= cap, `v in range at ${c}`)
  }),
])
...
suite('dynamics/gravity-field: gravityMoves conserve mass', [
  check('applying the moves preserves the occupied count', () => {
    ...
    occupied[10] = 1; occupied[11] = 1; occupied[12] = 1; occupied[16] = 1
    const source = bulkMass({ occupied, ...base, minNeighbours: 2 })
    const phi = relaxPotential({ source, ...base, sweeps: 20, strength: 3, cap: 6 })
    const before = countOccupied(occupied)
    const moves = gravityMoves({ occupied, phi, ...base, minNeighbours: 2 })
...
suite('dynamics/gravity-field: determinism', [
  check('relaxPotential is reproducible', () => {
    const source = new Int8Array(N)
    source[12] = 1
    const run = (): Int32Array => relaxPotential({ source, ...base, sweeps: 15, strength: 2, cap: 6 })
    const a = run()
    const b = run()
