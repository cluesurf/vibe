...
const unitLength = (spins: Spin[]): void => {
  for (const s of spins) close(Math.hypot(s[0], s[1], s[2]), 1, 1e-9, 'unit spin')
}

suite('dynamics/skyrmion-field: normalization', [
  check('the Skyrmion seed and the snapped-to-trit field are unit spins', () => {
...
suite('dynamics/skyrmion-field: topological degree', [
  check('a uniform all-up field has degree 0', () => {
    const uniform: Spin[] = Array.from({ length: size * size }, () => [0, 0, 1] as Spin)
...
suite('dynamics/skyrmion-field: determinism', [
  check('two precession runs agree', () => {
    const run = (): Spin[] => {
      let spins = makeSkyrmionField({ size, coreRadius: 5 })
      for (let t = 0; t < 10; t++) spins = precessSpins({ spins, params, dt: 0.1, open: false })
      return spins
    }
    const a = run()
    const b = run()
