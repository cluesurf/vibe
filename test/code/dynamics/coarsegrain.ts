...
suite('dynamics/coarsegrain: keep extremes', [
  check('keepProbability 1 keeps every element', () => {
    const out = decimate({ poset: chain(20), keepProbability: 1, rng: makeRng({ seed: 1 }) })
    equal(out.size, 20, 'all kept')
  }),
  check('keepProbability 0 keeps none', () => {
    const out = decimate({ poset: chain(20), keepProbability: 0, rng: makeRng({ seed: 1 }) })
    equal(out.size, 0, 'none kept')
  }),
])

suite('dynamics/coarsegrain: induced order', [
  check('a decimated chain is still a total order on its survivors', () => {
    const out = decimate({ poset: chain(30), keepProbability: 0.5, rng: makeRng({ seed: 4 }) })
...
suite('dynamics/coarsegrain: determinism', [
  check('the same seed gives the same survivor set', () => {
    const a = decimate({ poset: chain(40), keepProbability: 0.5, rng: makeRng({ seed: 11 }) })
    const b = decimate({ poset: chain(40), keepProbability: 0.5, rng: makeRng({ seed: 11 }) })
