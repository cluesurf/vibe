...
suite('dynamics/wilson-grid: actions', [
  ...
  check('the Wilson action approaches the Maxwell action in the small-field limit', () => {
    const L = 3
    const plaqs = gridPlaquettes(L)
    const n = 3 * L * L * L
    // a deterministic field pattern, scaled small
    const make = (eps: number): Float64Array =>
      Float64Array.from({ length: n }, (_, i) => eps * Math.sin(i * 1.3 + 0.7))
    const ratio = (eps: number): number => {
      const theta = make(eps)
      return gridMaxwellAction(theta, plaqs) / gridWilsonAction(theta, plaqs)
    }
    ok(Math.abs(ratio(1e-2) - 1) < Math.abs(ratio(1e-1) - 1), 'ratio improves as the field shrinks')
