...
function selfAndDist(n: number): { inSelf: Uint8Array; distC: Int32Array } {
...
suite('dynamics/pumped-reserve-sweep: conservation', [
  check('a pure hop conserves total charge and each sign count (pump on)', () => {
    ...
    for (let b = 0; b < 40; b++) {
      pumpedReserveSweep({ tone, edges: edges(N), inSelf, distC, rng, fieldLeak: 0.2, pump: true })
  ...
  check('pump off (unbiased walk) also conserves charge', () => {
    ...
    for (let b = 0; b < 30; b++) {
      pumpedReserveSweep({ tone, edges: edges(N), inSelf, distC, rng, fieldLeak: 0.5, pump: false })
      equal(charge(tone), q0, `charge ${b}`)
    }
  }),
])
