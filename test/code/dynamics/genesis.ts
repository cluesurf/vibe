...
suite('dynamics/genesis: counts and balancing', [
  check('chargedCount counts nonzero cells', () => {
    equal(chargedCount(Int8Array.from([0, 1, -1, 0, 1])), 3, 'three nonzero')
  }),
  check('balanceToZero forces total charge to 0', () => {
    const tone = makeTone(30)
    equal(totalCharge(balanceToZero(tone)), 0, 'net charge zero after balancing')
  }),
])
...
suite('dynamics/genesis: charge conservation', [
  ...
  check('growingMeshGenesis conserves charge from the void (Q = 0)', () => {
    const N = 24
    const depth = Int32Array.from({ length: N }, (_, i) => Math.abs(i - Math.floor(N / 2)))
    const out = growingMeshGenesis({ neighbors: ringNeighbors(N), depth })
...
suite('dynamics/genesis: first distinction', [
  check('the first creation from the void is a balanced, adjacent pair', () => {
    const N = 30
...
suite('dynamics/genesis: determinism', [
  check('two seeded trajectories agree', () => {
    const N = 30
    const run = (): number[] =>
      chargeTrajectory({ neighbors: ringNeighbors(N), initial: makeTone(N), beats: 20, arrow: 0.3, seed: 42 }).trajectory
    const a = run()
    const b = run()
    for (let i = 0; i < a.length; i++) equal(a[i]!, b[i]!, `trajectory ${i}`)
  }),
])
