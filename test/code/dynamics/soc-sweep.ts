...
suite('dynamics/soc-sweep: localActivity', [
  check('localActivity is the nonzero fraction, 0 for empty and 1 for full', () => {
    const empty = new Int8Array(N)
    ok(localActivity(empty, offsets, adj, 0, 1) === 0, 'empty neighbourhood = 0')
    const full = new Int8Array(N).fill(1)
    equal(localActivity(full, offsets, adj, 0, 1), 1, 'full neighbourhood = 1')
  }),
])
...
suite('dynamics/soc-sweep: conservation', [
  check('total charge is conserved (feedback creation)', () => {
    ...
    for (let b = 0; b < 40; b++) {
      socEdgeSweep({ tone, offsets, adj, eu, ev, moved, rng, arrow: 0.3, uniform: false })
  ...
  check('uniform control also conserves charge', () => {
    ...
    for (let b = 0; b < 30; b++) {
      socEdgeSweep({ tone, offsets, adj, eu, ev, moved, rng, arrow: 0.3, uniform: true })
