...
suite('dynamics/morphogenesis: Turing pattern formation', [
  check('the long-range rule forms a stable, regular, balanced striped pattern', () => {
    const coarse = morphogenesis({ n, activateRadius: 2, inhibitRadius: 10, inhibition: 1, beats: 200 })
  ...
  check('a longer inhibition range selects a coarser wavelength', () => {
    const coarse = morphogenesis({ n, activateRadius: 2, inhibitRadius: 10, inhibition: 1, beats: 200 })
    const fine = morphogenesis({ n, activateRadius: 1, inhibitRadius: 4, inhibition: 1, beats: 200 })
    const coarseWavelength = n / Math.max(1, coarse.walls)
    const fineWavelength = n / Math.max(1, fine.walls)
    ok(coarseWavelength > fineWavelength * 1.5, 'longer range -> coarser stripes')
  }),
])

suite('dynamics/morphogenesis: determinism', [
  check('two runs agree', () => {
    const a = morphogenesis({ n, activateRadius: 2, inhibitRadius: 10, inhibition: 1, beats: 100 })
    const b = morphogenesis({ n, activateRadius: 2, inhibitRadius: 10, inhibition: 1, beats: 100 })
