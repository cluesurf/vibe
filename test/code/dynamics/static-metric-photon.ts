...
suite('dynamics/static-metric-photon: bootstrap fixed point', [
  check('B_{n+1} = 1 + x B_n converges to 1/(1 - x)', () => {
    for (const x of [0.1, 0.3, 0.5]) {
      ...
      close(trail[trail.length - 1]!, 1 / (1 - x), 1e-9, `fixed point at x=${x}`)
    }
  }),
  check('the first iterate is 1 + x (one order of curvature energy)', () => {
...
suite('dynamics/static-metric-photon: factor-of-two bending', [
  check('the full spatial metric roughly doubles the time-only deflection', () => {
    const rs = 1
    const b = 100
    const full = staticMetricPhotonDeflection({ schwarzschildRadius: rs, impactParameter: b, spatialMetric: 'full' })
    const flat = staticMetricPhotonDeflection({ schwarzschildRadius: rs, impactParameter: b, spatialMetric: 'flat' })
