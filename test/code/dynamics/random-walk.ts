...
suite('dynamics/random-walk: diffusive baseline', [
  check('MSD starts at 0 and grows roughly linearly (exponent ~ 1)', () => {
  ...
  check('a line-graph walk is diffusive (exponent < 1.5, well below ballistic)', () => {
    const exp = graphWalkMsdExponent({ neighbors: ringNeighbors(401), start: 200, beats: 120, runs: 200 })
...
suite('dynamics/random-walk: determinism', [
  check('the same seed gives the same endpoint and path', () => {
    ...
    const e1 = randomWalkEndpoint({ neighbors, start: 0, steps: 100, rng: makeRng({ seed: 7 }) })
    const e2 = randomWalkEndpoint({ neighbors, start: 0, steps: 100, rng: makeRng({ seed: 7 }) })
    equal(e1, e2, 'endpoint reproducible')
    const p1 = randomWalkPath({ neighbors, start: 0, steps: 30, rng: makeRng({ seed: 9 }) })
    const p2 = randomWalkPath({ neighbors, start: 0, steps: 30, rng: makeRng({ seed: 9 }) })
    equal(p1.length, 31, 'path includes the start')
    for (let i = 0; i < p1.length; i++) equal(p1[i]!, p2[i]!, `path step ${i}`)
  }),
  check('classicalWalkMSD is seed-deterministic (fixed internal seeding)', () => {
...
suite('dynamics/random-walk: persistent walk limits', [
  check('mix = 0 is ballistic: displacement equals the number of steps', () => {
    const directions = [[1, 0], [-1, 0], [0, 1], [0, -1]]
    const d = persistentWalkMeanDisplacement({
      directions, mix: 0, steps: 50, runs: 8, rng: makeRng({ seed: 3 }),
    })
    close(d, 50, 1e-9, 'straight line of unit steps')
  }),
  check('mix = 1 (memoryless) is much shorter than the ballistic line', () => {
    const directions = [[1, 0], [-1, 0], [0, 1], [0, -1]]
    const d = persistentWalkMeanDisplacement({
      directions, mix: 1, steps: 400, runs: 200, rng: makeRng({ seed: 5 }),
    })
  ...
  check('persistentWalk is deterministic under a fixed seed', () => {
    const directions = [[1, 0], [-1, 0], [0, 1], [0, -1]]
    const run = (): number =>
      persistentWalkMeanDisplacement({ directions, mix: 0.5, steps: 80, runs: 20, rng: makeRng({ seed: 11 }) })
