...
suite('dynamics/swerve-walk: determinism', [
  check('the same point set gives an identical trace', () => {
    const a = swerveWalk({ points: points(), steps: 6, tauLo: 0.5, tauHi: 1.5 })
    const b = swerveWalk({ points: points(), steps: 6, tauLo: 0.5, tauHi: 1.5 })
...
suite('dynamics/swerve-walk: proper time and the straight limit', [
  check('proper time strictly increases along the trace', () => {
    const trace = swerveWalk({ points: points(), steps: 6, tauLo: 0.5, tauHi: 1.5 })
    ok(trace.length > 1, 'walk took several steps')
    for (let i = 1; i < trace.length; i++) {
      ok(trace[i]!.tau > trace[i - 1]!.tau, `tau increases at ${i}`)
    }
  }),
  check('a straight-ahead element keeps the rapidity at 0 (no swerve)', () => {
    const trace = swerveWalk({ points: points(), steps: 6, tauLo: 0.5, tauHi: 1.5 })
    for (const s of trace) {
      close(s.rapidity, 0, 1e-12, 'rapidity stays 0 on the straight column')
    }
  }),
])
