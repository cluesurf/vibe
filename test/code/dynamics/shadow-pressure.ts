...
suite('dynamics/shadow-pressure: D4 vacuum control and attraction', [
  check('the no-body control is exactly zero (root symmetry)', () => {
    close(shadowPressureD4({ ...base, body: false }), 0, 1e-12, 'symmetric vacuum cancels')
  }),
  check('a body makes the net momentum point toward it (attraction)', () => {
    ok(shadowPressureD4({ ...base, body: true }) < 0, 'net x-momentum is inward (negative)')
  }),
  check('the self-contained (self-generated vacuum) control is also ~ 0', () => {
    close(selfContainedShadowD4({ ...base, body: false }), 0, 1e-9, 'symmetric self-vacuum cancels')
  }),
])

suite('dynamics/shadow-pressure: 1D run', [
  check('a body shadows the +x flux, pushing the mass toward it', () => {
    const out = shadowPressureRun({
      length: 60, massStart: 40, bodyLo: 5, bodyHi: 12, beats: 200, threshold: 30, body: true,
    })
    ok(out.netMomentum < 0, 'net momentum toward the body')
    ok(out.hitsLeft > out.hitsRight, 'more hits from the open side than the shadowed side')
  }),
  check('shadowPressureD4 is deterministic', () => {
    equal(shadowPressureD4({ ...base, body: true }), shadowPressureD4({ ...base, body: true }), 'reproducible')
  }),
])
