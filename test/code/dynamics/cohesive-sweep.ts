...
import { cohesiveEdgeSweep, agreeCount } from '@/code/dynamics/cohesive-sweep'
...
suite('dynamics/cohesive-sweep: agreeCount', [
  check('agreeCount counts same-tone neighbours, skipping the partner', () => {
    ...
    equal(agreeCount(tone, small.offsets, small.adj, 0, 1, 1), 1, 'excludes the partner')
    // company of node 0 with tone 1, excluding nothing real (except = -1): both neighbours
    equal(agreeCount(tone, small.offsets, small.adj, 0, 1, -1), 2, 'both neighbours agree')
  }),
])

suite('dynamics/cohesive-sweep: conservation', [
  check('total charge is conserved (annihilate + create + cohesive hop)', () => {
    const tone = makeTone(N)
    const q0 = charge(tone)
    const moved = new Uint8Array(N)
    const rng = makeRng({ seed: 1 })
    for (let b = 0; b < 40; b++) {
      cohesiveEdgeSweep({ tone, eu, ev, offsets, adj, moved, rng, annihilate: true, arrow: 0.2 })
      equal(charge(tone), q0, `charge at beat ${b}`)
    }
  }),
  check('annihilate off, arrow 0: a pure conserved hop preserves each sign count', () => {
    const tone = makeTone(N)
