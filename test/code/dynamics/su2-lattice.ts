...
const det = (x: M): C => csub(cmul(x[0][0], x[1][1]), cmul(x[0][1], x[1][0]))
...
suite('dynamics/su2-lattice: cold-lattice observables', [
  check('average plaquette of an all-identity lattice is 1', () => {
    const lattice = makeSu2Lattice({ dim: 3, length: 2, hot: false, rng: makeRng({ seed: 1 }) })
    close(averagePlaquette({ lattice }), 1, TOL, 'cold plaquette = 1')
  }),
  check('Wilson loops of a cold lattice are 1', () => {
    const lattice = makeSu2Lattice({ dim: 3, length: 3, hot: false, rng: makeRng({ seed: 1 }) })
  ...
  check('Creutz ratio of a cold lattice is 0', () => {
    const lattice = makeSu2Lattice({ dim: 3, length: 3, hot: false, rng: makeRng({ seed: 1 }) })
    close(creutzRatio({ lattice, r: 2, t: 2 }), 0, TOL, 'cold Creutz ratio = 0')
  }),
])

suite('dynamics/su2-lattice: links and determinism', [
  check('a hot lattice has unit-norm links', () => {
    const lattice = makeSu2Lattice({ dim: 2, length: 3, hot: true, rng: makeRng({ seed: 7 }) })
  ...
  check('a Metropolis sweep is deterministic under a fixed seed', () => {
    const make = (): ReturnType<typeof makeSu2Lattice> =>
      makeSu2Lattice({ dim: 3, length: 2, hot: false, rng: makeRng({ seed: 11 }) })
    const a = make()
    const b = make()
    const accA = metropolisSweep({ lattice: a, beta: 2.3, eps: 0.3, rng: makeRng({ seed: 99 }) })
    const accB = metropolisSweep({ lattice: b, beta: 2.3, eps: 0.3, rng: makeRng({ seed: 99 }) })
    equal(accA, accB, 'acceptance must match across identical seeds')
    exactArray(a.links, b.links, 'link buffers must match bit-for-bit')
  }),
])
