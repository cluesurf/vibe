...
import { transitiveClosure, sampleCausalSets } from '@/code/dynamics/mcmc'
...
suite('dynamics/mcmc: transitive closure', [
  check('0->1 and 1->2 close to include 0->2', () => {
    ...
    ok(getBit(closed, { row: 0, col: 2 }), '0->2 inferred by transitivity')
    ok(!getBit(closed, { row: 1, col: 0 }), 'no spurious reverse relation')
  }),
])

suite('dynamics/mcmc: acceptance at beta = 0', [
  check('beta=0 accepts at least as often as a large beta, and is positive', () => {
...
suite('dynamics/mcmc: determinism', [
  check('two runs with the same seed agree exactly', () => {
    ...
    exactArray(a.trace, b.trace, 'the observable trace must match bit-for-bit')
  }),
])
