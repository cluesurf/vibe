...
suite('dynamics/ginzburg-landau: field construction', [
  ...
  check('uniform field has zero Dirichlet energy; a winding field has positive energy', () => {
    const uniform: Complex2[] = Array.from({ length: 16 }, () => ({ re: 1, im: 0 }))
    close(ringFieldEnergy(uniform), 0, 1e-12, 'uniform energy = 0')
    ok(ringFieldEnergy(ringFieldWithWinding(16, 1)) > 0, 'winding energy > 0')
  }),
])

suite('dynamics/ginzburg-landau: gradient flow', [
  check('relaxation lowers the energy of a defect/anti-defect pair', () => {
  ...
  check('energy is monotone non-increasing along the flow', () => {
    ...
    relaxRingField({
      field, steps: 400, dt: 0.05, sampleEvery: 50,
      onSample: f => samples.push(ringFieldEnergy(f)),
    })
    for (let i = 1; i < samples.length; i++) {
      ok(samples[i]! <= samples[i - 1]! + 1e-9, `non-increasing at ${i}`)
    }
  }),
])
...
suite('dynamics/ginzburg-landau: determinism', [
  check('two relaxations agree', () => {
    const a = relaxRingField({ field: ringDefectPair(32), steps: 100, dt: 0.05 })
    const b = relaxRingField({ field: ringDefectPair(32), steps: 100, dt: 0.05 })
