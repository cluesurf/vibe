...
suite('dynamics/wave-field: exact integer reversibility', [
  check('one beat then the reversed beat recovers the previous slice (zero force)', () => {
    const field = makeWaveField({ size: SIZE, fill })
    const prev0 = field.prev.slice()
    const curr0 = field.curr.slice()
    const after = stepWaveField({ field, accel: zeroAccel, boundary: periodic })
    ...
    const reversed: WaveField = { prev: after.curr, curr: after.prev, size: SIZE }
    const back = stepWaveField({ field: reversed, accel: zeroAccel, boundary: periodic })
  ...
  check('reversibility holds for a NONLINEAR accel too', () => {
    ...
    const reversed: WaveField = { prev: after.curr, curr: after.prev, size: SIZE }
    const back = stepWaveField({ field: reversed, accel, boundary: periodic })
    exactArray(back.curr, prev0, 'recovered with nonlinear force')
  }),
])

suite('dynamics/wave-field: fixed point and determinism', [
  check('a constant field with accel(c)=0 is a fixed point (periodic)', () => {
    const c = 4
    ...
    const after = stepWaveField({ field, accel: zeroAccel, boundary: periodic })
    exactArray(after.curr, new Int32Array(SIZE).fill(c), 'constant field unchanged')
  }),
  check('two identical beats are bit-for-bit equal', () => {
    const a = stepWaveField({ field: makeWaveField({ size: SIZE, fill }), accel: zeroAccel, boundary: periodic })
    const b = stepWaveField({ field: makeWaveField({ size: SIZE, fill }), accel: zeroAccel, boundary: periodic })
    exactArray(a.curr, b.curr, 'deterministic')
  }),
])

suite('dynamics/wave-field: accel and measures', [
  check('doubleWellAccel: zeros at 0 and +/-amplitude, pushes toward the vacua', () => {
  ...
  check('non-saturating accel pulls back linearly beyond a vacuum', () => {
    const accel = doubleWellAccel({ amplitude: 5, saturating: false })
    equal(accel(8), -3, 'pull-back = -(magnitude - amplitude)')
  }),
  check('fieldMaxAbs and domainWallCount read the configuration', () => {
