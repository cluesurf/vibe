...
import { suite, check, close, closeArray, equal } from '@/test/code/harness'
import {
  leapfrogWaveStep,
  blockAverage,
  evolveLeapfrogWave,
...
suite('dynamics/leapfrog-wave: reversibility', [
  ...
  check('a multi-step forward then backward round trip recovers the start', () => {
    const L = 24
    const r2 = 0.3
    const steps = 20
...
suite('dynamics/leapfrog-wave: known limits and helpers', [
  check('a flat (constant) field is a fixed point: no gradient, no motion', () => {
    const L = 10
    const c = 3.5
  ...
  check('blockAverage is the exact mean over consecutive blocks', () => {
    const u = Float64Array.from([1, 3, 10, 20, 100, 200])
    const out = blockAverage(u, 2)
    closeArray(out, Float64Array.from([2, 15, 150]), TOL, 'block means')
    equal(out.length, 3, 'one coarse cell per block')
  }),
  check('evolveLeapfrogWave is deterministic', () => {
    const L = 16
    const run = (): Float64Array =>
      evolveLeapfrogWave({ u: sineSlice(L, 0), uPrev: sineSlice(L, -0.1), r2: 0.25, steps: 30 })
    closeArray(run(), run(), 0, 'identical inputs give identical output')
  }),
])
