...
suite('dynamics/coined-walk-distribution: conservation', [
  check('the quantum distribution carries unit norm (unitarity)', () => {
  ...
  check('the classical distribution sums to 1', () => {
    for (const steps of [5, 20, 50]) {
      const { distribution } = coinedWalkClassicalDistribution({ steps })
      let sum = 0
...
suite('dynamics/coined-walk-distribution: layout and symmetry', [
  ...
  check('the symmetric seed gives a left-right symmetric distribution', () => {
    const { distribution, offset } = coinedWalkQuantumDistribution({ steps: 12 })
    for (let d = 1; d <= 12; d++) {
      close(distribution[offset + d]!, distribution[offset - d]!, 1e-9, `symmetry at d=${d}`)
    }
  }),
  check('the classical distribution is also symmetric', () => {
    const { distribution, offset } = coinedWalkClassicalDistribution({ steps: 12 })
    for (let d = 1; d <= 12; d++) {
      close(distribution[offset + d]!, distribution[offset - d]!, 1e-12, `classical symmetry at d=${d}`)
    }
  }),
])
