...
suite('geometry/tree-embedding: the complete tree', [
  ...
  check('every internal node has exactly b children and there are b^d leaves', () => {
    const b = 2
    const d = 3
    const tree = completeTree({ branching: b, depth: d })
    let leaves = 0
...
suite('geometry/tree-embedding: hop distance', [
  check('tree distances match the hand-computed values on a depth-2 binary tree', () => {
    ...
    equal(treeDistance(tree.parent, depth, 1, 2), 2, 'level-1 siblings')
    equal(treeDistance(tree.parent, depth, 0, 5), 2, 'root to leaf')
    equal(treeDistance(tree.parent, depth, 3, 5), 4, 'cross-subtree leaves')
    equal(treeDistance(tree.parent, depth, 3, 6), 4, 'cross-subtree leaves')
  }),
])

suite('geometry/tree-embedding: the isometric placement', [
  check('every tree edge has exactly the requested hyperbolic length', () => {
  ...
  check('hyperbolicDistance is symmetric and zero on the diagonal', () => {
...
suite('geometry/tree-embedding: distortion bound', [
  ...
  check('the Euclidean plane distorts the tree more than hyperbolic space', () => {
    const tree = completeTree({ branching: 3, depth: 3 })
    const hyp = embedTree({
      parent: tree.parent,
      children: tree.children,
