...
import { suite, check, equal, ok, close, allFinite } from '@/test/code/harness'
...
suite('geometry/packing: the 24-cell is the optimal 4D kissing shell', [
  check('the minimum pairwise angle is 60 degrees (max cosine 1/2)', () => {
    ...
    close(maxPairwiseCosine(D4), 0.5, 1e-12, 'closest pair subtends exactly 60 degrees')
  }),
  check('the 24 roots are a kissing configuration at 60 degrees', () => {
    ok(isKissingConfiguration(D4, 60), 'every pair at least 60 degrees apart')
  }),
  check('a too-close pair is NOT a kissing configuration', () => {
    ...
    ok(!isKissingConfiguration(close45, 60), '45 degrees is below the floor')
  }),
  check('the 24-cell is 8-regular at the minimum angle', () => {
    ...
    equal(histogram[8], 24, 'all 24 roots have 8 neighbours at 60 degrees')
    equal(Object.keys(histogram).length, 1, 'no other coordination number appears')
  }),
])

suite('geometry/packing: kissing maximality', [
  check('no D4 root can be re-added (the 24-cell is maximal at 60 degrees)', () => {
    ok(
      !canExtendKissing(D4, D4, 60),
      'every candidate is already present or too close',
    )
  }),
  check('an orthogonal direction CAN extend a sparse configuration', () => {
    ok(
...
suite('geometry/packing: deterministic spiral and Riesz relaxation', [
  check('the spiral returns the requested count of finite unit vectors', () => {
  ...
  check('honest negative: a generic start does NOT relax to the 24-cell', () => {
    const start = deterministicSpiral(24, 4)
    const relaxed = relaxRiesz(start, { steps: 400 })

    for (const p of relaxed) {
      close(Math.hypot(...p), 1, 1e-9, 'relaxed points stay on the sphere')
    }
