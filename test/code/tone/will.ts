...
suite('tool/tone: will storage and charge', [
  ...
  check('charge equals the independent sum over the fill formula', () => {
  ...
  check('phase shifts the fill deterministically', () => {
    ...
    equal(charge(will), expected, 'phased charge matches the phased formula')
    // every slot in {-1,0,1}
    for (let i = 0; i < will.data.length; i++) {
      const v = will.data[i]!
...
suite('tool/tone: structured fills', [
  check('loneParticle places exactly one charge of the given tone', () => {
    const mesh = squareMesh({ side: 4 })
  ...
  check('gliderLine of length L carries charge tone * L on distinct cells', () => {
