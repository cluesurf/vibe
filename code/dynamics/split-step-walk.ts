...
function coinRotate(up: Complex, down: Complex, theta: number): [Complex, Complex] {
  ...
  const newUp: Complex = [c * up[0] - s * down[0], c * up[1] - s * down[1]]
  const newDown: Complex = [s * up[0] + c * down[0], s * up[1] + c * down[1]]
...
export function meanChiralDisplacement(input: {
  ...
  for (let t = 0; t < steps; t++) {
    ...
    let chiral = 0
    for (let x = 0; x < L; x++) {
      const sigmaX = 2 * (up[x]![0] * down[x]![0] + up[x]![1] * down[x]![1])
      chiral += (x - x0) * sigmaX
    }
