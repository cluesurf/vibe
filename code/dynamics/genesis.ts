...
export function balanceToZero(tone: Int8Array): Int8Array {
  ...
  for (let i = 0; i < tone.length && q !== 0; i++) {
    if (q > 0 && tone[i] === 1) { tone[i] = 0; q-- }
    else if (q < 0 && tone[i] === -1) { tone[i] = 0; q++ }
  }
  return tone
}
...
export function differenceTrajectory(input: {
  ...
  for (let t = 0; t < beats; t++) {
    conservingEdgeListSweepPumped({ tone: a, edges, moved: movedA, rng: rngA, arrow, pump: null })
    conservingEdgeListSweepPumped({ tone: b, edges, moved: movedB, rng: rngB, arrow, pump: null })
    difference.push(diff())
  }
  return { difference, startDiff: difference[0]!, endDiff: difference[difference.length - 1]! }
}
...
export function growingMeshGenesis(input: {
...
}): { trajectory: number[]; qStart: number; qEnd: number; conserved: boolean; maxDepth: number; bornEnd: number } {
  ...
  const step = (b: number, frontier: number): void => {
    ...
    for (const [v, w] of edges) {
      ...
      const c = tone[w]!
      if ((a === 1 && c === -1) || (a === -1 && c === 1)) {
        tone[v] = 0; tone[w] = 0; moved[v] = 1; moved[w] = 1
      } else if (frontier >= 0 && a === 0 && c === 0 && ((dv === frontier && dw === frontier - 1) || (dw === frontier && dv === frontier - 1))) {
        const outer = dv > dw ? v : w
        const inner = outer === v ? w : v
        tone[outer] = 1; tone[inner] = -1; moved[v] = 1; moved[w] = 1
      } else if ((a === 0) !== (c === 0)) {
        const charged = a === 0 ? w : v
        const empty = a === 0 ? v : w
        const doHop = integerHop ? ((v + w + b) & 1) === 0 : hashRand(v * 131071 + w, b, 1) < 0.5
        if (doHop) { tone[empty] = tone[charged]!; tone[charged] = 0; moved[v] = 1; moved[w] = 1 }
      }
    }
  }
  for (let b = 1; b <= maxDepth; b++) {
    ...
    let born = 0, alive = 0
    for (let i = 0; i < n; i++) if (depth[i]! <= b) { born++; if (tone[i] !== 0) alive++ }
    trajectory.push(alive)
  }
  let bornEnd = 0
  for (let i = 0; i < n; i++) if (depth[i]! <= maxDepth) bornEnd++
  for (let s = 0; s < settleBeats; s++) { step(maxDepth, -1); trajectory.push(chargedCount(tone)) } // settle, no new creation
  const qEnd = totalCharge(tone)
  return { trajectory, qStart, qEnd, conserved: qStart === qEnd, maxDepth, bornEnd }
}
...
export function oneBeat(input: { tone: Int8Array; edges: ReadonlyArray<readonly [number, number]>; arrow: number; seed: number }): Int8Array {
  const out = input.tone.slice()
  const moved = new Uint8Array(out.length)
  conservingEdgeListSweepPumped({ tone: out, edges: input.edges, moved, rng: makeRng({ seed: input.seed }), arrow: input.arrow, pump: null })
  return out
}
...
export function gardenOfEdenFraction(input: {
  ...
  arrow: number
  seed: number
}): { states: number; reachable: number; goeFraction: number; injectivity: number } {
  const { neighbors, cells, arrow, seed } = input
  const edges = edgesOf(neighbors)
  const total = 3 ** cells
  const image = new Set<number>()
  const tone = new Int8Array(cells)
  const code = (t: Int8Array): number => { let k = 0; for (let i = cells - 1; i >= 0; i--) k = k * 3 + (t[i]! + 1); return k }
  for (let s = 0; s < total; s++) {
    let x = s
    for (let i = 0; i < cells; i++) { tone[i] = (x % 3) - 1; x = (x / 3) | 0 }
    image.add(code(oneBeat({ tone, edges, arrow, seed })))
  }
  const reachable = image.size
  return { states: total, reachable, goeFraction: (total - reachable) / total, injectivity: reachable / total }
}
...
export function attractorSignature(input: { tone: Int8Array; neighbors: ReadonlyArray<ReadonlyArray<number>> }): {
  density: number
  netBalance: number
  ...
  let charged = 0
  let net = 0
  for (let i = 0; i < n; i++) { if (tone[i] !== 0) charged++; net += tone[i]! }
  let pairs = 0
  let same = 0
  for (let v = 0; v < n; v++) for (const w of neighbors[v]!) if (w > v && tone[v] !== 0 && tone[w] !== 0) { pairs++; if (tone[v] === tone[w]) same++ }
  return { density: charged / n, netBalance: net / n, sameSignFraction: pairs > 0 ? same / pairs : 0 }
}
