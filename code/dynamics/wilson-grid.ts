...
function plaquetteCurl(theta: Float64Array, plaq: PlaquetteLink[]): number {
  let f = 0
...
export function gridWilsonAction(theta: Float64Array, plaqs: PlaquetteLink[][]): number {
  let s = 0
  for (const plaq of plaqs) s += 1 - Math.cos(plaquetteCurl(theta, plaq))
  return s
}

// The Maxwell action sum over plaquettes of (1/2) F^2, the small-field limit of the Wilson action.
export function gridMaxwellAction(theta: Float64Array, plaqs: PlaquetteLink[][]): number {
  let s = 0
