...
export function phaseRelaxStep(theta: ReadonlyArray<number>, alpha: number): number[] {
  const n = theta.length
  const next = new Array<number>(n)
  for (let i = 0; i < n; i++) {
    const left = theta[(i - 1 + n) % n]!
    const right = theta[(i + 1) % n]!
    next[i] = wrapAngle(theta[i]! + alpha * (Math.sin(left - theta[i]!) + Math.sin(right - theta[i]!)))
  }
  return next
}
...
export function phaseWaveStep(theta: number[], velocity: number[], dt: number): void {
  const n = theta.length
  const acceleration = new Array<number>(n)
  for (let i = 0; i < n; i++) {
    acceleration[i] = Math.sin(theta[(i + 1) % n]! - theta[i]!) + Math.sin(theta[(i - 1 + n) % n]! - theta[i]!)
  }
  for (let i = 0; i < n; i++) {
...
export function gradientStructure(theta: ReadonlyArray<number>): number {
  const n = theta.length
  const gradient = new Array<number>(n)
  for (let i = 0; i < n; i++) gradient[i] = Math.sin(theta[(i + 1) % n]! - theta[i]!)
  const mean = gradient.reduce((sum, value) => sum + value, 0) / n
  return Math.sqrt(gradient.reduce((sum, value) => sum + (value - mean) ** 2, 0) / n)
}

// a winding-w kink with a localized lump perturbation at the centre, a deterministic non-equilibrium initial field.
export function windingKinkWithLump(input: { size: number; winding: number; lumpAmplitude: number; lumpWidth?: number }): number[] {
  ...
  for (let i = 0; i < size; i++) {
    ...
    const lump = lumpAmplitude * Math.exp(-(((i - size / 2) / width) ** 2))
