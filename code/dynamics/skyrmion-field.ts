...
const at = (size: number, x: number, y: number): number =>
...
function pinEdge(out: Spin[], size: number): void {
  for (let i = 0; i < size; i++) {
    out[at(size, i, 0)] = [0, 0, 1]
    out[at(size, i, size - 1)] = [0, 0, 1]
    out[at(size, 0, i)] = [0, 0, 1]
    out[at(size, size - 1, i)] = [0, 0, 1]
  }
}

// one DAMPED relaxation step (gradient descent on energy), finds the metastable bound Skyrmion.
export function relaxSpins(input: {
  spins: Spin[]
  params: SkyrmionParams
  rate: number
}): Spin[] {
...
export function precessSpins(input: {
  spins: Spin[]
  params: SkyrmionParams
  dt: number
  open: boolean
...
export function makeSkyrmionField(input: {
  size: number
  coreRadius: number
}): Spin[] {
