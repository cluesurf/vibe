...
export function makeGridGrid(side: number): number[][] {
  return Array.from({ length: side }, () => new Array<number>(side).fill(0))
}

// The plaquette flux (curl A) at the cell with lower-left corner (x, y), on a torus of the given side.
export function plaquetteFlux(g: GridGauge, input: { x: number; y: number; side: number }): number {
  const { x, y, side } = input
  return g.Ax[x]![y]! + g.Ay[(x + 1) % side]![y]! - g.Ax[x]![(y + 1) % side]! - g.Ay[x]![y]!
}

// The Wilson loop (sum of A around the rectangle [x0,x1] x [y0,y1]) = the enclosed flux.
export function gridWilsonLoop(g: GridGauge, input: { x0: number; x1: number; y0: number; y1: number }): number {
...
export function gridGaugeTransform(g: GridGauge, lambda: number[][], side: number): GridGauge {
  ...
  for (let x = 0; x < side; x++) {
    for (let y = 0; y < side; y++) {
      Ax[x]![y] = g.Ax[x]![y]! + lambda[(x + 1) % side]![y]! - lambda[x]![y]!
      Ay[x]![y] = g.Ay[x]![y]! + lambda[x]![(y + 1) % side]! - lambda[x]![y]!
    }
  }
  return { Ax, Ay }
}
