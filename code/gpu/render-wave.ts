...
import { WAVE_STEP_WGSL, WAVE_RENDER_WGSL } from '~/gpu/wave.wgsl'
...
async function run(): Promise<void> {
  ...
  device.queue.writeBuffer(params, 0, new Uint32Array([SIZE, SIZE, count, 0]))
  ...
  const renderModule = device.createShaderModule({ code: WAVE_RENDER_WGSL })
  ...
  const pixelBuffer = device.createBuffer({ size: bytesPerRow * SIZE, usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ })
  renderEnc.copyTextureToBuffer({ texture: target }, { buffer: pixelBuffer, bytesPerRow, rowsPerImage: SIZE }, { width: SIZE, height: SIZE })
  ...
  console.log(`rendered ${SIZE}x${SIZE} field after ${BEATS} beats, wrote ${outPath}`)
}
