...
async function run(): Promise<void> {
  ...
  const px = new Int32Array(B), py = new Int32Array(B)
  for (let a = 0; a < B; a++) { const i = bandList[a]!; px[a] = Math.round(IMG / 2 + ((U[i]! - cu) / ext) * halfPx); py[a] = Math.round(IMG / 2 + ((V[i]! - cv) / ext) * halfPx) }
  ...
  const offBuf = device.createBuffer({ size: offsets.byteLength, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST }); device.queue.writeBuffer(offBuf, 0, offsets)
  const adjBuf = device.createBuffer({ size: adj.byteLength, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST }); device.queue.writeBuffer(adjBuf, 0, adj)
  const pipeline = device.createComputePipeline({ layout: 'auto', compute: { module: device.createShaderModule({ code: BULK_STEP_WGSL }), entryPoint: 'main' } })
  ...
  for (let f = 0; f < FRAMES; f++) {
    ...
    for (let a = 0; a < B; a++) { const t = signedTone(currentOf(tones[bandList[a]!]!)); sm[a] = t; if (reached[a]! < 0 && t !== 0) reached[a] = f }
    for (let p = 0; p < SMOOTH_PASSES; p++) {
      const ns = new Float32Array(B)
      for (let a = 0; a < B; a++) { let s = sm[a]!, d = 1; for (const b of bandNbr[a]!) { s += sm[b]!; d++ } ns[a] = s / d }
      sm = ns
    }
    let mx = 1e-6
    for (let a = 0; a < B; a++) { const v = Math.abs(sm[a]!); if (v > mx) mx = v }
    const eps = 0.06 * mx
    ...
    writeFileSync(join(outDir, `glide_${String(f).padStart(4, '0')}.png`), encodePng(rgba, IMG, IMG))
    if (f % 25 === 0) console.log(`  beat ${f}/${FRAMES}`)
  }
  console.log(`wrote ${FRAMES} frames to ${outDir}`)
  console.log(`ffmpeg -y -framerate 24 -i ${join(outDir, 'glide_%04d.png')} -pix_fmt yuv420p ${join(here, '..', '..', 'make', '534', 'horosphere-glide-534.mp4')}`)
}
