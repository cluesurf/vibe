...
function perm(a: number, b: number): [number, number] {
  if (a === -1 && b === -1) return [-1, -1] // same sign, inert
  if (a === 1 && b === 1) return [1, 1]
  if (a === -1 && b === 0) return [0, -1] // hop
  if (a === 0 && b === -1) return [-1, 0]
  if (a === 1 && b === 0) return [0, 1]
  if (a === 0 && b === 1) return [1, 0]
  if (a === 0 && b === 0) return [1, -1] // the arrow, peace creates a balanced pair
  if (a === 1 && b === -1) return [-1, 1] // the create-flip-annihilate 3-cycle
...
function run(): void {
  ...
  console.log(`pure rule, slab ${n.toLocaleString()} cells, band ${slab.bandCount.toLocaleString()}, no interventions`)
  ...
  for (let f = 0; f < FRAMES; f++) {
    ...
    if (f % 40 === 0 || f === FRAMES - 1) {
      ...
      console.log(`  beat ${f}, charged ${(charged / n * 100).toFixed(0)}%, charge conserved ${q === q0}`)
    }
  }
  console.log(`wrote ${FRAMES} frames of the pure base rule, assemble with task/render-video.sh`)
}
