...
const HALF = 0.5, MARGIN = 0.6, FRAMES = 150, IMG = 1200, RADIUS = 2, ZOOM_FIT = 0.92
...
function run(): void {
  ...
  const nz = (v: number[]): number[] => { const m = norm(v) || 1; return v.map((x) => x / m) }
  ...
  const e2 = nz(sub(sub(seedVec(axis2), xi, dot(seedVec(axis2), xi)), e1, dot(sub(seedVec(axis2), xi, dot(seedVec(axis2), xi)), e1)))
  ...
  const Cn = maxC + 1; const off = new Array(Cn + 1).fill(0); for (let i = 0; i < E; i++) off[color[i]! + 1]++; for (let c = 0; c < Cn; c++) off[c + 1] += off[c]!
  ...
  A[ctr] = ((A[ctr]! + 1) % 3) as 0 | 1 | 2; for (const w of nb[ctr]!) A[w] = ((A[w]! + 1) % 3) as 0 | 1 | 2

  const beat = (t: Uint8Array): void => { for (let c = 0; c < Cn; c++) for (let e = off[c]!; e < off[c + 1]!; e++) { const v = edgeV[e]!, w = edgeW[e]!; const o = PERM[t[v]! * 3 + t[w]!]!; t[v] = (o / 3) | 0; t[w] = o % 3 } }

  const outDir = join(dirname(fileURLToPath(import.meta.url)), '..', '..', 'make', 'frames')
  rmSync(outDir, { recursive: true, force: true }); mkdirSync(outDir, { recursive: true })
  console.log(`lightcone: ${bandCells.length.toLocaleString()} cells, ${Cn} colours, ${FRAMES} frames`)
