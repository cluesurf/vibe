...
async function run(): Promise<void> {
  ...
  const g = buildCellGraph({ symbol: [3, 4, 3, 4], maxCells: BUILD_CELLS })
  const n = g.cellCount
  ...
  console.log(`built {3,4,3,4} 4D bulk, ${n.toLocaleString()} cells, ${adjU.length.toLocaleString()} directed edges (mean degree ${meanDeg.toFixed(1)})`)
  ...
  console.log(`benchmark ${n.toLocaleString()} 4D bulk cells, ${BENCH_BEATS} beats in ${seconds.toFixed(2)}s`)
  console.log(`  ${beatsPerSec.toFixed(0)} beats/sec, ${((beatsPerSec * n) / 1e9).toFixed(2)} billion cell-updates/sec`)
  console.log(ok ? 'OK, the {3,4,3,4} 4D bulk runs correctly on the GPU' : 'FAILED self-check')
}
