...
export function cyclicRotations(count: number): (ring: string) => string[] {
  return (ring: string): string[] => {
    const out: string[] = []
    for (let r = 0; r < count; r++) out.push(ring.slice(r) + ring.slice(0, r))
    return out
  }
}
...
export function doubleRingRotations(count: number): (rings: string) => string[] {
  return (rings: string): string[] => {
    const a = rings.slice(0, count)
    const b = rings.slice(count, count * 2)
    const out: string[] = []
    for (let r = 0; r < count; r++) out.push(a.slice(r) + a.slice(0, r) + (b.slice(r) + b.slice(0, r)))
    return out
  }
}
...
export function compileMargensternCa(input: {
  ...
  const { name, rules, neighbourLen, rotate, onConflict = 'throw' } = input
  const table = new Map<string, string>()
  const states = new Set<string>()
  let conflicts = 0
  for (const rule of rules) {
    if (rule.length !== neighbourLen + 2) throw new Error(`${name}: rule "${rule}" is not ${neighbourLen + 2} long`)
    ...
    if (existing !== undefined && existing !== next) {
      if (onConflict === 'throw') throw new Error(`${name}: rule conflict at ${key}: ${existing} vs ${next} (rule ${rule})`)
      conflicts++
  ...
  return {
    ...
    neighbourLen,
    next(current: string, neighbours: string): string {
      const padded = (neighbours + 'W'.repeat(neighbourLen)).slice(0, neighbourLen)
