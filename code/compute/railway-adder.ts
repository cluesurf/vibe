...
import { makeBinaryCounter, type BinaryCounter } from '@/code/compute/railway-ca'

// dst := dst + src, by running `src.count()` real increments of `dst` (each a locomotive carry ripple)
export function addInto(dst: BinaryCounter, src: BinaryCounter): number {
...
export function fibOnRailway(input: { n: number; bits?: number }): { value: number; increments: number } {
  ...
  for (let iter = 0; iter < input.n; iter++) {
    t.clear(); increments += addInto(t, a); increments += addInto(t, b) // t = a + b
    copyInto(a, b); increments += b.count() // a = b
    copyInto(b, t); increments += t.count() // b = t
  }
