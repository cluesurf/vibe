...
async function run(): Promise<void> {
  const adapter = await navigator.gpu.requestAdapter(); if (!adapter) { console.log('no GPU'); return }
  const device = await adapter.requestDevice()
  const g = buildDodecagrid({ maxCells: CELLS }); const N = g.cellCount
  ...
  for (let i = 0; i < E; i++) { const used = mask[eu[i]!]! | mask[ev[i]!]!; let c = 0; while (used & (1 << c)) c++; color[i] = c; mask[eu[i]!]! |= (1 << c); mask[ev[i]!]! |= (1 << c); if (c > maxColor) maxColor = c }
  const C = maxColor + 1
  const colorOffsets = new Array(C + 1).fill(0); for (let i = 0; i < E; i++) colorOffsets[color[i]! + 1]++
  for (let c = 0; c < C; c++) colorOffsets[c + 1] += colorOffsets[c]!
  const edgeV = new Uint32Array(E), edgeW = new Uint32Array(E); const cur = colorOffsets.slice()
  for (let i = 0; i < E; i++) { const at = cur[color[i]!]!++; edgeV[at] = eu[i]!; edgeW[at] = ev[i]! }
  console.log(`built ${N.toLocaleString()} cells, ${E.toLocaleString()} edges, ${C} colours`)
  ...
  const pipeline = device.createComputePipeline({ layout: 'auto', compute: { module: device.createShaderModule({ code: PURE_RULE_WGSL }), entryPoint: 'main' } })
  const layout = pipeline.getBindGroupLayout(0)
  const bind = device.createBindGroup({ layout, entries: [{ binding: 0, resource: { buffer: params } }, { binding: 1, resource: { buffer: toneBuf } }, { binding: 2, resource: { buffer: vBuf } }, { binding: 3, resource: { buffer: wBuf } }] })
  const staging = device.createBuffer({ size: N * 4, usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ })
  const beatGpu = () => { for (let c = 0; c < C; c++) { const start = colorOffsets[c]!, count = colorOffsets[c + 1]! - start; if (!count) continue; device.queue.writeBuffer(params, 0, new Uint32Array([start, count, 0, 0])); const enc = device.createCommandEncoder(); const p = enc.beginComputePass(); p.setPipeline(pipeline); p.setBindGroup(0, bind); p.dispatchWorkgroups(Math.ceil(count / WORKGROUP)); p.end(); device.queue.submit([enc.finish()]) } }

  for (let b = 0; b < WARMUP; b++) beatGpu(); await device.queue.onSubmittedWorkDone()

  // snapshots: coarse charge per scale per snapshot
  const series = partitions.map((p) => Array.from({ length: M }, () => new Float64Array(p.count)))
  for (let s = 0; s < M; s++) {
  ...
  console.log('Persistence by scale: temporal autocorrelation of coarse charge (slower decay at coarse = a layer)')
  console.log('  scale | C(1 step) | C(2) | C(4) | C(8)  [steps of ' + K + ' beats]')
  for (let sc = 0; sc < SCALES.length; sc++) {
    const ser = series[sc]!; const n = partitions[sc]!.count
    const c0 = (): number => { let s = 0; for (let st = 0; st < M; st++) for (let i = 0; i < n; i++) s += ser[st]![i]! * ser[st]![i]!; return s }
    const ctau = (tau: number): number => { let s = 0, cnt = 0; for (let st = 0; st + tau < M; st++) { for (let i = 0; i < n; i++) s += ser[st]![i]! * ser[st + tau]![i]!; cnt++ } return s }
    const base = c0() / M
    const c = (tau: number) => (ctau(tau) / Math.max(1, M - tau)) / Math.max(1e-9, base)
    console.log(`   R${SCALES[sc]}   |   ${c(1).toFixed(2)}    | ${c(2).toFixed(2)} | ${c(4).toFixed(2)} | ${c(8).toFixed(2)}`)
  }
  console.log('  => if C decays SLOWER as scale grows, a persistent slow mode (middle layer) emerges. flat across scales = pure churn, no layer.')
}
