...
async function gpuResponders(input: {
  ...
  const { device, pipeline, cellCount, wordBits, minScore, wordsU, comparandU, maskU } = input
  ...
  device.queue.writeBuffer(params, 0, new Uint32Array([cellCount, wordBits, minScore, 0]))
  ...
  const readback = device.createBuffer({ size: cellCount * 4, usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ })
...
async function run(): Promise<void> {
  ...
  if (!adapter) {
    console.log('no WebGPU adapter available (needs a GPU). The GPU associative match is written and will run where an adapter is present.')
    return
  }
  const device = await adapter.requestDevice()
  const module = device.createShaderModule({ code: ASSOCIATIVE_MATCH_WGSL })
  const pipeline = device.createComputePipeline({ layout: 'auto', compute: { module, entryPoint: 'match_kernel' } })
  ...
  const n = g.cellCount
  const mem = makeAssociativeMemory({ neighbors: g.neighbors, wordBits: WORD_BITS })
  for (let c = 0; c < n; c++) storeWord(mem, c, ternaryWord(c, WORD_BITS))
  ...
  const gpuExact = await gpuResponders({
    device, pipeline, cellCount: n, wordBits: WORD_BITS, minScore: WORD_BITS, wordsU, comparandU, maskU,
  })
  const gpuExactCells: number[] = []
  for (let c = 0; c < n; c++) if (gpuExact.responders[c] === 1) gpuExactCells.push(c)
  const exactMatch = cpuExact.length === gpuExactCells.length && cpuExact.every((c, i) => c === gpuExactCells[i])
  ...
  const gpuPartial = await gpuResponders({
    device, pipeline, cellCount: n, wordBits: WORD_BITS, minScore, wordsU, comparandU, maskU,
  })
  const gpuPartialCells: number[] = []
  for (let c = 0; c < n; c++) if (gpuPartial.responders[c] === 1) gpuPartialCells.push(c)
  const partialMatch =
    cpuPartial.length === gpuPartialCells.length && cpuPartial.every((c, i) => c === gpuPartialCells[i])

  console.log(`{3,4,3,4} bulk, ${n} cells, word ${WORD_BITS} trits`)
  console.log(`exact match, cpu ${cpuExact.length} responder(s), gpu ${gpuExactCells.length}, agree ${exactMatch}`)
  console.log(`partial match (minScore ${minScore}), cpu ${cpuPartial.length}, gpu ${gpuPartialCells.length}, agree ${partialMatch}`)

  // BENCHMARK at scale
  const gb = buildCellGraph({ symbol: [3, 4, 3, 4], maxCells: BENCH_CELLS })
  const nb = gb.cellCount
  const memb = makeAssociativeMemory({ neighbors: gb.neighbors, wordBits: WORD_BITS })
  for (let c = 0; c < nb; c++) storeWord(memb, c, ternaryWord(c, WORD_BITS))
  const wordsUb = Uint32Array.from(memb.words)
  const comparandUb = Uint32Array.from(readWord(memb, Math.floor(nb / 2)))
  const bench = await gpuResponders({
    device, pipeline, cellCount: nb, wordBits: WORD_BITS, minScore: WORD_BITS, wordsU: wordsUb, comparandU: comparandUb, maskU,
  })
  console.log(`benchmark, ${nb.toLocaleString()} cells searched on the GPU in ${bench.ms.toFixed(2)} ms (one parallel dispatch)`)

  if (!exactMatch || !partialMatch) {
    console.error('MISMATCH, the GPU responder set does not equal the CPU ground truth')
    process.exit(1)
  }
  console.log('OK, the GPU associative match equals the CPU ground truth')
}
