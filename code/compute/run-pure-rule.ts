...
function cpuBeat(tone: Uint32Array, edgeV: Uint32Array, edgeW: Uint32Array, colorOffsets: number[]): void {
  for (let c = 0; c < colorOffsets.length - 1; c++) {
    for (let e = colorOffsets[c]!; e < colorOffsets[c + 1]!; e++) {
      const v = edgeV[e]!, w = edgeW[e]!
      const out = PERM[tone[v]! * 3 + tone[w]!]!
      tone[v] = Math.floor(out / 3); tone[w] = out % 3
    }
  }
}

const charge = (t: Uint32Array): number => { let s = 0; for (let i = 0; i < t.length; i++) s += t[i] === 1 ? 1 : t[i] === 2 ? -1 : 0; return s }

async function run(): Promise<void> {
  const adapter = await navigator.gpu.requestAdapter()
  if (!adapter) { console.log('no WebGPU adapter (needs a GPU)'); return }
  ...
  for (let v = 0; v < N; v++) for (let p = g.offsets[v]!; p < g.offsets[v + 1]!; p++) { const w = g.adj[p]!; if (w > v) { eu.push(v); ev.push(w) } }
  const E = eu.length
  console.log(`built {5,3,4}, ${N.toLocaleString()} cells, ${E.toLocaleString()} edges`)

  // greedy edge-colouring (each colour a matching, no vertex repeats a colour)
  const mask = new Uint32Array(N); const color = new Int32Array(E); let maxColor = 0
  for (let i = 0; i < E; i++) {
    const used = mask[eu[i]!]! | mask[ev[i]!]!
    let c = 0; while (used & (1 << c)) c++
    color[i] = c; mask[eu[i]!]! |= (1 << c); mask[ev[i]!]! |= (1 << c)
    if (c > maxColor) maxColor = c
  }
  const C = maxColor + 1
  // sort edges by colour, build offsets
  const counts = new Array(C).fill(0); for (let i = 0; i < E; i++) counts[color[i]!]++
  const colorOffsets = new Array(C + 1).fill(0); for (let c = 0; c < C; c++) colorOffsets[c + 1] = colorOffsets[c]! + counts[c]!
  const edgeV = new Uint32Array(E), edgeW = new Uint32Array(E); const cursor = colorOffsets.slice()
  for (let i = 0; i < E; i++) { const c = color[i]!; const at = cursor[c]!++; edgeV[at] = eu[i]!; edgeW[at] = ev[i]! }
  ...
  const wBuf = device.createBuffer({ size: E * 4, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST }); device.queue.writeBuffer(wBuf, 0, edgeW)
  ...
  const bind = device.createBindGroup({ layout, entries: [
    { binding: 0, resource: { buffer: params } }, { binding: 1, resource: { buffer: toneBuf } },
    { binding: 2, resource: { buffer: vBuf } }, { binding: 3, resource: { buffer: wBuf } } ] })

  const beatGpu = (): void => {
    for (let c = 0; c < C; c++) {
      const start = colorOffsets[c]!, count = colorOffsets[c + 1]! - start
      if (count === 0) continue
      device.queue.writeBuffer(params, 0, new Uint32Array([start, count, 0, 0]))
  ...
  await staging.mapAsync(GPUMapMode.READ); const gpuOut = new Uint32Array(staging.getMappedRange().slice(0)); staging.unmap()
  const cpu = seed.slice(); for (let b = 0; b < CHECK_BEATS; b++) cpuBeat(cpu, edgeV, edgeW, colorOffsets)
  let mism = 0; for (let i = 0; i < N; i++) if (cpu[i] !== gpuOut[i]) mism++
  console.log(`self-check ${CHECK_BEATS} beats: GPU vs CPU mismatches ${mism} -> ${mism === 0 ? 'IDENTICAL' : 'FAIL'}`)
  console.log(`charge conservation: start ${startCharge} -> after ${charge(gpuOut)} -> ${startCharge === charge(gpuOut) ? 'CONSERVED' : 'BROKEN'}`)

  // (3) benchmark at scale
  device.queue.writeBuffer(toneBuf, 0, seed)
  beatGpu(); await device.queue.onSubmittedWorkDone()
  const t0 = performance.now(); for (let b = 0; b < BENCH_BEATS; b++) beatGpu(); await device.queue.onSubmittedWorkDone()
  const secs = (performance.now() - t0) / 1000
  console.log(`benchmark: ${N.toLocaleString()} cells, ${BENCH_BEATS} beats in ${secs.toFixed(2)}s, ${(BENCH_BEATS / secs).toFixed(0)} beats/sec, ${((BENCH_BEATS * E) / secs / 1e9).toFixed(2)} billion edge-updates/sec`)
  console.log(mism === 0 ? 'OK, the pure five-thing rule runs faithfully on the GPU at scale' : 'FAILED self-check')
  ...
  device.queue.writeBuffer(toneBuf, 0, seed)
  for (let b = 0; b < 3000; b++) beatGpu(); await device.queue.onSubmittedWorkDone()
  { const enc = device.createCommandEncoder(); enc.copyBufferToBuffer(toneBuf, 0, staging, 0, N * 4); device.queue.submit([enc.finish()]) }
  await staging.mapAsync(GPUMapMode.READ); const fin = new Uint32Array(staging.getMappedRange().slice(0)); staging.unmap()
  const sign = new Int8Array(N); for (let i = 0; i < N; i++) sign[i] = fin[i] === 1 ? 1 : fin[i] === 2 ? -1 : 0
  const largestSameSign = (s: Int8Array): { largest: number, charged: number } => {
    ...
    for (let v = 0; v < N; v++) { if (s[v] === 0) continue; for (let p = g.offsets[v]!; p < g.offsets[v + 1]!; p++) { const w = g.adj[p]!; if (w > v && s[w] === s[v]) par[find(v)] = find(w) } }
    const sz = new Map<number, number>(); let charged = 0; for (let i = 0; i < N; i++) { if (s[i] === 0) continue; charged++; const r = find(i); sz.set(r, (sz.get(r) ?? 0) + 1) }
    let m = 0; for (const v of sz.values()) m = Math.max(m, v); return { largest: m, charged }
  }
  const l0 = largestSameSign(sign)
  console.log(`L0 structure (descriptive): ${l0.charged.toLocaleString()} charged cells (${(100 * l0.charged / N).toFixed(0)}%), largest same-sign blob ${l0.largest} cells (small = churn, no persistent selves, as P101)`)
}
