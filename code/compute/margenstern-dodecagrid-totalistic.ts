...
export const DODECAGRID_TOTALISTIC_RULES: Array<[string, number, string]> = [
  ['W', 0, 'W'], ['R', 1, 'R'], ['G', 0, 'G'], ['R', 2, 'R'], ['R', 0, 'R'], ['R', 3, 'R'], ['W', 2, 'W'], ['G', 1, 'G'],
  ['W', 3, 'W'], ['G', 2, 'G'], ['W', 6, 'W'], ['G', 3, 'G'], ['W', 5, 'W'], ['W', 8, 'W'], ['W', 11, 'W'], ['W', 15, 'W'],
  ['W', 18, 'W'], ['W', 7, 'R'], ['W', 10, 'G'], ['R', 6, 'W'], ['G', 8, 'W'], ['W', 9, 'W'], ['W', 14, 'B'], ['B', 11, 'B'],
  ['W', 13, 'W'], ['W', 12, 'W'], ['R', 15, 'R'], ['W', 19, 'R'], ['W', 17, 'R'], ['R', 18, 'W'], ['R', 17, 'W'], ['R', 20, 'W'],
  ['W', 16, 'W'], ['G', 5, 'G'],
]

const TOTALISTIC = new Map<string, string>(DODECAGRID_TOTALISTIC_RULES.map(([s, w, n]) => [s + ':' + w, n]))
...
export function dodecagridTotalisticNext(current: string, neighbours: string[]): string {
  return TOTALISTIC.get(current + ':' + dodecagridWeight(neighbours)) ?? current
}
