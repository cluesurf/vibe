...
async function run(): Promise<void> {
  ...
  type Dot = { index: number; px: number; py: number; rad: number; r2: number }
  const dots: Dot[] = []
  for (let i = 0; i < n; i++) {
    const x = g.coords[i]![0]!, y = g.coords[i]![1]!
    const r2 = norm(g.coords[i]!) ** 2
    dots.push({ index: i, px: half + scale * x, py: half - scale * y, rad: Math.max(0.6, DOT_SCALE * (1 - r2)), r2 })
  }
  ...
  const bind = (read: GPUBuffer, write: GPUBuffer): GPUBindGroup => device.createBindGroup({
    layout,
    entries: [
  ...
  const drawDot = (rgba: Uint8Array, cx: number, cy: number, rad: number, col: [number, number, number]): void => {
    ...
    for (let py = c0; py <= c1; py++) for (let px = r0; px <= r1; px++) {
      ...
      if (dx * dx + dy * dy <= rr) { const o = (py * IMG + px) * 4; rgba[o] = col[0]; rgba[o + 1] = col[1]; rgba[o + 2] = col[2]; rgba[o + 3] = 255 }
    }
  }
  ...
  for (let f = 0; f < FRAMES; f++) {
    ...
    for (let i = 0; i < rgba.length; i += 4) { rgba[i] = 8; rgba[i + 1] = 8; rgba[i + 2] = 11; rgba[i + 3] = 255 }
    // boundary ring
    for (let a = 0; a < 3600; a++) { const th = (a / 3600) * 2 * Math.PI; const px = Math.round(half + scale * Math.cos(th)), py = Math.round(half + scale * Math.sin(th)); if (px >= 0 && px < IMG && py >= 0 && py < IMG) { const o = (py * IMG + px) * 4; rgba[o] = 60; rgba[o + 1] = 60; rgba[o + 2] = 68 } }
    for (const d of dots) {
  ...
  console.log(`ffmpeg -framerate 24 -i ${join(outDir, 'wave-%03d.png')} -pix_fmt yuv420p ${join(here, 'nesting-wave-534.mp4')}`)
}
