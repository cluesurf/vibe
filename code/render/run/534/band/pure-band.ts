...
function run(): void {
  ...
  const C = maxC + 1
  const off = new Array<number>(C + 1).fill(0)

  for (let i = 0; i < E; i++)
    off[color[i]! + 1] = off[color[i]! + 1]! + 1

  for (let c = 0; c < C; c++) off[c + 1] = off[c + 1]! + off[c]!
  ...
  console.log(
    `pure rule on the band, ${bandCells.length.toLocaleString()} cells, ${C} colours, rendering ${FRAMES} frames (coarse density)`,
  )
  ...
  for (let f = 0; f < FRAMES; f++) {
    ...
    for (const bandCell of bandCells) {
      const v = Math.abs(q[bandCell]!)
