...
import { makeUnaryCounter, makeBinaryCounter, makeTernaryCounter, type RailRegister } from '@/code/compute/railway-ca'
...
type Config = { name: string; base: number; shown: number; make: () => RailRegister }
const CONFIGS: Config[] = [
  { name: 'unary', base: 1, shown: 60, make: () => makeUnaryCounter() },
  { name: 'binary', base: 2, shown: 10, make: () => makeBinaryCounter(8) },
  { name: 'ternary', base: 3, shown: 8, make: () => makeTernaryCounter(8) },
]
...
function renderBase(cfg: Config, tiling: ReturnType<typeof buildTilingFaces>, outDir: string): void {
  const cellCount = tiling.cellCount
  const a = cfg.make(), b = cfg.make(), t = cfg.make()
  a.clear(); b.set(1); t.clear()
  const counters = [a, b, t]
  const snaps: Snap[] = []
  let latched = 0
  const snap = (active: number): void => { snaps.push({ regs: counters.map((c) => c.count()), active, display: latched }) }
  const addInto = (dstIdx: number, srcIdx: number): void => {
    const times = counters[srcIdx]!.count()
    for (let k = 0; k < times; k++) { counters[dstIdx]!.increment(); snap(dstIdx) }
  }
  snap(0)
  for (let iter = 0; iter < N; iter++) {
    t.clear(); addInto(2, 0); addInto(2, 1) // t = a + b
    a.clear(); addInto(0, 1) // a = b
    latched = a.count()
    b.clear(); addInto(1, 2) // b = t
  }
  const finalTerm = a.count()
  console.log(`literal railway CA (${cfg.name}): fib(${N}) = ${finalTerm} via ${snaps.length} locomotive increments`)

  const wedges = layoutWedges(tiling.centers, counters.length)
  const edges = cellOutlines(tiling.polygons)
  const frames: Uint8Array[] = []
  for (let i = 0; i < snaps.length; i++) frames.push(renderFrame({ tiling, edges, wedges, snap: snaps[i]!, prev: snaps[Math.max(0, i - 1)]!, cellCount, cfg }))
  for (let h = 0; h < 16; h++) frames.push(renderFrame({ tiling, edges, wedges, snap: { regs: counters.map((c) => c.count()), active: -1, display: finalTerm }, prev: snaps[snaps.length - 1]!, cellCount, cfg }))

  writeFileSync(join(outDir, `fibonacci-7-3-railway-${cfg.name}-frame.png`), encodePng(frames[frames.length - 1]!, SIZE, SIZE))
  const gif = encodeGif({ frames, width: SIZE, height: SIZE, delayMs: 40 })
  writeFileSync(join(outDir, `fibonacci-7-3-railway-${cfg.name}.gif`), gif)
  console.log(`wrote fibonacci-7-3-railway-${cfg.name}.gif  ${(gif.length / 1024).toFixed(0)} KB  ${frames.length} frames  ${SIZE}x${SIZE}`)
}
...
function renderFrame(input: {
  ...
  const digit = (value: number, p: number): number => Math.floor(value / cfg.base ** p) % cfg.base
    ...
    for (let p = 0; p < track.length; p++) {
      ...
      if (cfg.base === 1) {
        ...
        if (p < value) {
          ...
          const rgb01 = changed ? ([0.96, 0.96, 0.99] as const) : shade(hue, 0.5)
          color = [Math.round(rgb01[0] * 255), Math.round(rgb01[1] * 255), Math.round(rgb01[2] * 255)]
        }
      } else if (p < cfg.shown) {
        ...
        const rgb01 = changed ? ([0.96, 0.96, 0.99] as const) : shade(hue, 0.8 - 0.3 * (d / (cfg.base - 1)))
        color = [Math.round(rgb01[0] * 255), Math.round(rgb01[1] * 255), Math.round(rgb01[2] * 255)]
      }
  ...
  const opRgb = shade(snap.active >= 0 ? 'emerald' : 'zinc', snap.active >= 0 ? 0.5 : 0.4)
  ...
  const { rgba } = renderSceneToRgba({ scene, size: SIZE, segments: 16, lineWidth: 1.0, near: [56, 56, 66], far: [56, 56, 66], model: 'poincare', margin: MARGIN, superSample: 2 })
  drawCentralNumber({ rgba, size: SIZE, margin: MARGIN, centralPolygon: tiling.polygons[0]!, text: String(snap.display) })
  return rgba
}
