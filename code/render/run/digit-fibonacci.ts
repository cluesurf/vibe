...
export type DigitStep = { kind: string; reg: number; registers: bigint[] }

// run the shared Fibonacci animation for a positional base
export function renderDigitFibonacci(input: {
  base: number
  runner: (program: BinaryProgram, initial: bigint[], onStep: (step: DigitStep) => void) => void
  ...
  console.log(`${outName}: ${compiled.program.code.length} ops, executed ${steps.length} ${input.costLabel} computing fib(${n}) = ${finalTerm}`)

  const wedges = layoutWedges(tiling.centers, names.length)
  const edges = cellOutlines(tiling.polygons)

  const digit = (value: bigint, p: number): number => Number((value / BigInt(base) ** BigInt(p)) % BigInt(base))
  const frames: Uint8Array[] = []
  for (let i = 0; i < steps.length; i++) {
    const step = steps[i]!
    const prev = i === 0 ? initial : steps[i - 1]!.registers
    const changed = new Set<number>()
    for (let p = 0; p < digitsShown; p++) if (digit(step.registers[step.reg]!, p) !== digit(prev[step.reg]!, p)) changed.add(p)
    ...
    for (let f = 0; f < FRAMES_PER_OP; f++) {
      ...
      frames.push(renderFrame({ tiling, edges, wedges, names, registers: step.registers, active: step.reg, changed: flash, mode, display: display[i]!, cellCount, base, digitsShown, digit }))
    }
  }
  const lastRegs = steps[steps.length - 1]!.registers
  for (let h = 0; h < 14; h++) frames.push(renderFrame({ tiling, edges, wedges, names, registers: lastRegs, active: compiled.returnRegister, changed: new Set(), mode: 'idle', display: finalTerm, cellCount, base, digitsShown, digit }))

  writeFileSync(join(outDir, `${outName}-frame.png`), encodePng(frames[frames.length - 1]!, SIZE, SIZE))
  const gif = encodeGif({ frames, width: SIZE, height: SIZE, delayMs: 70 })
  writeFileSync(join(outDir, `${outName}.gif`), gif)
  console.log(`wrote ${outName}.gif  ${(gif.length / 1024).toFixed(0)} KB  ${frames.length} frames  ${SIZE}x${SIZE}`)
}
...
function renderFrame(input: {
  ...
  const { tiling, edges, wedges, names, registers, active, changed, mode, display, cellCount, base, digitsShown, digit } = input
  ...
  for (let r = 0; r < names.length; r++) {
    ...
    for (let p = 0; p < track.length; p++) {
      ...
      if (p < digitsShown) {
        ...
        const rgb01 = isFlash ? ([0.96, 0.96, 0.99] as const) : shade(hue, t)
