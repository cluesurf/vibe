...
function run(): void {
  ...
  const finalTerm = trace[trace.length - 1]!.registers[compiled.returnRegister]!
  console.log(`compiled ${compiled.program.code.length} instructions; executed ${trace.length} steps computing fib(${N}) = ${finalTerm}`)
  ...
  type Segment = { pre: number[]; post: number[]; display: number; active: number; mode: 'add' | 'sub' | 'idle' }
  const segments: Segment[] = []
  let start = 0
  for (let i = 1; i <= trace.length; i++) {
    const boundary = i === trace.length || opIndex[trace[i]!.pc] !== opIndex[trace[start]!.pc]
    if (!boundary) continue
    const pre = start === 0 ? initial.slice() : trace[start - 1]!.registers
    const post = trace[i - 1]!.registers
    let active = 0, delta = 0
    for (let r = 0; r < post.length; r++) { const d = post[r]! - pre[r]!; if (Math.abs(d) > Math.abs(delta)) { delta = d; active = r } }
    segments.push({ pre, post, display: latchedAt[i - 1]!, active, mode: delta > 0 ? 'add' : delta < 0 ? 'sub' : 'idle' })
    start = i
  }
  ...
  for (const seg of segments) {
    for (let f = 1; f <= framesPerOp; f++) {
      const registers = seg.post.map((post, r) => Math.round(seg.pre[r]! + (post - seg.pre[r]!) * (f / framesPerOp)))
      frames.push(renderFrame({ tiling, edges, wedges, names, registers, active: seg.active, mode: seg.mode, display: seg.display, cellCount }))
    }
  }
  // hold the final answer for a moment
  const lastRegs = trace[trace.length - 1]!.registers
  for (let h = 0; h < 14; h++) frames.push(renderFrame({ tiling, edges, wedges, names, registers: lastRegs, active: compiled.returnRegister, mode: 'idle', display: finalTerm, cellCount }))

  writeFileSync(join(outDir, 'fibonacci-7-3-poincare-frame.png'), encodePng(frames[frames.length - 1]!, SIZE, SIZE))
  const gif = encodeGif({ frames, width: SIZE, height: SIZE, delayMs: 55 })
  writeFileSync(join(outDir, 'fibonacci-7-3-poincare.gif'), gif)
  console.log(`wrote fibonacci-7-3-poincare.gif  ${(gif.length / 1024).toFixed(0)} KB  ${frames.length} frames  ${SIZE}x${SIZE}`)
}
