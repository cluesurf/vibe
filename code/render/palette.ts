...
export type Hue = 'violet' | 'emerald' | 'blue' | 'rose' | 'yellow' | 'zinc'

// the preference order, violet first; zinc is the neutral default rather than a "favourite"
export const HUE_ORDER: Hue[] = ['violet', 'emerald', 'blue', 'rose', 'yellow']
...
export const PALETTE_RGB01: Record<Hue, Record<Shade, [number, number, number]>> = Object.fromEntries(
  (Object.keys(PALETTE_HEX) as Hue[]).map((hue) => [
    hue,
    Object.fromEntries(SHADES.map((s) => [s, hexToRgb01(PALETTE_HEX[hue][s])])) as Record<Shade, [number, number, number]>,
  ]),
