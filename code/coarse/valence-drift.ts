...
function selfCentroidX(tone: Int8Array, graph: Graph, L: number, margin: number): number {
...
export function valenceDrift(input: {
  ...
  for (let t = 0; t < beats; t++) {
    for (let c = 0; c < graph.cellCount; c++) {
      const x = c % L
      if (x >= L - margin) tone[c] = (plusSide === 'right' ? 1 : -1) as -1 | 1
      else if (x < margin) tone[c] = (plusSide === 'right' ? -1 : 1) as -1 | 1
    }
...
export function valenceDifferential(input: { L: number; beats: number; seed: number; withDynamics: boolean }): number {
  return (
    valenceDrift({ ...input, plusSide: 'right' }) - valenceDrift({ ...input, plusSide: 'left' })
  )
}
