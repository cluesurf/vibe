...
import { flatGraph, beat, largestPositiveCluster } from '@/code/model/self-kit'
...
export function activePersistence(input: {
  ...
  const refugeX = refuge === 'left' ? 0 : refuge === 'right' ? L - refugeWidth : -1
  const inRefuge = (c: number): boolean =>
    refuge !== 'none' && c % L >= refugeX && c % L < refugeX + refugeWidth

  for (let t = 0; t < beats; t++) {
    if (refuge !== 'none') for (let c = 0; c < graph.cellCount; c++) if (inRefuge(c)) tone[c] = 1
  ...
  return { survivingSize: cells.length, finalX: cells.length ? sx / cells.length : -1 }
}
