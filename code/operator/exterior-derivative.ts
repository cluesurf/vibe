...
export function multiply(left: Matrix, right: Matrix): Matrix {
  ...
  return left.map((row) =>
    (right[0] ?? []).map((_, column) => {
      let sum = 0
      for (let index = 0; index < inner; index++) sum += (row[index] ?? 0) * (right[index]![column] ?? 0)
      return sum
    }),
  )
}
...
export function exteriorDerivative(complex: CellComplex, grade: number): Matrix {
...
export function kahlerDirac(complex: CellComplex): Matrix {
  const total = complex.dimensions.reduce((sum, count) => sum + count, 0)
  const offset: number[] = []
  let running = 0
  ...
  const operator: Matrix = Array.from({ length: total }, () => new Array(total).fill(0))
  for (let grade = 0; grade < complex.boundary.length; grade++) {
    ...
    for (let row = 0; row < derivative.length; row++) {
      for (let column = 0; column < derivative[row]!.length; column++) {
        ...
        operator[lowerRow]![upperColumn] = (operator[lowerRow]![upperColumn] ?? 0) + value
        operator[upperColumn]![lowerRow] = (operator[upperColumn]![lowerRow] ?? 0) + value
      }
    }
  }
  return operator
...
export function polygonComplex(sides: number): CellComplex {
  ...
  const edgesToVertices: Matrix = Array.from({ length: sides }, () => new Array(sides).fill(0))
  ...
  return { dimensions: [sides, sides, 1], boundary: [edgesToVertices, faceToEdges] }
}

// Whether d composed with d is zero (the defining identity of a complex).
export function boundaryOfBoundaryIsZero(complex: CellComplex): boolean {
  for (let grade = 0; grade + 1 < complex.boundary.length; grade++) {
    const product = multiply(complex.boundary[grade]!, complex.boundary[grade + 1]!)
    if (product.some((row) => row.some((value) => Math.abs(value) > 1e-9))) return false
  }
  return true
}
