// E-FRC-0159's battery on the BOUNCE knit (code/rule/bounce-pair-knit) over a dock-varying living vacuum (E-RLT-0085).
// It is code/measure/varying-living-battery's candidate items (E-RLT-0082), item for item, with the collision a property
// of the vacuum handed in (`collision`: 'lone', 'bounce' or 'isometric', the last being E-RLT-0082's knit exactly) and
// the kernel code/measure/bounce-pair-kernel. The reference battery (referenceAcceptance) and the sides are imported
// from varying-living-battery, not copied. What follows is that file's description of its items, unchanged.
//
...
import { bounceBeat as livingBeat, bounceBeatBack as livingBeatBack, bounceCollide as livingCollide, makeBounceKnit, type BounceKnit as LivingKnit, type CollisionKind } from '@/code/rule/bounce-pair-knit'
import { cloneReduced, tritDifference, type Reduced } from '@/code/measure/living-pair-kernel'
import { bounceRunner as livingRunner, collideBounce as collideLiving, makeBounceKernel, type BounceKernel as LivingKernel } from '@/code/measure/bounce-pair-kernel'
...
const makeLivingKnit = (weave: ColorWeave, collision: CollisionKind): LivingKnit => makeBounceKnit(weave, 'alternate', true, collision)
...
function vacuumTrack(vac: VaryingVacuum, side: number, anchor: number): VacuumTrack {
  ...
  const kernel = kernelOf(side, vac.collision)
...
export function reversalAndCharge(vac: VaryingVacuum, side: number): { reverses: boolean; chargeKept: boolean } {
  ...
  const knit = makeLivingKnit(weave, vac.collision)
...
function frameOf(weave: ColorWeave): { frame: number[]; links: Int16Array } {
...
// `carryOwnPoints`: the frame change carries each coordinate's own point (carryCoordinate), as E-FRC-0159's battery
// does under the adopted comoving fear beat (the comoving beat reads a meeting about the own points, so a frame change
// that leaves them behind is not a frame change of the rule). false reproduces varying-living-battery's moveCoordinate
export function quantum(vac: VaryingVacuum, mode: 'on' | 'off', side: number, carryOwnPoints = true): QuantumRun {
  const weave = weaveOf(side)
  const knit = makeLivingKnit(weave, vac.collision)
  ...
  const { frame, links: gaugeLinks } = frameOf(weave)
  ...
  // the swap-phase control changes only the love-fear kernel, so it can fail only on a pair that meets as a love and a
  // fear: the vacuum pair when it does (the one-line vacuum's), else the matter pair that does (the hub vacuum's pairs
  // meet as two loves or two fears, so there the vacuum pair cannot read the control)
  const vacuumMeetsLoveFear = vacuumStudy?.records.some(r => (r.signs ?? []).some(([sa, sb]) => sa !== sb)) ?? false
  const frameControl =
    hasVacuum && vacuumMeetsLoveFear
  ...
  return {
    ...
    metrics: {
      ...
      controlOnVacuumPair: hasVacuum && vacuumMeetsLoveFear ? 1 : 0,
