...
export function recoverByMajority(boundary: ReadonlyArray<number>): number {
  const ones = boundary.reduce((sum, bit) => sum + bit, 0)
  return ones * 2 >= boundary.length ? 1 : 0
}

export function corruptConnectedRegion(input: { size: number; fraction: number; logical: number }): number[] {
  const boundary = new Array(input.size).fill(input.logical)
  const corrupt = Math.round(input.size * input.fraction)
  for (let index = 0; index < corrupt; index++) boundary[index] = 1 - input.logical // flip a connected block
  return boundary
}
