...
export function surveyTessellation(input: {
  ...
  specDimT1?: number
  specDimT2?: number
}): { ok: boolean; cells: number; degree: number; growth: number; betheAlpha: number; specDim: number } {
  try {
    const g = input.flat
      ? buildEuclideanLattice({ symbol: input.symbol as never, maxCells: input.maxCells })
      : buildCellGraph({ symbol: input.symbol as never, maxCells: input.maxCells })
    const N = g.cellCount, nb = g.neighbors
    const minCells = input.minCells ?? 50
    if (N < minCells) return { ok: false, cells: N, degree: 0, growth: 0, betheAlpha: 0, specDim: 0 }
    ...
    const growth = shellGrowthRatio({ shellCounts: shell, from: input.growthFrom ?? 2, to: input.growthTo ?? 6, safeDenominator: input.safeDenominator })
    const betheAlpha = betheCorrelatorExponent(degree)
    const specDim = input.withSpectralDimension
      ? Math.round(spectralDimension({ neighbors: nb, start: center, t1: input.specDimT1 ?? 2, t2: input.specDimT2 ?? 4 }) * 100) / 100
      : 0
    return { ok: true, cells: N, degree, growth, betheAlpha, specDim }
  } catch (e) { return { ok: false, cells: 0, degree: 0, growth: 0, betheAlpha: 0, specDim: 0 } }
}
