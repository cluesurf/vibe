...
// (g after e)(i) = g[e[i]]
const composePerm = (g: number[], e: number[]): number[] =>
  e.map(x => g[x]!)
...
export function generationCosetStructure(): {
  ...
  const f4 = permutationGroupElements([
    ...b4Generators,
    ...f4Reflections,
  ])
  ...
  if (triality !== null && cosetLabels.length === 3) {
    ...
    const imageLabel = (label: string): string => {
      ...
      return labelOf.get(
        permKey(composePerm(representative, triality)),
      )!
    }
...
// The maximal symmetry ANY collision law can have, by exhaustive stabilizer scan. For every one
// of the 10395 matchings, count the elements of the full F4 line-image (order 576) that fix it,
// and take the maximum. This closes the gap the endpoint counts leave open: B4-invariant gives
// one law and F4-invariant gives zero, but a law could in principle have a large stabilizer that
// does not contain B4 (an order-288 subgroup, say). The scan shows no law does: the maximum
// stabilizer over all 10395 is exactly the B4 line-image order 192, attained by exactly THREE
// laws, the knit and its two triality images, one per conjugate B4, one per generation frame
// (orbit-stabilizer: the knit's F4 orbit has size 576/192 = 3). So B4 up to conjugacy really is
// the ceiling for any collision law, and choosing the knit is choosing a generation frame. (For
// stabilizers CONTAINING B4 the scan is not even needed: the index [F4:B4] is the prime 3, so a
// subgroup properly between them cannot exist, and F4 itself fixes nothing.)
export function maxMatchingStabilizer(): {
  maxStabilizerOrder: number
  lawsAttainingMax: number
  maxIsTheKnit: boolean
} {
  const { b4Generators, f4Reflections } = buildLineStructure()
  const f4 = permutationGroupElements([
    ...b4Generators,
  ...
  let maxStabilizerOrder = 0
  let lawsAttainingMax = 0
  let maxIsTheKnit = false

  for (const m of perfectMatchings(all)) {
    const partner = new Array<number>(12)
    ...
    for (const g of f4) {
      if (isFixedBy(partner, g)) {
        stabilizer++
      }
    }

    if (stabilizer > maxStabilizerOrder) {
      maxStabilizerOrder = stabilizer
      lawsAttainingMax = 1
      maxIsTheKnit = b4Generators.every(g => isFixedBy(partner, g))
    } else if (stabilizer === maxStabilizerOrder) {
      lawsAttainingMax++
    }
  }

  return { maxStabilizerOrder, lawsAttainingMax, maxIsTheKnit }
}
