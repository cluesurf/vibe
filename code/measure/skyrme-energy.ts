...
export function hedgehogTexture3d(input: { size: number; radius: number }): Vector3[][][] {
  const { size: M, radius: R } = input
  const c = M / 2
  const field: Vector3[][][] = Array.from({ length: M }, () =>
    Array.from({ length: M }, () => Array.from({ length: M }, () => [0, 0, 1] as Vector3)),
  )
  for (let x = 0; x < M; x++) for (let y = 0; y < M; y++) for (let z = 0; z < M; z++) {
    ...
    const rhat: Vector3 = rho > 1e-6 ? [dx / rho, dy / rho, dz / rho] : [0, 0, 1]
...
export function blankDirectionField2d(size: number): Vector3[][] {
  return Array.from({ length: size }, () => Array.from({ length: size }, () => [0, 0, 1] as Vector3))
}
...
export function placeSkyrmion2d(input: {
  ...
  const { field, centerX: cx, centerY: cy, radius: R, charge: ch } = input
  const N = field.length
  for (let x = 0; x < N; x++) for (let y = 0; y < N; y++) {
    const r = Math.hypot(x - cx, y - cy); if (r > 2.2 * R) continue
    const phi = Math.atan2(y - cy, x - cx) * ch, fr = Math.PI * Math.max(0, 1 - r / R)
    const nv = normalize3([Math.sin(fr) * Math.cos(phi), Math.sin(fr) * Math.sin(phi), Math.cos(fr)])
