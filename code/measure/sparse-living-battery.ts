// E-FRC-0159's battery on the living-pair knit over a SPARSE living vacuum (E-RLT-0077 to E-RLT-0079). It is
// code/measure/living-pair-battery (E-RLT-0075's battery) with one change: every item that took the vacuum's store as
// one trit on all twelve lines (tau) takes a store pattern instead, one trit per line, the same on every dock
// (code/measure/sparse-living-vacuum). With the pattern ALL_LINES it is E-RLT-0075's battery item for item, and with
// all zeros its cold vacuum. The dock-level items that do not read the vacuum (CPT at the collision, the coin-map
// ledger, the lone-love response, the generation planes) are imported from living-pair-battery unchanged.
//
// E-RLT-0075's header, kept: the four transcription changes that the living vacuum forces, each decided before any
// number was read:
...
export function lineComponents(pattern: StorePattern, dense: boolean, schedule: LivingSchedule = 'alternate'): { trits: number; vibes: number } {
  ...
  for (let direction = 0; direction < 24; direction++) {
    const start = reducedFill(fill, pattern, layoutOf(5))
...
export function dressing(pattern: StorePattern, side: number, tone: number, schedule: LivingSchedule = 'alternate'): LivingDressing {
  ...
  const track = vacuumTrack(side, pattern, schedule)
...
export function travel(pattern: StorePattern, schedule: LivingSchedule = 'alternate'): { travellers: number; fullSpeed: number; meanReach: number } {
  ...
  const track = vacuumTrack(13, pattern, schedule)
...
export function boxGates(pattern: StorePattern, schedule: LivingSchedule = 'alternate'): Record<string, number> {
  ...
  const start = stateFill(goldenFill(cells * 24, 1.37), pattern, layout)
