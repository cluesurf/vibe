...
export function branePotential(input: { radius: number; extraDimension: number }): number {
  const { radius, extraDimension: L } = input
  return 1 / (4 * Math.PI * L * radius * (1 - Math.exp((-2 * Math.PI * radius) / L)))
}
...
export function weakFieldLightDeflection(input: {
  ...
  let integral = 0
  for (let x = -L; x <= L; x += dx) integral += ((M * b) / (x * x + b * b) ** 1.5) * dx
