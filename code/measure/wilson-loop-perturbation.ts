...
export function treeLevelLoop(input: {
  r: number
  t: number
  box: number
}): number {
  ...
  for (let a = 0; a < box; a++) {
    ...
    for (let b = 0; b < box; b++) {
      ...
      const current =
        loopMu * 4 * Math.sin((t * kNu) / 2) ** 2 +
        sumSquared(t, kNu) * edgeMu
...
export function treeLevelCreutz(input: {
  r: number
  box: number
}): number {
  const f = (r: number, t: number): number =>
    treeLevelLoop({ r, t, box: input.box })
  const { r } = input
