...
export function rotationCurveFromPotential(input: {
...
}): { r: number[]; v2: number[]; outerSlope: number; flatnessRatio: number } {
  ...
  const innerMean = v2.slice(0, third).reduce((a, b) => a + b, 0) / third
  const outerMean = v2.slice(v2.length - third).reduce((a, b) => a + b, 0) / third
