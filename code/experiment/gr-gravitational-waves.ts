// Gravitational-wave FORMS on the substrate. The substrate supplies a massless spin-2 field (the
// graviton, P21/P24/P73, derived Christoffel->Ricci->Einstein) propagating on the flat 3D cusp at the
// rule's light-cone speed z = 1. GIVEN that emergent graviton, the radiation it carries takes the
// standard general-relativistic forms, which this experiment computes and verifies:
//   1. the graviton itself, massless, 2 transverse-traceless polarizations, dispersion omega = |k|,
//   2. the quadrupole waveform of a binary, h_+ and h_x, with GW frequency = 2 x orbital frequency,
//   3. the inspiral CHIRP, frequency f(t) ~ (t_c - t)^(-3/8) (the signature LIGO chirp exponent),
...
export function main(): void {
  ...
  console.log(`\nVerdict: gravitational-wave forms reproduced: ${all ? 'CONFIRMED' : 'INCOMPLETE'}`)
  console.log('(graviton 2 polarizations + quadrupole waveform + radiated power + the -3/8 chirp. The substrate')
  console.log(' supplies the spin-2 field on flat 3D space at z=1; the waveforms are the GR forms it must carry.)')
