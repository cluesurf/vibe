...
import { buildHorosphere } from '~/substrate/coxeter/cell-direct'
...
export function toCSR(neighbors: number[][], coords?: number[][]): Graph {
  const n = neighbors.length
  const offsets = new Int32Array(n + 1)
  for (let i = 0; i < n; i++) offsets[i + 1] = offsets[i]! + neighbors[i]!.length
  const adj = new Int32Array(offsets[n]!)
  let p = 0
  for (let i = 0; i < n; i++) for (const w of neighbors[i]!) adj[p++] = w
...
export const horosphereGraph = (maxCells: number, bandHalfWidth = 0.45): Graph => {
...
export const squareGraph = (L: number): Graph => {
  ...
  const adj = new Int32Array(offsets[N]!)
  let p = 0
  for (let y = 0; y < L; y++) for (let x = 0; x < L; x++) for (let k = 0; k < 4; k++) {
    const a = x + dx[k]!
    const b = y + dy[k]!
    if (a >= 0 && a < L && b >= 0 && b < L) adj[p++] = b * L + a
  }
...
export function ball(g: Graph, center: number, radius: number): number[] {
...
export function boundaryFraction(cells: number[], g: Graph): number {
  ...
  for (const c of cells) {
    let hasOut = false
    for (let p = g.offsets[c]!; p < g.offsets[c + 1]!; p++) if (!inSet.has(g.adj[p]!)) {
      hasOut = true
      break
    }
    if (hasOut) boundary++
...
export const degreeOf = (g: Graph, v: number): number => g.offsets[v + 1]! - g.offsets[v]!
...
export function beat(tone: Int8Array, g: Graph, moved: Uint8Array, rng: Rng, arrow: number, cohesion: number): void {
  const { offsets, adj } = g
  const N = tone.length
  moved.fill(0)
...
export function sameSignNeighbors(tone: Int8Array, g: Graph, c: number, sign: number): number {
  let k = 0
  for (let p = g.offsets[c]!; p < g.offsets[c + 1]!; p++) if (tone[g.adj[p]!] === sign) k++
  return k
}

export function largestPositiveCluster(tone: Int8Array, g: Graph): number[] {
  const { offsets, adj } = g
  const N = tone.length
...
export function emergeSelf(g: Graph, rng: Rng, moved: Uint8Array, opts?: { beats?: number; density?: number }): { tone: Int8Array; cluster: number[] } {
  const N = g.cellCount
...
export function discreteArrow(tone: Int8Array, g: Graph, beatIndex: number, period: number): number {
  const N = tone.length
  let created = 0
