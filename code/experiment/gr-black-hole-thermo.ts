// Quantitative black-hole thermodynamics on the substrate. The substrate already supplies the three
// non-trivial inputs from earlier experiments:
//   - the AREA LAW S = A / 4G (entanglement entropy scales with the boundary, P15 / P33),
//   - the HAWKING temperature T = kappa / 2pi, derived from the Unruh detector response (P71),
//   - the de Sitter expansion a(t) = e^(H t), H = ln(R)/3 from the measured shell growth (cosmology).
// GIVEN those, the FULL quantitative thermodynamics is fixed and self-consistent. This experiment
// verifies the quantitative relations that "full GR" black-hole thermodynamics must satisfy, and reports
// the substrate's own de Sitter horizon numbers. Geometric units G = c = hbar = k_B = 1.
//
// Run: npx tsx --no-warnings=ExperimentalWarning code/experiment/gr-black-hole-thermo.ts

import { pathToFileURL } from 'node:url'
import { buildAddressing } from '~/substrate/coxeter/addressing-3434'
...
export function main(): void {
  ...
  console.log(`\nVerdict: quantitative black-hole thermodynamics is self-consistent and complete: ${all ? 'CONFIRMED' : 'INCOMPLETE'}`)
  console.log('(First law, Smarr, Bekenstein saturation, negative heat capacity, M^3 lifetime, plus the substrate\'s')
  console.log(' own de Sitter horizon numbers. The area law and Hawking T are the substrate inputs; the rest follows.)')
