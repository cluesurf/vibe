...
export function matrixKey(matrix: number[][]): string {
  return matrix.flat().map((value) => Math.round(value * 1e5)).join(',')
}
...
export function buildCoxeterMatrixMesh(symbol: number[], maxCells: number): { shells: number[]; adjacency: number[][] } {
  ...
  const adjacency = matrices.map((matrix) =>
    generators.map((generator) => index.get(matrixKey(multiply(matrix, generator))) ?? -1),
  )
