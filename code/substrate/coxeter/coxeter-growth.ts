...
function finiteGrowthFromExponents(exponents: number[]): Poly {
  let p: Poly = [1n]
  for (const e of exponents) p = mul(p, qInteger(e + 1))
  return p
}
...
function componentExponents(component: number[], labels: number[]): number[] | null {
  ...
  if (size === 3) {
    ...
    if (key === '3,3') return [1, 2, 3] // A3
    if (key === '4,3' || key === '3,4') return [1, 3, 5] // B3
    if (key === '5,3' || key === '3,5') return [1, 5, 9] // H3
    return null
  }
  if (size === 4) {
    const key = edgeLabels.join(',')
    if (key === '3,3,3') return [1, 2, 3, 4] // A4
    if (key === '4,3,3' || key === '3,3,4') return [1, 3, 5, 7] // B4
    if (key === '3,4,3') return [1, 5, 7, 11] // F4
    if (key === '5,3,3' || key === '3,3,5') return [1, 11, 19, 29] // H4
    return null
  }
  return null
}
...
export function coxeterGrowthSeries(labels: number[]): Rational {
  ...
  if (shiftAmount > 0) num = shift(num, shiftAmount)
  if (shiftAmount < 0) den = shift(den, -shiftAmount)
  if (den[0]! < 0n) {
    num = scale(num, -1n)
    den = scale(den, -1n)
  }
