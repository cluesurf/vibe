...
function dihedral(s1: { c: V3; r2: number }, s2: { c: V3; r2: number }): number {
...
function cellAt(r0: number, base: { verts: V3[]; faces: number[][] }): { verts: V3[]; spheres: ({ c: V3; r2: number } | null)[] } {
  const s = r0 / Math.sqrt(3)
  const verts = base.verts.map((v) => ({ x: v.x * s, y: v.y * s, z: v.z * s }))
  const spheres = base.faces.map((f) => faceSphere([verts[f[0]!]!, verts[f[1]!]!, verts[f[2]!]!]))
...
function solveRadius(base: { verts: V3[]; faces: number[][] }): number {
  ...
  outer: for (let i = 0; i < base.faces.length; i++) {
    for (let j = i + 1; j < base.faces.length; j++) {
      const shared = base.faces[i]!.filter((v) => base.faces[j]!.includes(v))
      if (shared.length === 2) {
        fa = i
        fb = j
        break outer
...
export function hyperbolicDodecagrid(input: { depth: number; connectThreshold: number; maxVertices?: number }): Graph {
  const base = dodecahedron()
  const r0 = solveRadius(base)
  const central = cellAt(r0, base)

  const key = (v: V3): string => `${Math.round(v.x * 1e4)},${Math.round(v.y * 1e4)},${Math.round(v.z * 1e4)}`
  ...
  const cellKey = (verts: V3[]): string => {
    ...
    return key({ x: cx / verts.length, y: cy / verts.length, z: cz / verts.length })
  }
  ...
  for (let d = 0; d < input.depth && vx.length < cap; d++) {
    ...
    for (const cellVerts of frontier) {
      ...
      for (const face of base.faces) {
        const sph = faceSphere([cellVerts[face[0]!]!, cellVerts[face[1]!]!, cellVerts[face[2]!]!])
        if (!sph) {
          continue
        }
  ...
  for (let i = 0; i < n; i++) {
    oneMinus[i] = Math.max(1e-9, 1 - ((vx[i] ?? 0) ** 2 + (vy[i] ?? 0) ** 2 + (vz[i] ?? 0) ** 2))
  }
  for (let i = 0; i < n; i++) {
    for (let j = i + 1; j < n; j++) {
  ...
  const manifold: ManifoldSpec = { form: 'hyperbolic', dimension: 3, curvature: -1 }
  const embedding: Embedding = { form: 'embedding', dimension, signature: 'riemannian', coords, manifold }
