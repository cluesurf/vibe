...
const reduce = (value: number, prime: number): number => ((value % prime) + prime) % prime
    ...
    for (let b = 0; b < prime; b++) {
      for (let c = 0; c < prime; c++) {
        for (let d = 0; d < prime; d++) {
          if (reduce(a * d - b * c, prime) === 1) group.push([a, b, c, d])
        }
      }
    }
  }
...
export function multiplyModP(left: MatrixModP, right: MatrixModP, prime: number): MatrixModP {
  return [
...
export function equalsModP(left: MatrixModP, right: MatrixModP): boolean {
...
export function centre(prime: number): MatrixModP[] {
  ...
  for (let lambda = 0; lambda < prime; lambda++) {
    if (reduce(lambda * lambda, prime) === 1) result.push([lambda, 0, 0, lambda])
  }
  return result
}
