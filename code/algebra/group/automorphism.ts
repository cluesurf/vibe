...
function matrixKey(matrix: number[][]): string {
  return matrix
    .map(column =>
      column.map(value => Math.round(value * 64) / 64).join(','),
    )
    .join(';')
}
...
export function weylGroupOrder(roots: number[][]): number {
  ...
  const generators = roots.map(root =>
    reflectionMatrix(root, dimension),
  )
...
export function automorphismGroupOrder(roots: number[][]): number {
  ...
  const coordinates = roots.map(root =>
    solveSymmetric(
      gram,
      basis.map(b => dotVec(b, root)),
    ),
  )
