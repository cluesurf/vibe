...
const O: C = [0, 0]
const I: C = [1, 0]
const omega = (k: number): C => [
  Math.cos((2 * Math.PI * k) / 3),
  Math.sin((2 * Math.PI * k) / 3),
]
const epsilon = (k: number): C => [
  Math.cos((4 * Math.PI * k) / 9),
  Math.sin((4 * Math.PI * k) / 9),
]
const times = (a: C, b: C): C => [
  a[0] * b[0] - a[1] * b[1],
  a[0] * b[1] + a[1] * b[0],
]
...
export const FOURIER: Matrix3 = matrix3(
  [0, 1, 2].map(j =>
    [0, 1, 2].map(k => times(omega(j * k), [0, -1 / Math.sqrt(3)])),
  ),
)
...
const ninth = (k: number): C => [
  Math.cos((2 * Math.PI * k) / 9),
  Math.sin((2 * Math.PI * k) / 9),
]
...
export const SU3_SUBGROUPS = {
  ...
  sigma1080: {
    order: 1080,
    generators: [SIGN, SHIFT, GOLDEN, VALENTINER],
  },
} as const
