...
export function directorLoop(input: { winding: number; steps: number }): number[] {
  const { winding, steps } = input
  return Array.from({ length: steps }, (_, k) => (2 * Math.PI * winding * k) / steps)
}
...
function spinorZRotor(beta: number): ComplexMatrix {
  const half = beta / 2
  return [
    [complex({ re: Math.cos(half), im: -Math.sin(half) }), complex({ re: 0, im: 0 })],
    [complex({ re: 0, im: 0 }), complex({ re: Math.cos(half), im: Math.sin(half) })],
  ]
}
...
export function spinorHolonomy(input: { winding: number; steps: number }): ComplexMatrix {
  const { winding, steps } = input
  const stepRotor = spinorZRotor((2 * Math.PI * winding) / steps)
  let holonomy = cmIdentity(2)
  for (let k = 0; k < steps; k++) holonomy = cmMultiply(holonomy, stepRotor)
  return holonomy
}
...
export function disclinationHolonomy(input: { winding: number; steps: number }): {
  spinorIsMinusOne: boolean
  spinorIsPlusOne: boolean
  vectorReturnsToSelf: boolean
} {
  const holonomy = spinorHolonomy(input)
  const spinorIsMinusOne = cmIsScalar(holonomy, complex({ re: -1, im: 0 }))
  const spinorIsPlusOne = cmIsScalar(holonomy, complex({ re: 1, im: 0 }))
