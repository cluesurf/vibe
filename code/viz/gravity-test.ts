...
function beat(tone: Int8Array, edges: number[][], neighbors: number[][], moved: Uint8Array, rng: Rng, arrow: number): void {
  moved.fill(0)
  ...
  for (const e of edges) {
    ...
    if ((a === 1 && b === -1) || (a === -1 && b === 1)) {
      ...
      const empty = a === 0 ? v : w
      const q = tone[charge]!
      if (agree(empty, q, charge) >= agree(charge, q, empty) || rng.next() < 0.02) {
        tone[empty] = q
        tone[charge] = 0
        moved[v] = 1
        moved[w] = 1
...
function bfsDistance(neighbors: number[][], source: number, n: number): Int32Array {
...
function makeLump(neighbors: number[][], center: number, size: number, n: number): number[] {
  ...
  while (frontier.length > 0 && lump.length < size) {
    ...
    for (const u of frontier) {
      ...
      lump.push(u)
      for (const w of neighbors[u]!) if (!inLump[w]) {
        inLump[w] = 1
        next.push(w)
      }
    }
...
export function gravityTest(input?: { maxCells?: number; lumpSize?: number; beats?: number }): {
  maxCells: number
  lumpSize: number
  trials: { startGap: number; gaps: number[]; approached: boolean; merged: boolean }[]
  ...
  const separation = (tone: Int8Array, lumpA: number[], lumpB: number[]): number => {
    const distA = new Int32Array(n).fill(-1)
    let frontier: number[] = []
    for (const i of lumpA) if (tone[i] !== 0) {
      distA[i] = 0
      frontier.push(i)
    }
    ...
    while (frontier.length > 0) {
      ...
      for (const u of frontier) {
        ...
        for (const w of neighbors[u]!) if (distA[w] === -1) {
          distA[w] = distA[u]! + 1
          next.push(w)
        }
      }
  ...
  const trials: { startGap: number; gaps: number[]; approached: boolean; merged: boolean }[] = []
  for (const startGap of [3, 6, 10]) {
    ...
    for (const i of lumpA) tone[i] = (rng.next() < 0.5 ? 1 : -1) as -1 | 1
    for (const i of lumpB) tone[i] = (rng.next() < 0.5 ? 1 : -1) as -1 | 1
  ...
  return { maxCells, lumpSize, trials, anyLongRangeAttraction, onlyContactInteraction, gravityMissing }
}

export function main(): void {
  const r = gravityTest()
  console.log('Gravity hole-finder: do two lumps of matter attract across a gap on the {5,3,4}?')
  console.log('')
  console.log(`  ${r.maxCells} cells, two lumps of ${r.lumpSize} cells (neutral matter)`)
  console.log('')
  for (const tr of r.trials) {
    console.log(`  start gap ${tr.startGap}: separation over beats = [${tr.gaps.join(', ')}]  approached: ${tr.approached}  merged: ${tr.merged}`)
  }
  console.log('')
  console.log(`  LONG-RANGE attraction (far lumps fall together, gravity): ${r.anyLongRangeAttraction}`)
  console.log(`  only CONTACT interaction (merge when touching, ignore across a gap): ${r.onlyContactInteraction}`)
  console.log('')
  if (r.gravityMissing) {
    console.log('  => HOLE FOUND. The bare rule has contact forces (merge on touch, P110) but NO long-range')
    console.log('     attraction. Matter does not fall together across space, so there is NO GRAVITY in the')
    console.log('     five base things as they stand. Gravity is a missing ingredient.')
  } else {
    console.log('  => Surprising, the far lumps approached, an emergent long-range attraction worth probing.')
  }
}
