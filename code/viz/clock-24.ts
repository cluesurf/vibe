...
function tick(r: number, deg: number, len: number, wid: number, fill: string): string {
...
function buildSvg(): string {
  ...
  // the two rings
  parts.push(`<circle cx="${CX}" cy="${CY}" r="${R_IN}" fill="none" stroke="${ZINC[400]}" stroke-width="1.4"/>`)
  parts.push(`<circle cx="${CX}" cy="${CY}" r="${R_OUT}" fill="none" stroke="${ZINC[700]}" stroke-width="2.5"/>`)

  // 12 inner ticks (lighter, smaller), then 12 outer ticks (dark, larger), on top
  for (let i = 0; i < COUNT; i++) parts.push(tick(R_IN, angleDeg(i), 30, 11, ZINC[400]))
  for (let i = 0; i < COUNT; i++) parts.push(tick(R_OUT, angleDeg(i), 46, 15, ZINC[800]))
...
function main(): void {
  const out = '/Users/lancepollard/base/crew/cluesurf/deck/vibe/make/24-clock.svg'
