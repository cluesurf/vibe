...
typecheck clean, 103 of 103 known-answer tests pass.
...
P70 to P74 are the deep physics frontiers: the Born-rule derivation
(probability is the squared amplitude because amplitude is the square
root of a vibe density, so fair sampling gives |c|^2), Hawking radiation
(a thermal across-horizon spectrum at kappa/2pi, temperature scaling as
1/M, and a Page curve that turns over), the full nonlinear Einstein
equation (the Friedmann system, consistent through the Bianchi
identity), the fully-discrete graviton (a lattice operator that is
gauge-invariant, massless, and spin-2), and a height-changing cluster
move that unblocks the large-N free-energy crossing.
