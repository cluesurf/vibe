...
<p align='center'>
  A discrete possible universe Φ
</p>
...
one local rule to its own 24 slots (the knit: the directions are grouped
into lines, the lines into couples, and a fixed 9-state table turns the
tones over), and then every tone slides one cell along its direction.
...
between domains are real, thin, permanent objects. That tiling is the
model's condensate, and the arrow of time is what wound it.
...
machine precision, and carries a fixed phase class of its own.
...
**Where the program stands (2026-09-02).** The discrete quantum
kinematics is complete on the proposed cross-plane palindrome knit:
...
swept. The proposed knit is under its acceptance programme now: every
canon pillar re-measured under it before anything is committed, with
the verdict data recorded as experiments. The quantum sector map is
...
| 🔵 | structure derived, dynamics open | 19 |
| 📌 | free input, not predicted | 1 |
| ❌ | open | 0 |
| ⛔ | blocked on a base decision | 2 |
...
| | | [`E-FRC-0002`](test/experiment/gauge/anomaly-cancellation-octonion.ts) |
| dynamical gauge field | 🔵 | A carrier exists on the committed rule and its bare coupling is measured (unit absorption per encounter): what remains is the emission vertex and the gauge algebra, open work rather than a base decision. |
...
| fine structure constant value | 🔵 | The base has provably no coupling knob (the bare vertex is unity, measured), so alpha is a coarse quantity, collision rarity under dilution and running. |
...
| distinct generation masses | 🔵 | The hierarchy scale and ratios are derived and the growth-written condensate supplies an asymmetric vacuum, so the open dynamics is the triality-to-shell tie, not a base addition. |
...
[Vibe Theory](https://doi.org/10.5281/zenodo.20694262) treats reality
as one thing, a growing crystal of experience, and the image above is
its simplest face, the hyperbolic {7,3} tessellation, meant literally:
...
touching tiles **note** (experience) one another. A tile is a quantum
of experience, a patch of tiles is a thing or a mind, the whole
crystal is the universe, and its ever-receding growing edge is the
present. Physical reality is the thin skin at the crystal's cusp,
worked from within: consciousness is the grid, the physical universe
its projection.
...
That the base genuinely IS experience is the model's one **axiom**,
and like every axiom it is unfalsifiable: no lab reading distinguishes
a felt universe from an identical structural one. It is held as a
frame, never as a result. Everything under the frame is a concrete
discrete dynamical system, and that part is highly falsifiable: every
deep claim carries a control where the answer should be no (the spinor
that appears on {3,4,3,4} and provably fails on {5,3,4}, the area-law
exponent that had to land near 2, the isotropy that could have stayed
anisotropic), negatives are kept, and a test that cannot fail counts
for nothing. So "vibe theory is unfalsifiable" is half right: the
axiom is, the physics under it is not, and the experiments confirm
structure, never feeling.
...
reproducible. Real numbers appear only as measured outputs, never in
the base, and much of the code was written with AI assistance, which
changes nothing about trusting it: run it and verify. Each question is
one experiment in `test/experiment/<category>/`, a single `experiment`
...
L3 is the prize, L1 and L2 are necessary groundwork labeled as such,
and L0 marks what is assumed. The full rubric and the rules the runner
...
code and the catalog are one source of truth, sorted strongest first.
As of the latest regeneration it holds **880 experiments across 18
categories**:

| total | L3 emergent, novel | L2 known physics | L1 known math | L0 circular | backing a paper claim |
| ----: | -----------------: | ---------------: | ------------: | ----------: | --------------------: |
|   880 |                 47 |              606 |           213 |          14 |                   559 |

The largest categories are selves, quantum, foundations, gauge,
gravity, and cosmology. The standing depth audit regrades overclaimed
...
**[concepts cross-index](note/experiment/concepts.md)**, curated
reading paths, and a guide for adding your own.
...
- **rule**: the knits (the committed charge table, the momentum
  collision, the proposed traveller palindromes `lineHop` and
  `linePalindrome`), streaming, growth, and the discrete C, P, T
  transforms.
