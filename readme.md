...
| P2      | a dynamics that favors manifold-like order  | candidate progress      | exact enumeration (no sampling bias) confirms the smeared action drives the true measure to manifold-like orders (manifold fraction to 100 percent) with a dimension sweet spot at moderate coupling; only the large-N entropic regime remains, needing a scalable uniform-measure sampler |
