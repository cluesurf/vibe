...
| P2      | a dynamics that favors manifold-like order  | candidate progress      | the smeared Benincasa-Dowker action gives near-2D ensembles where the sharp action collapses to layered; warm starts show the manifold phase is a stable basin (sprinkle stays ~1.9, layered climbs from 0.35, sharp action erodes to 1.51) |
...
| P7      | quantum statistics from a classical base    | quantified + precise    | CHSH S climbs 1.0 to 4.0 with correlation; determinism (monism) makes violation possible, but a generic shared past gives S~1 while an aligned one reaches 4, so the correlation must be aligned, not just present |
