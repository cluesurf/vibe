...
| P2      | a dynamics that favors manifold-like order  | solved at scale         | a correct uniform-measure sampler (validated vs exact enumeration) reaches the entropic regime and shows a first-order transition: at N=128 the smeared action makes manifold spacetime a stable phase coexisting with the layered phase, so the dynamics produces spacetime (free-energy crossing is the remaining refinement) |
...
| P8      | gauge, fermion, confinement, index, condensate | validated (A-C + index + Schwinger condensate) | U(1) couples to the fermion; 3D SU(2) confines (string tension 1.32 to 0.40); the overlap index equals the gauge topological charge (lattice Atiyah-Singer); and in a dynamical gauge field a chiral condensate forms from the anomaly (zero free, nonzero gauged, the Schwinger model) |
