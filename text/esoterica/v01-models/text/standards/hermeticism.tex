...
A modern Hermetic teaching states the tradition as \textbf{seven principles}. They are usually given in this fixed order.

\begin{enumerate}
 \item \textbf{Mentalism}. The All is Mind. The universe is mental, held in the mind of the All.
 \item \textbf{Correspondence}. As above, so below. As below, so above. The same pattern repeats across the planes.
 \item \textbf{Vibration}. Nothing rests. Everything moves and vibrates. Differences are differences of rate.
 \item \textbf{Polarity}. Everything is dual and has two poles. Opposites are the same thing in degree, such as heat and cold.
 \item \textbf{Rhythm}. Everything swings between its poles. The pendulum-swing of the rise answers the swing of the fall.
 \item \textbf{Cause and Effect}. Every cause has an effect and every effect has a cause. Nothing happens by chance.
 \item \textbf{Gender}. Everything has a masculine and a feminine principle. Gender works on all planes to generate.
\end{enumerate}

These seven principles are not from the ancient Corpus Hermeticum. They come from the 1908 work the \textbf{Kybalion}, by \textbf{William Walker Atkinson}, a New Thought writer. They restate genuine Hermetic themes, above all mentalism and correspondence, in a modern form.
