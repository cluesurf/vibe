...
a will is, how a beat acts, and what is conserved, and then records the principal emergent results as formal
...
$V$ & the set of vibes (cells) of the mesh & \\
$v_1, v_2$ & the vibes, elements of $V$ & \\
$B$ & the beat, the discrete tick & \\
$b$ & the beat index, in $\mathbb{Z}$ & \\
$T = \{-1, 0, +1\}$ & the tone alphabet, the three tone values & \\
$C$ & the $24$ directions of a vibe, the coin & \\
$c$ & a direction, an element of $C$ & \\
$n(v, c)$ & the neighbour (note) of $v$ across face $c$ & \\
$t(v, c)$ & the tone in slot $c$ of cell $v$, in $T$ & \\
$w$ & a will, a full configuration, a map $V \times C \to T$ & \\
$Q$ & total charge, the sum of all tones & \\
$R = \mathsf{Collide} \circ \mathsf{Stream}$ & the rule, one beat & \\
$\mathsf{Stream}$ & the global transport map on wills & \\
$\mathsf{Collide}$ & the local interaction map on wills & \\
\bottomrule
\end{tabular}
...
\begin{definition}[The flow tuple]
...
A \emph{will} is a map $w : V \times C \to T$. The value $w(v, c)$, also written $t(v, c)$, is the tone in
directional slot $c$ of cell $v$, read as the charge currently leaving $v$ through face $c$. The set of all wills
is $T^{\,V \times C}$. A single cell has local will space $T^{C}$ of size $3^{24}$. There is no other state, in
...
The \emph{scalar tone} of cell $v$ in will $w$ is $\mathsf{tone}(v) = \sum_{c \in C} t(v, c)$, an integer in
$[-24, 24]$. The \emph{total charge} is
\[
Q(w) \;=\; \sum_{v \in V} \; \sum_{c \in C} t(v, c),
\]
...
neighbour. Writing $\overline{c}$ for the opposite direction, the streamed will is
\[
(\mathsf{Stream}\, w)(v, c) \;=\; w\big(n(v, \overline{c}), \, c\big),
\]
that is, the value now in slot $c$ of $v$ is the value that the back-neighbour was sending forward along line $c$.
Each slot has exactly one source and one destination, so $\mathsf{Stream}$ is a bijection of wills, a global
...
cell simultaneously,
\[
(\mathsf{Collide}\, w)(v, \cdot) \;=\; \mathsf{col}\big(w(v, \cdot)\big).
\]
The local map $\mathsf{col}$ is required to be (i) a \emph{bijection} of $T^{C}$, hence reversible, and (ii)
\emph{charge-conserving} on the cell, $\sum_c \mathsf{col}(u)_c = \sum_c u_c$ for every local will $u$. It is
...
\]
The will at beat $b$ from initial will $w_0$ is $w_b = R^{\,b}(w_0)$. Time is the iteration index $b$, there is no
external clock.
\end{definition}
...
For every will $w$, $\; Q(R\, w) = Q(w)$. Hence $Q$ is constant along every trajectory.
\end{proposition}
...
\begin{proposition}[Reversibility of the rule]
$R$ is a bijection of wills, with inverse
$R^{-1} = \mathsf{Stream}^{-1} \circ \mathsf{Collide}^{-1}$. Hence every will has a unique past and a
unique future, and $R^{-N}(R^{N}(w)) = w$ exactly.
\end{proposition}
...
\begin{result}[The arrow orients the create-annihilate cycle]
...
\begin{result}[Fermi statistics from odd Hopf charge]
...
\begin{proposition}[Tone extends the coin to $\mathrm{SO}(10)$]
...
\[
\mathsf{decay} \;=\; \frac{2 \ln(1/z)}{\ln q}, \qquad z = \frac{1}{q} \;\Rightarrow\; \mathsf{decay} = 2,
\]
...
two $-1$ inputs, computes $\mathrm{NAND}$, which is functionally complete, so it builds any Boolean circuit and in
...
By the nothing-added discipline of Principle~\ref{prin:nothing-added}, every result above is a property of the
four base things of the flow $F = \{M, B, T, R\}$ or an emergent consequence of them. No fifth ingredient is used.
Where a phenomenon does not emerge, the negative is recorded as a formal statement, not hidden.

\begin{result}[Recorded negatives]
The following are recorded negatives, established by the same standard as the positives.
\begin{enumerate}
\item \emph{No spontaneous selves from generic initial data.} The pure rule from a generic will churns, a
